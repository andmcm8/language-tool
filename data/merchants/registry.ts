import { MerchantConfig } from '@/types/merchant';
import m_1741_pub_and_grill from './1741-pub-and-grill.json';
import m_1754_house_restaurant_and_inn from './1754-house-restaurant-and-inn.json';
import m_19th_hole from './19th-hole.json';
import m_2_wives_brick_oven_eatery from './2-wives-brick-oven-eatery.json';
import m_374_kitchen_cocktails from './374-kitchen-cocktails.json';
import m_50_west from './50-west.json';
import m_7_brew_coffee from './7-brew-coffee.json';
import m_7_brew from './7-brew.json';
import m_80_proof_american_kitchen_bar from './80-proof-american-kitchen-bar.json';
import m_903_brewers from './903-brewers.json';
import abbotsford from './abbotsford.json';
import abc_pizza_restaurant from './abc-pizza-restaurant.json';
import abigails_grille_and_wine_bar from './abigails-grille-and-wine-bar.json';
import abis from './abis.json';
import ac_petersons_farms from './ac-petersons-farms.json';
import ackert_electric_and_barbershop from './ackert-electric-and-barbershop.json';
import adriatiks_restaurant_and_pizzeria from './adriatiks-restaurant-and-pizzeria.json';
import aero_diner from './aero-diner.json';
import aeronaut_brewing_company from './aeronaut-brewing-company.json';
import airway_restaurant from './airway-restaurant.json';
import akita_ramen from './akita-ramen.json';
import akita from './akita.json';
import alameda_brewing_co from './alameda-brewing-co.json';
import albany_avenue_food_mart_and_deli from './albany-avenue-food-mart-and-deli.json';
import alebrijes_mexican_fusion from './alebrijes-mexican-fusion.json';
import alinas_patisserie from './alinas-patisserie.json';
import almanac from './almanac.json';
import along_came_carol from './along-came-carol.json';
import alpen_pantry from './alpen-pantry.json';
import alpine_eatery from './alpine-eatery.json';
import altones_italian_restaurant from './altones-italian-restaurant.json';
import alvarium_beer_co from './alvarium-beer-co.json';
import ambrosia_bakery from './ambrosia-bakery.json';
import american_mexican_diner from './american-mexican-diner.json';
import anas_kitchen from './anas-kitchen.json';
import anaya_sushi from './anaya-sushi.json';
import anchor_cafe_sweets from './anchor-cafe-sweets.json';
import anchor_spa from './anchor-spa.json';
import andros_diner_restaurant from './andros-diner-restaurant.json';
import angelos_restaurant from './angelos-restaurant.json';
import angies_pizza from './angies-pizza.json';
import anna_liffeys_irish_pub_restaurant from './anna-liffeys-irish-pub-restaurant.json';
import antep_evi from './antep-evi.json';
import anya from './anya.json';
import applausi from './applausi.json';
import applebees from './applebees.json';
import aqui_me_quedo_restaurant from './aqui-me-quedo-restaurant.json';
import arch_street_tavern from './arch-street-tavern.json';
import archie_moores from './archie-moores.json';
import area_143 from './area-143.json';
import arezzo from './arezzo.json';
import armstrong_dining_hall from './armstrong-dining-hall.json';
import aroma_joes from './aroma-joes.json';
import aroma from './aroma.json';
import aroom from './aroom.json';
import artisan_roast from './artisan-roast.json';
import artisan from './artisan.json';
import ashleys_ice_cream from './ashleys-ice-cream.json';
import aspen from './aspen.json';
import aspetuk_brew_lab from './aspetuk-brew-lab.json';
import astin_js from './astin-js.json';
import asylum_street_pizza from './asylum-street-pizza.json';
import athena_diner from './athena-diner.json';
import atwater_brewery from './atwater-brewery.json';
import augie_and_rays from './augie-and-rays.json';
import august_moon from './august-moon.json';
import aux_delices from './aux-delices.json';
import away_days_brewing_co from './away-days-brewing-co.json';
import awesom_sauce_caribbean_restaurant from './awesom-sauce-caribbean-restaurant.json';
import azteca_market from './azteca-market.json';
import back_9_social from './back-9-social.json';
import back_east_brewing_company from './back-east-brewing-company.json';
import back_porch from './back-porch.json';
import baerlic_brewery from './baerlic-brewery.json';
import bagel_grove from './bagel-grove.json';
import bagel_king from './bagel-king.json';
import bagelicious_ii from './bagelicious-ii.json';
import bagelman from './bagelman.json';
import baingan from './baingan.json';
import baiten_bakery from './baiten-bakery.json';
import baja_cocina from './baja-cocina.json';
import bajas_mexican_restaurant from './bajas-mexican-restaurant.json';
import bakers_dozen from './bakers-dozen.json';
import bamboo_leaf from './bamboo-leaf.json';
import bar_louie from './bar-louie.json';
import bar_max from './bar-max.json';
import bar_new_haven from './bar-new-haven.json';
import bar_noodle from './bar-noodle.json';
import bar_rosso from './bar-rosso.json';
import barbary_coast from './barbary-coast.json';
import barcelona_wine_bar_stamford from './barcelona-wine-bar-stamford.json';
import barcelona from './barcelona.json';
import bare_burger from './bare-burger.json';
import barnaan from './barnaan.json';
import barosa_indian_kitchen_bar from './barosa-indian-kitchen-bar.json';
import barracuda from './barracuda.json';
import barrel_brothers_brewing_company from './barrel-brothers-brewing-company.json';
import bartaco from './bartaco.json';
import barvida from './barvida.json';
import baskin_robbins from './baskin-robbins.json';
import basta_tratoria from './basta-tratoria.json';
import bastard_barista from './bastard-barista.json';
import bears_smokehouse_barbecue from './bears-smokehouse-barbecue.json';
import bears_smokehouse_bbq from './bears-smokehouse-bbq.json';
import bedford_thai from './bedford-thai.json';
import beijing from './beijing.json';
import bella_fiore_restaurante_modelo_bar_grill from './bella-fiore-restaurante-modelo-bar-grill.json';
import bella_napoli_pizza_restaurant from './bella-napoli-pizza-restaurant.json';
import bella_napoli from './bella-napoli.json';
import bella_sicily from './bella-sicily.json';
import belle_vie_cafe from './belle-vie-cafe.json';
import bennys from './bennys.json';
import bereket_turkish_restaurant from './bereket-turkish-restaurant.json';
import berkins_blend_cafe from './berkins-blend-cafe.json';
import bertuccis from './bertuccis.json';
import bess_eaton from './bess-eaton.json';
import best_video_coffee_bar from './best-video-coffee-bar.json';
import between_rounds_bagels from './between-rounds-bagels.json';
import between_rounds from './between-rounds.json';
import bf_clydes_cider_mill from './bf-clydes-cider-mill.json';
import bianca from './bianca.json';
import big_fish_sushi from './big-fish-sushi.json';
import bills_crossroads_cafe from './bills-crossroads-cafe.json';
import billys_bakery from './billys-bakery.json';
import bin_228 from './bin-228.json';
import bin228 from './bin228.json';
import birdhouse_coffee from './birdhouse-coffee.json';
import bistro_coco from './bistro-coco.json';
import bistro_mediterranean_tapas_bar from './bistro-mediterranean-tapas-bar.json';
import bistro_v from './bistro-v.json';
import black_angus from './black-angus.json';
import black_bear_saloon from './black-bear-saloon.json';
import black_eyed_sallys from './black-eyed-sallys.json';
import black_pond_brews from './black-pond-brews.json';
import black_rock_tavern_and_restaurant from './black-rock-tavern-and-restaurant.json';
import black_sheep_coffee from './black-sheep-coffee.json';
import blackies from './blackies.json';
import blackwood_cafe_bar from './blackwood-cafe-bar.json';
import blairally_vintage_arcade from './blairally-vintage-arcade.json';
import bliss_vybz_west_indian_restaurant from './bliss-vybz-west-indian-restaurant.json';
import bloom_bake_shop from './bloom-bake-shop.json';
import blue_camel_cafe from './blue-camel-cafe.json';
import blue_colony_diner from './blue-colony-diner.json';
import blue_gate_boulanger_patissier from './blue-gate-boulanger-patissier.json';
import blue_orchid from './blue-orchid.json';
import blue_state_coffee from './blue-state-coffee.json';
import bobby_bs_roti_shop from './bobby-bs-roti-shop.json';
import bobby_vs_restaurant_sports_bar from './bobby-vs-restaurant-sports-bar.json';
import bobbys_apizza from './bobbys-apizza.json';
import bobbys_burger_palace from './bobbys-burger-palace.json';
import bohemian_pizza_and_taco from './bohemian-pizza-and-taco.json';
import bollywood_dreams from './bollywood-dreams.json';
import bombay_olive from './bombay-olive.json';
import bonani_kitchen from './bonani-kitchen.json';
import bonchon_chicken from './bonchon-chicken.json';
import bonessesss_apizza from './bonessesss-apizza.json';
import boost_bowls from './boost-bowls.json';
import border_grill from './border-grill.json';
import boston_bar from './boston-bar.json';
import boston_fish_n_chips from './boston-fish-n-chips.json';
import boulders_bar_grill from './boulders-bar-grill.json';
import bozrah_pizza_restaurant from './bozrah-pizza-restaurant.json';
import bradfords_grill_tavern from './bradfords-grill-tavern.json';
import bradley_wall from './bradley-wall.json';
import bramble from './bramble.json';
import branford_townhouse_restaurant from './branford-townhouse-restaurant.json';
import bravo from './bravo.json';
import bread_chocolate from './bread-chocolate.json';
import bread_street_kitchen_bar from './bread-street-kitchen-bar.json';
import brendas_cupcakes from './brendas-cupcakes.json';
import brennans_by_the_beach from './brennans-by-the-beach.json';
import brennans_shebeen from './brennans-shebeen.json';
import brewdog from './brewdog.json';
import brewery_legitimus from './brewery-legitimus.json';
import brewett_city_pub from './brewett-city-pub.json';
import brewport from './brewport.json';
import brick_basil_wood_fired_pizza_company from './brick-basil-wood-fired-pizza-company.json';
import brick_oven_on_main from './brick-oven-on-main.json';
import brickhouse_bar_grill from './brickhouse-bar-grill.json';
import brickoven_pizza from './brickoven-pizza.json';
import bridgewater_village_store_bistro from './bridgewater-village-store-bistro.json';
import brisas_latin_fusion from './brisas-latin-fusion.json';
import broasterant from './broasterant.json';
import brookside_bagels from './brookside-bagels.json';
import brown_beard_brewing from './brown-beard-brewing.json';
import brunos_cafe from './brunos-cafe.json';
import brunos_pizza from './brunos-pizza.json';
import bryac_restaurant_raw_bar from './bryac-restaurant-raw-bar.json';
import buckland_billiards from './buckland-billiards.json';
import buckland_grill_pizza from './buckland-grill-pizza.json';
import bucks_bar from './bucks-bar.json';
import buen_gusto from './buen-gusto.json';
import buffalo_bills from './buffalo-bills.json';
import buffalo_wild_wings from './buffalo-wild-wings.json';
import burrito_shack from './burrito-shack.json';
import butchers_bakers from './butchers-bakers.json';
import butterfly_chinese_restaurant from './butterfly-chinese-restaurant.json';
import buttonwood_farm_ice_cream from './buttonwood-farm-ice-cream.json';
import c_j_eaterieslottery_bar from './c-j-eaterieslottery-bar.json';
import cadys_tea_room from './cadys-tea-room.json';
import cafe_amici from './cafe-amici.json';
import cafe_ca_nole from './cafe-ca-nole.json';
import cafe_feva from './cafe-feva.json';
import cafe_hanover from './cafe-hanover.json';
import cafe_luca_belle from './cafe-luca-belle.json';
import cafe_portrait from './cafe-portrait.json';
import cafe_sofia from './cafe-sofia.json';
import cafe_susu_by_suitsupply from './cafe-susu-by-suitsupply.json';
import cafe_tinto from './cafe-tinto.json';
import cafeteria_morla_restaurant from './cafeteria-morla-restaurant.json';
import caffe_bene from './caffe-bene.json';
import caffe_nero from './caffe-nero.json';
import caffe_strega from './caffe-strega.json';
import caio_cafe from './caio-cafe.json';
import cairngorm_coffee_shop from './cairngorm-coffee-shop.json';
import caius_farm_brewery from './caius-farm-brewery.json';
import cajun_cafe from './cajun-cafe.json';
import calabria_coffee from './calabria-coffee.json';
import california_pizza_kitchen from './california-pizza-kitchen.json';
import cambridge_brew_pub from './cambridge-brew-pub.json';
import camerons_deli from './camerons-deli.json';
import camilles_wood_fired_pizza from './camilles-wood-fired-pizza.json';
import candlewood_inn from './candlewood-inn.json';
import canggio_restaurant_and_bar from './canggio-restaurant-and-bar.json';
import canoe from './canoe.json';
import cantina_mexicana from './cantina-mexicana.json';
import cantina_ristorante from './cantina-ristorante.json';
import capitol_lunch from './capitol-lunch.json';
import capitol_pizza from './capitol-pizza.json';
import captain_scotts_lobster_dock from './captain-scotts-lobster-dock.json';
import cardillos_deli_plus from './cardillos-deli-plus.json';
import carmelas_pasta_shop from './carmelas-pasta-shop.json';
import carmines_bar_grill_stage from './carmines-bar-grill-stage.json';
import carmines_pizza from './carmines-pizza.json';
import carribean_delices from './carribean-delices.json';
import carvel from './carvel.json';
import casa_mia_at_the_hawthorne from './casa-mia-at-the-hawthorne.json';
import caseys_cavern from './caseys-cavern.json';
import catch_a_healthy_habit_cafe from './catch-a-healthy-habit-cafe.json';
import cedarhurst_cafe from './cedarhurst-cafe.json';
import cedars_steaks_oysters from './cedars-steaks-oysters.json';
import cellarmaker_brewing_co from './cellarmaker-brewing-co.json';
import center_street_brewing_company from './center-street-brewing-company.json';
import center_street from './center-street.json';
import cfcf_coffee from './cfcf-coffee.json';
import chaiwalla_tea_room from './chaiwalla-tea-room.json';
import chameleon_cafe from './chameleon-cafe.json';
import chapter_one from './chapter-one.json';
import charhause_gluten_free_microbakery_cafe from './charhause-gluten-free-microbakery-cafe.json';
import charlies_sandwich_shoppe from './charlies-sandwich-shoppe.json';
import charter_oak_brewing from './charter-oak-brewing.json';
import chaves_bakery_deli from './chaves-bakery-deli.json';
import chaves_bakery_ii from './chaves-bakery-ii.json';
import chaves_restaurant from './chaves-restaurant.json';
import chengs_kitchen_chinese_restaurant from './chengs-kitchen-chinese-restaurant.json';
import cherry_tree_farm from './cherry-tree-farm.json';
import cheshire_pizza_and_ale from './cheshire-pizza-and-ale.json';
import chesters_barbecue from './chesters-barbecue.json';
import chiangmai_thai from './chiangmai-thai.json';
import chicago_sams_sports_bar_grille from './chicago-sams-sports-bar-grille.json';
import chicken_joes from './chicken-joes.json';
import chicken_pizza_point from './chicken-pizza-point.json';
import chickys_grill_and_pizza from './chickys-grill-and-pizza.json';
import chilis from './chilis.json';
import china_garden from './china-garden.json';
import china_one from './china-one.json';
import china_sky from './china-sky.json';
import chinese_gourmet_kitchen from './chinese-gourmet-kitchen.json';
import choupette_creperie from './choupette-creperie.json';
import christys_restaurant from './christys-restaurant.json';
import chubbys from './chubbys.json';
import chucks_steak_house_and_margarita_grill_mexican_cantina from './chucks-steak-house-and-margarita-grill-mexican-cantina.json';
import chucks_steak_house from './chucks-steak-house.json';
import chulos_stuffed_cookies from './chulos-stuffed-cookies.json';
import chutni_biryani_noodle_bar from './chutni-biryani-noodle-bar.json';
import cinco_de_mayo from './cinco-de-mayo.json';
import citrus from './citrus.json';
import cjs_coffee_cafe from './cjs-coffee-cafe.json';
import claim_52_brewing from './claim-52-brewing.json';
import claires_corner_copia from './claires-corner-copia.json';
import clocktown_brewing from './clocktown-brewing.json';
import clover_pharmacy from './clover-pharmacy.json';
import club_glenvale from './club-glenvale.json';
import cobs_bread from './cobs-bread.json';
import coco_thai_kitchen from './coco-thai-kitchen.json';
import coconut_cafe from './coconut-cafe.json';
import coffee_an_donut_shop from './coffee-an-donut-shop.json';
import coffee_cabinet from './coffee-cabinet.json';
import cold_plate_dessert from './cold-plate-dessert.json';
import cold_stone_creamery from './cold-stone-creamery.json';
import cold_town_brewery from './cold-town-brewery.json';
import coldfire_brewing from './coldfire-brewing.json';
import coles_road_brewing from './coles-road-brewing.json';
import colony_grill from './colony-grill.json';
import comix_roadhouse_comedy_club from './comix-roadhouse-comedy-club.json';
import common_grounds from './common-grounds.json';
import concentric_brewing_company from './concentric-brewing-company.json';
import connecticut_valley_brewing_company from './connecticut-valley-brewing-company.json';
import corbos_corner_deli from './corbos-corner-deli.json';
import coromandel_cuisine_of_india from './coromandel-cuisine-of-india.json';
import corwall_country_bakery from './corwall-country-bakery.json';
import cos_cobber from './cos-cobber.json';
import cosmic_cafe_smoothie_bar from './cosmic-cafe-smoothie-bar.json';
import costellos_clam_shack from './costellos-clam-shack.json';
import counterweight_brewing_company from './counterweight-brewing-company.json';
import country_diner from './country-diner.json';
import country_view_restaurant from './country-view-restaurant.json';
import courtyard_restaurant from './courtyard-restaurant.json';
import cove_pizza from './cove-pizza.json';
import crackd_kitchen_coffee from './crackd-kitchen-coffee.json';
import craft_260 from './craft-260.json';
import craft_pizza_company from './craft-pizza-company.json';
import cravings_an_american_eatery from './cravings-an-american-eatery.json';
import cream_coffee_shop from './cream-coffee-shop.json';
import creative_cakes_by_donna from './creative-cakes-by-donna.json';
import cromwell_diner from './cromwell-diner.json';
import cromwell_pizza_pasta from './cromwell-pizza-pasta.json';
import crown_and_hammer from './crown-and-hammer.json';
import crown_bar from './crown-bar.json';
import crown_super_market from './crown-super-market.json';
import crumbles_bakery from './crumbles-bakery.json';
import cuginos_pizzeria from './cuginos-pizzeria.json';
import curleys_diner from './curleys-diner.json';
import curry_hurry from './curry-hurry.json';
import cw_chops_catch from './cw-chops-catch.json';
import da_capo from './da-capo.json';
import dads_restaurant from './dads-restaurant.json';
import dal_mare_pizzeria from './dal-mare-pizzeria.json';
import danos_pub from './danos-pub.json';
import daybreak from './daybreak.json';
import deborah_anns_sweet_shoppe from './deborah-anns-sweet-shoppe.json';
import debs_place_restaurant from './debs-place-restaurant.json';
import dees_one_smart_cookie from './dees-one-smart-cookie.json';
import deja_brew from './deja-brew.json';
import demo from './demo.json';
import democracy_brewing from './democracy-brewing.json';
import den_sake_brewery from './den-sake-brewery.json';
import denovellis from './denovellis.json';
import devils_advocate from './devils-advocate.json';
import dew_drop_inn from './dew-drop-inn.json';
import dhaba_wala_indian_kitchen from './dhaba-wala-indian-kitchen.json';
import dianes_restaurant from './dianes-restaurant.json';
import diesel_lounge from './diesel-lounge.json';
import difrancos from './difrancos.json';
import dimare_pastry_shop from './dimare-pastry-shop.json';
import dimitris_diner from './dimitris-diner.json';
import dippin_dots from './dippin-dots.json';
import dirt_20 from './dirt-20.json';
import dish_n_dat from './dish-n-dat.json';
import dishoom from './dishoom.json';
import dive_bar_restaurant from './dive-bar-restaurant.json';
import dockside_brewery from './dockside-brewery.json';
import dog_lane_cafe from './dog-lane-cafe.json';
import dolce_italian_cafe from './dolce-italian-cafe.json';
import doms_broad_st_eatery from './doms-broad-st-eatery.json';
import doms_coffee from './doms-coffee.json';
import donde_julio_restaurant from './donde-julio-restaurant.json';
import dono_cafe from './dono-cafe.json';
import donovans_reef from './donovans-reef.json';
import donut_delight from './donut-delight.json';
import donut_inc from './donut-inc.json';
import dora_marketplace from './dora-marketplace.json';
import dorchester_brewing_company from './dorchester-brewing-company.json';
import doro_marketplace from './doro-marketplace.json';
import downeast_cider from './downeast-cider.json';
import dp_dough from './dp-dough.json';
import dragon_168 from './dragon-168.json';
import dragos_kitchen from './dragos-kitchen.json';
import drawdown_brewing from './drawdown-brewing.json';
import duchess from './duchess.json';
import e_e_enterprises from './e-e-enterprises.json';
import early_bird_cafe from './early-bird-cafe.json';
import east_london_brewery from './east-london-brewery.json';
import east_rock_bakery from './east-rock-bakery.json';
import east_side_restaurant from './east-side-restaurant.json';
import eastend from './eastend.json';
import echo_craft_kitchen from './echo-craft-kitchen.json';
import eco_chic_salon_spa_blow_dry_bar from './eco-chic-salon-spa-blow-dry-bar.json';
import eco_coffee_house from './eco-coffee-house.json';
import eddies_pizza_pasta from './eddies-pizza-pasta.json';
import edis_pizzaria_restaurant from './edis-pizzaria-restaurant.json';
import edo_ichi_sushi_and_hibachi_steakhouse from './edo-ichi-sushi-and-hibachi-steakhouse.json';
import ekkalak_thai_cuisine from './ekkalak-thai-cuisine.json';
import el_agave_bar from './el-agave-bar.json';
import el_coyote from './el-coyote.json';
import el_pollo_guapo from './el-pollo-guapo.json';
import el_rio_lempa from './el-rio-lempa.json';
import el_tenampa from './el-tenampa.json';
import el_traviezo_mexican_food from './el-traviezo-mexican-food.json';
import el_traviezo_mexican_restaurant from './el-traviezo-mexican-restaurant.json';
import elbow_room from './elbow-room.json';
import elephant_trail from './elephant-trail.json';
import elicit_brewing_co from './elicit-brewing-co.json';
import elis_on_whitney from './elis-on-whitney.json';
import elis_tavern from './elis-tavern.json';
import elm_city_social from './elm-city-social.json';
import elm_st_diner from './elm-st-diner.json';
import elmos_dockside from './elmos-dockside.json';
import elsol from './elsol.json';
import eltana from './eltana.json';
import enl_nail_bar from './enl-nail-bar.json';
import entenmanns_oroweat_bimbo_bakeries from './entenmanns-oroweat-bimbo-bakeries.json';
import enzos_pizza_deli from './enzos-pizza-deli.json';
import epicure_brewing from './epicure-brewing.json';
import erniess_delicatessen from './erniess-delicatessen.json';
import esca_restaurant_wine_bar from './esca-restaurant-wine-bar.json';
import euro_lounge from './euro-lounge.json';
import euro_plate from './euro-plate.json';
import evelyns_coffee_banh_mi from './evelyns-coffee-banh-mi.json';
import everlasting_nutrition from './everlasting-nutrition.json';
import express_ranch_pizzeria from './express-ranch-pizzeria.json';
import faction_brewing from './faction-brewing.json';
import fairfield_pizza from './fairfield-pizza.json';
import fairways_bar_and_grill from './fairways-bar-and-grill.json';
import falling_sky_brewing_house from './falling-sky-brewing-house.json';
import family_pizza from './family-pizza.json';
import fancy_pants_cafe from './fancy-pants-cafe.json';
import far_eastern_buffet from './far-eastern-buffet.json';
import farmer_horse_coffee from './farmer-horse-coffee.json';
import farmington_gardens from './farmington-gardens.json';
import farmington_miniature_golf_ice_cream_parlor from './farmington-miniature-golf-ice-cream-parlor.json';
import farrells from './farrells.json';
import fat_cat_grill_and_bar from './fat-cat-grill-and-bar.json';
import federal_cafe from './federal-cafe.json';
import feng from './feng.json';
import fenton_river_grill from './fenton-river-grill.json';
import ferris_acres_creamery from './ferris-acres-creamery.json';
import fifen_drum_restaurant_and_inn from './fifen-drum-restaurant-and-inn.json';
import figaro from './figaro.json';
import figs from './figs.json';
import fika from './fika.json';
import filberts_old_time_root_beer from './filberts-old-time-root-beer.json';
import filhos_cucina from './filhos-cucina.json';
import fino_wood_fired_pizza_bar from './fino-wood-fired-pizza-bar.json';
import fire_at_the_ridge from './fire-at-the-ridge.json';
import fire_engine_pizza_co from './fire-engine-pizza-co.json';
import fire_n_spice_vegan_restaurant_sports_cafe from './fire-n-spice-vegan-restaurant-sports-cafe.json';
import firebrands_bar_grill from './firebrands-bar-grill.json';
import fish from './fish.json';
import five_horses_tavern___south_end from './five-horses-tavern---south-end.json';
import flanders_fish_market_and_restaurant from './flanders-fish-market-and-restaurant.json';
import fletchs_bagels from './fletchs-bagels.json';
import flinders_lane from './flinders-lane.json';
import flipside from './flipside.json';
import flobees_roadside_stand from './flobees-roadside-stand.json';
import flour_bakery_cafe from './flour-bakery-cafe.json';
import flour_water_salt_bread from './flour-water-salt-bread.json';
import flying_monkey_grill_bar from './flying-monkey-grill-bar.json';
import flying_saucer_cypress_waters from './flying-saucer-cypress-waters.json';
import focaccias_cafe_catery from './focaccias-cafe-catery.json';
import fonda from './fonda.json';
import forbidden_llama from './forbidden-llama.json';
import ford_news_diner from './ford-news-diner.json';
import fords_lobster from './fords-lobster.json';
import forge_vine from './forge-vine.json';
import fornarellis_ristorante_bar from './fornarellis-ristorante-bar.json';
import fortina from './fortina.json';
import fortune_fuel from './fortune-fuel.json';
import fortune_garden_chinese_restaurant from './fortune-garden-chinese-restaurant.json';
import four_mile_inn from './four-mile-inn.json';
import four_provinces_brewing_company from './four-provinces-brewing-company.json';
import francescas_wine_bar_bistro from './francescas-wine-bar-bistro.json';
import franco_giannis from './franco-giannis.json';
import franco_manca from './franco-manca.json';
import frank_pepe_pizzeria_napoletana from './frank-pepe-pizzeria-napoletana.json';
import frank_pepes_west_hartford from './frank-pepes-west-hartford.json';
import frankie_jos from './frankie-jos.json';
import frankies_diner from './frankies-diner.json';
import frankies_pizza from './frankies-pizza.json';
import frankies_sport_lounge from './frankies-sport-lounge.json';
import franklin_town_line_diner from './franklin-town-line-diner.json';
import franks_willow_inn_bar_and_grill from './franks-willow-inn-bar-and-grill.json';
import fremont_brewing_company from './fremont-brewing-company.json';
import frenchies_coffee_bar_at_silver_sands_state_park from './frenchies-coffee-bar-at-silver-sands-state-park.json';
import fresca_tequila_bar_grill from './fresca-tequila-bar-grill.json';
import friendlys from './friendlys.json';
import front_street_bistro from './front-street-bistro.json';
import frost_bite from './frost-bite.json';
import frutiere_de_bevaix from './frutiere-de-bevaix.json';
import fuego_mexican_grill from './fuego-mexican-grill.json';
import fuji from './fuji.json';
import g_cafe from './g-cafe.json';
import galaxy_diner from './galaxy-diner.json';
import gallo from './gallo.json';
import gallos_restaurant from './gallos-restaurant.json';
import gansett_wraps from './gansett-wraps.json';
import garibaldi from './garibaldi.json';
import garos_place from './garos-place.json';
import gaucho from './gaucho.json';
import gayles_depot_square_farm_shoppe from './gayles-depot-square-farm-shoppe.json';
import gennaros_apizza_cafe from './gennaros-apizza-cafe.json';
import georges_pizza_restaurant from './georges-pizza-restaurant.json';
import georgetown_cupcake from './georgetown-cupcake.json';
import georgias_restaurant_pizza from './georgias-restaurant-pizza.json';
import georginas_restaurant from './georginas-restaurant.json';
import geronimo from './geronimo.json';
import ggs_wood_fired_pizza from './ggs-wood-fired-pizza.json';
import gibbet_hill_grill from './gibbet-hill-grill.json';
import giffords from './giffords.json';
import gigantic_brewing_company from './gigantic-brewing-company.json';
import gilson_cafe_cinema from './gilson-cafe-cinema.json';
import ginger_root from './ginger-root.json';
import ginos_pizza from './ginos-pizza.json';
import ginza_express from './ginza-express.json';
import giovannis_brick_oven_pizzaria from './giovannis-brick-oven-pizzaria.json';
import giv_coffee from './giv-coffee.json';
import glastonbury_coffee_shop from './glastonbury-coffee-shop.json';
import glastonbury_pizza_house from './glastonbury-pizza-house.json';
import gluten_free_oasis from './gluten-free-oasis.json';
import godfathers_pizza from './godfathers-pizza.json';
import golden_greek from './golden-greek.json';
import golden_lamb_buttery from './golden-lamb-buttery.json';
import golden_mango from './golden-mango.json';
import golden_palace from './golden-palace.json';
import goldenrod_pastries from './goldenrod-pastries.json';
import goldies_eatery from './goldies-eatery.json';
import golf_lounge_18 from './golf-lounge-18.json';
import gong_cha from './gong-cha.json';
import goofy_foot_tap_room from './goofy-foot-tap-room.json';
import gran_cafe_coffee_lounge from './gran-cafe-coffee-lounge.json';
import grand_cru from './grand-cru.json';
import grand_pizza from './grand-pizza.json';
import grandpas_diner from './grandpas-diner.json';
import grano_arso from './grano-arso.json';
import granola_bar from './granola-bar.json';
import grapevine from './grapevine.json';
import gratitude_cafe_bakery from './gratitude-cafe-bakery.json';
import gravity_well_brewing from './gravity-well-brewing.json';
import grays_inn from './grays-inn.json';
import great_harvest_bread_co from './great-harvest-bread-co.json';
import great_northern_hotel from './great-northern-hotel.json';
import great_tso from './great-tso.json';
import great_wall_chinese_food from './great-wall-chinese-food.json';
import green_door_restaurant from './green-door-restaurant.json';
import green_kettle_coffee from './green-kettle-coffee.json';
import green_olive_diner from './green-olive-diner.json';
import green_onion_pizza_restaurant from './green-onion-pizza-restaurant.json';
import green_tonic from './green-tonic.json';
import greggs from './greggs.json';
import grey_sail_taproom from './grey-sail-taproom.json';
import grigg_street from './grigg-street.json';
import grill_four_forty_five from './grill-four-forty-five.json';
import grindstone_tavern from './grindstone-tavern.json';
import griswold_coastal_forest_trail from './griswold-coastal-forest-trail.json';
import grk_grill from './grk-grill.json';
import groton_publick_house_pub_eatery from './groton-publick-house-pub-eatery.json';
import grounded_coffee_co from './grounded-coffee-co.json';
import gs_burgers from './gs-burgers.json';
import guildford_arms from './guildford-arms.json';
import gulianos_bakery from './gulianos-bakery.json';
import haagen_dazs from './haagen-dazs.json';
import hachiroku from './hachiroku.json';
import hacienda_villa from './hacienda-villa.json';
import hadlyme_country_market from './hadlyme-country-market.json';
import hammonesset_connector from './hammonesset-connector.json';
import hana_tokyo from './hana-tokyo.json';
import hanami_japanese_sushi_hibachi_steak_house from './hanami-japanese-sushi-hibachi-steak-house.json';
import hangry_goose from './hangry-goose.json';
import hanks_dairy_bar from './hanks-dairy-bar.json';
import hanks_restaurant from './hanks-restaurant.json';
import hanover_tap from './hanover-tap.json';
import happy_monkey from './happy-monkey.json';
import harborview_market from './harborview-market.json';
import hard_rock_cafe from './hard-rock-cafe.json';
import harlan_social from './harlan-social.json';
import harp_dragon_pub from './harp-dragon-pub.json';
import harpoon_brewery from './harpoon-brewery.json';
import harpoon_tap_room from './harpoon-tap-room.json';
import harpos_bakery_and_cafe from './harpos-bakery-and-cafe.json';
import harriet_beecher_stowe_center_little_free_library_charter_175887 from './harriet-beecher-stowe-center-little-free-library-charter-175887.json';
import harrys_pizzeria_tavern from './harrys-pizzeria-tavern.json';
import hartford_baking_co from './hartford-baking-co.json';
import hartford_road_cafe from './hartford-road-cafe.json';
import harvest_cafe_and_bakery from './harvest-cafe-and-bakery.json';
import harvest from './harvest.json';
import harwinton_pizza_restaurant from './harwinton-pizza-restaurant.json';
import hash_house_a_go_go from './hash-house-a-go-go.json';
import hata from './hata.json';
import havenly from './havenly.json';
import hawksmoor from './hawksmoor.json';
import heibecks_stand from './heibecks-stand.json';
import heights_pizza from './heights-pizza.json';
import heirloom_food_company from './heirloom-food-company.json';
import heirloom_market_at_comstock_ferre from './heirloom-market-at-comstock-ferre.json';
import hf_crave from './hf-crave.json';
import high_nine_brewing from './high-nine-brewing.json';
import hilltop_cafe from './hilltop-cafe.json';
import hinoki from './hinoki.json';
import hm_pizza_giant_grinders from './hm-pizza-giant-grinders.json';
import hog_river_brewing_company from './hog-river-brewing-company.json';
import holy_cow from './holy-cow.json';
import home_on_the_range from './home-on-the-range.json';
import honey_joes_family_coffeehouse from './honey-joes-family-coffeehouse.json';
import honeyspot_pizza from './honeyspot-pizza.json';
import honeysuckle_gelato from './honeysuckle-gelato.json';
import hong_kong_tokyo from './hong-kong-tokyo.json';
import hooters from './hooters.json';
import hop_culture_farms_brew_company from './hop-culture-farms-brew-company.json';
import hop_haus_berlin from './hop-haus-berlin.json';
import hop_valley_brewing from './hop-valley-brewing.json';
import hopfly_brewing_co from './hopfly-brewing-co.json';
import hops_on_the_hill from './hops-on-the-hill.json';
import hopsters_brewing_co from './hopsters-brewing-co.json';
import hornets_nest_deli from './hornets-nest-deli.json';
import hot_table from './hot-table.json';
import hotshots_sports_bar_and_grill from './hotshots-sports-bar-and-grill.json';
import howells from './howells.json';
import hteao from './hteao.json';
import hubbard_park_pizza from './hubbard-park-pizza.json';
import hudson_social from './hudson-social.json';
import hunan_noodle_bar from './hunan-noodle-bar.json';
import hungry_pot_korean_bbq_hot_pot from './hungry-pot-korean-bbq-hot-pot.json';
import hungry_pot from './hungry-pot.json';
import hushys_sandwiches from './hushys-sandwiches.json';
import huxleys_bookmark_cafe_family_restaurant from './huxleys-bookmark-cafe-family-restaurant.json';
import ichiban_express_hibachi_grill_and_poke_bowl from './ichiban-express-hibachi-grill-and-poke-bowl.json';
import ihop from './ihop.json';
import illianos_pizzeria_and_restaurant from './illianos-pizzeria-and-restaurant.json';
import illianos_ristorante_pizzeria from './illianos-ristorante-pizzeria.json';
import inferno_apizza from './inferno-apizza.json';
import irenes_family_restaurant from './irenes-family-restaurant.json';
import irmak_restaurant from './irmak-restaurant.json';
import iron_horse_pizzeria_and_sports_pub from './iron-horse-pizzeria-and-sports-pub.json';
import iron_horse_sports_pub from './iron-horse-sports-pub.json';
import iso_iso_ramen_boba from './iso-iso-ramen-boba.json';
import istanbul_gyro_kebab from './istanbul-gyro-kebab.json';
import italian_gourmet_deli_and_bakery from './italian-gourmet-deli-and-bakery.json';
import ivy_cafe from './ivy-cafe.json';
import izumi from './izumi.json';
import j_roos_restaurant from './j-roos-restaurant.json';
import j_timothys_taverne from './j-timothys-taverne.json';
import jack_smyth_brewing_company from './jack-smyth-brewing-company.json';
import jacks_brick_oven_pizza from './jacks-brick-oven-pizza.json';
import jade_lee_kitchen from './jade-lee-kitchen.json';
import jaho_coffee_roaster_wine_bar from './jaho-coffee-roaster-wine-bar.json';
import jazzys_cabaret from './jazzys-cabaret.json';
import jds_pizzeria_bar_grill from './jds-pizzeria-bar-grill.json';
import jesses_ice_cream from './jesses-ice-cream.json';
import jessicas_lobster_shack from './jessicas-lobster-shack.json';
import jessies_cafe from './jessies-cafe.json';
import jesters_coffee from './jesters-coffee.json';
import jewett_city_pizza_palace from './jewett-city-pizza-palace.json';
import jg_ross from './jg-ross.json';
import jia_mei_asian_kitchen from './jia-mei-asian-kitchen.json';
import jitters from './jitters.json';
import jjstacks from './jjstacks.json';
import joe_pizza from './joe-pizza.json';
import joeys_pizza_pie from './joeys-pizza-pie.json';
import john_marias_pizzeria_restaurant from './john-marias-pizzeria-restaurant.json';
import john_the_baker from './john-the-baker.json';
import johnny_rockets from './johnny-rockets.json';
import johnnys_clam_shack from './johnnys-clam-shack.json';
import johnnys_diner from './johnnys-diner.json';
import johns_cafe from './johns-cafe.json';
import jojos_breakfast_mexican_food from './jojos-breakfast-mexican-food.json';
import josephs_bagel_grill from './josephs-bagel-grill.json';
import josies_corner from './josies-corner.json';
import joy_bar_coffee_co from './joy-bar-coffee-co.json';
import jregos_gathering_place from './jregos-gathering-place.json';
import jubi_tea_sweets from './jubi-tea-sweets.json';
import jukebox_studios from './jukebox-studios.json';
import julianos_pizza_restaurant from './julianos-pizza-restaurant.json';
import juniors_coffee_shop from './juniors-coffee-shop.json';
import juno_bakery from './juno-bakery.json';
import kaliabon_ramen from './kaliabon-ramen.json';
import kaliubon_ramen from './kaliubon-ramen.json';
import karma from './karma.json';
import kat_su_restaurant from './kat-su-restaurant.json';
import katsute100 from './katsute100.json';
import keller from './keller.json';
import kennedy_fried_chicken from './kennedy-fried-chicken.json';
import kens_corner_breakfast_and_lunch from './kens-corner-breakfast-and-lunch.json';
import kent_falls_brewing_company from './kent-falls-brewing-company.json';
import kent_pizza_garden from './kent-pizza-garden.json';
import kenyon_st_little_free_library_charter_27991 from './kenyon-st-little-free-library-charter-27991.json';
import kg_corner_deli from './kg-corner-deli.json';
import kickback_lunch_lounge from './kickback-lunch-lounge.json';
import kimberlys_diner from './kimberlys-diner.json';
import king_donut from './king-donut.json';
import kings_court from './kings-court.json';
import kingston_kafe from './kingston-kafe.json';
import kira_sushi from './kira-sushi.json';
import kiwi_spoon from './kiwi-spoon.json';
import klekolo_world_coffee from './klekolo-world-coffee.json';
import knead_doughnuts from './knead-doughnuts.json';
import kokomos_restaurant_beach_bar from './kokomos-restaurant-beach-bar.json';
import koma_japanese_steakhouse from './koma-japanese-steakhouse.json';
import koon_thai from './koon-thai.json';
import kosciuszko_club from './kosciuszko-club.json';
import koto_japanese_steak_house_and_sushi_bar from './koto-japanese-steak-house-and-sushi-bar.json';
import kotobuki from './kotobuki.json';
import kumo from './kumo.json';
import kung_fu_tea from './kung-fu-tea.json';
import kuyi_sushi from './kuyi-sushi.json';
import la_cabana from './la-cabana.json';
import la_carnita_of_bantam from './la-carnita-of-bantam.json';
import la_casa_mania_mexican_restaurant_cantina from './la-casa-mania-mexican-restaurant-cantina.json';
import la_figata from './la-figata.json';
import la_flor_de_mexico from './la-flor-de-mexico.json';
import la_foresta from './la-foresta.json';
import la_fortuna_bar_restaurant from './la-fortuna-bar-restaurant.json';
import la_kabanita from './la-kabanita.json';
import la_mexicana from './la-mexicana.json';
import la_molienda from './la-molienda.json';
import la_piastra from './la-piastra.json';
import la_piazza from './la-piazza.json';
import la_poblanita from './la-poblanita.json';
import la_quartier_bakery_cafe from './la-quartier-bakery-cafe.json';
import la_taqueria from './la-taqueria.json';
import la_vita_gustosa from './la-vita-gustosa.json';
import lady_libertine from './lady-libertine.json';
import lagan from './lagan.json';
import lalibela from './lalibela.json';
import lamplighter_brewing_company from './lamplighter-brewing-company.json';
import las_vetas_lounge from './las-vetas-lounge.json';
import lasalle_market_deli from './lasalle-market-deli.json';
import lasowiak_polish_deli_baker_grocery from './lasowiak-polish-deli-baker-grocery.json';
import lasting_brass_brewing from './lasting-brass-brewing.json';
import laylas_falafel_greenleaf_cafe from './laylas-falafel-greenleaf-cafe.json';
import laylas_falafel from './laylas-falafel.json';
import lc_chens from './lc-chens.json';
import le_fat_poodle from './le-fat-poodle.json';
import le_gamin from './le-gamin.json';
import le_pain_quotidien from './le-pain-quotidien.json';
import le_petit_beefbar_edinburgh from './le-petit-beefbar-edinburgh.json';
import legends_pizzeria from './legends-pizzeria.json';
import lenas_pizzaria from './lenas-pizzaria.json';
import lennys_steakhouse from './lennys-steakhouse.json';
import les_dairy_bar from './les-dairy-bar.json';
import lescargot_bleu_wine_bar from './lescargot-bleu-wine-bar.json';
import lescargot_bleu from './lescargot-bleu.json';
import lianas_trottoria from './lianas-trottoria.json';
import lilac_house from './lilac-house.json';
import lime_rock_cafe from './lime-rock-cafe.json';
import linden_lounge from './linden-lounge.json';
import little_aladdin_mediterranean_grill from './little-aladdin-mediterranean-grill.json';
import little_barn from './little-barn.json';
import little_free_library from './little-free-library.json';
import little_house_brewing_co from './little-house-brewing-co.json';
import little_india from './little-india.json';
import little_mazen_pizza from './little-mazen-pizza.json';
import little_pub_greenwich from './little-pub-greenwich.json';
import livs_oyster_bar from './livs-oyster-bar.json';
import lobster_landing from './lobster-landing.json';
import local_63 from './local-63.json';
import locali from './locali.json';
import lorca from './lorca.json';
import los_sarapes_mexican_restaurant from './los-sarapes-mexican-restaurant.json';
import louies from './louies.json';
import loyal_order_of_moose from './loyal-order-of-moose.json';
import luanns_bakery_cafe from './luanns-bakery-cafe.json';
import lubranos_pizza from './lubranos-pizza.json';
import lucas_pizzeria from './lucas-pizzeria.json';
import luce from './luce.json';
import lucky_dragon_chinese_restaurant from './lucky-dragon-chinese-restaurant.json';
import lucky_lous from './lucky-lous.json';
import lucky_star from './lucky-star.json';
import lucky_yu from './lucky-yu.json';
import lugano_salumeria_wine_bar from './lugano-salumeria-wine-bar.json';
import luigis_bakery from './luigis-bakery.json';
import luigis_restaurant from './luigis-restaurant.json';
import lumleys_bakery from './lumleys-bakery.json';
import lupi_legna from './lupi-legna.json';
import lyme_grill from './lyme-grill.json';
import m_white_lounge from './m-white-lounge.json';
import mac_n_out from './mac-n-out.json';
import macros_healthy_restaurant from './macros-healthy-restaurant.json';
import made_by_pam from './made-by-pam.json';
import mademoiselle_macaron from './mademoiselle-macaron.json';
import madhouse_cafe from './madhouse-cafe.json';
import maggie_mcflys_brookfield from './maggie-mcflys-brookfield.json';
import maggie_mcflys from './maggie-mcflys.json';
import main_street_diner from './main-street-diner.json';
import main_street_restaurant_and_bar from './main-street-restaurant-and-bar.json';
import main_wah from './main-wah.json';
import maine_fish_market_restaurant from './maine-fish-market-restaurant.json';
import mainport_fish_chips from './mainport-fish-chips.json';
import maiones from './maiones.json';
import maison_mathis from './maison-mathis.json';
import maks_deli from './maks-deli.json';
import mama_carmelas_deli from './mama-carmelas-deli.json';
import mama_chow from './mama-chow.json';
import mamas_house_family_diner from './mamas-house-family-diner.json';
import mamas_kitchen from './mamas-kitchen.json';
import mamouns from './mamouns.json';
import manana_cafe from './manana-cafe.json';
import mangetsu_farmington from './mangetsu-farmington.json';
import mangos_wood_fired_pizza_co from './mangos-wood-fired-pizza-co.json';
import many_bennies from './many-bennies.json';
import marco_polo from './marco-polo.json';
import marcos_pizzeria_and_restaurant from './marcos-pizzeria-and-restaurant.json';
import marcos from './marcos.json';
import marina_indian_restaurant from './marina-indian-restaurant.json';
import marker_37 from './marker-37.json';
import market_place_tavern from './market-place-tavern.json';
import marlborough_pizza_restaurant from './marlborough-pizza-restaurant.json';
import marlborough_tavern from './marlborough-tavern.json';
import marmalade from './marmalade.json';
import martel_bistro_bar from './martel-bistro-bar.json';
import martys_cafe from './martys-cafe.json';
import marys_diner from './marys-diner.json';
import massimo_pizzeria_ristorante from './massimo-pizzeria-ristorante.json';
import max_a_mia from './max-a-mia.json';
import max_bibos from './max-bibos.json';
import max_burger from './max-burger.json';
import max_downtown from './max-downtown.json';
import max_fish from './max-fish.json';
import max_oyster_bar from './max-oyster-bar.json';
import mcalisters_deli from './mcalisters-deli.json';
import mcconneys_farm from './mcconneys-farm.json';
import meadow from './meadow.json';
import mecha_noodle_bar from './mecha-noodle-bar.json';
import mediterranea from './mediterranea.json';
import mediterraneo from './mediterraneo.json';
import melilli_pizzeria from './melilli-pizzeria.json';
import menchies from './menchies.json';
import menya_gumi from './menya-gumi.json';
import meraki from './meraki.json';
import mercurys_coffee_co from './mercurys-coffee-co.json';
import metro_bis from './metro-bis.json';
import mew_haven_cat_cafe from './mew-haven-cat-cafe.json';
import mex_on_main from './mex-on-main.json';
import mexicali_cantina_grill from './mexicali-cantina-grill.json';
import mexicue from './mexicue.json';
import mexipho from './mexipho.json';
import mezcal from './mezcal.json';
import mezzo_grille from './mezzo-grille.json';
import mi_pueblo from './mi-pueblo.json';
import mi_rancho_deli from './mi-rancho-deli.json';
import michael_jordans_steakhouse from './michael-jordans-steakhouse.json';
import michelinas_apizza_ristorante from './michelinas-apizza-ristorante.json';
import micro_creamery from './micro-creamery.json';
import middletown_halal_grill_and_pizza from './middletown-halal-grill-and-pizza.json';
import midway_pizza from './midway-pizza.json';
import midwest_coast_brewing from './midwest-coast-brewing.json';
import mighty_squirrel_taproom_kitchen from './mighty-squirrel-taproom-kitchen.json';
import mikes_pizza_diner from './mikes-pizza-diner.json';
import mikes_pizza_restaurant from './mikes-pizza-restaurant.json';
import miku from './miku.json';
import milestone from './milestone.json';
import milford_pizza_and_mexican_food from './milford-pizza-and-mexican-food.json';
import mill_plain_diner from './mill-plain-diner.json';
import millwrights from './millwrights.json';
import mimmo_play_cafe from './mimmo-play-cafe.json';
import ming_moon_restaurant from './ming-moon-restaurant.json';
import ming_moon from './ming-moon.json';
import mini_melts_usa from './mini-melts-usa.json';
import misfit_club from './misfit-club.json';
import misimi from './misimi.json';
import mitchells_on_main from './mitchells-on-main.json';
import mizzas_pizza from './mizzas-pizza.json';
import mj23_sports_bar_grill from './mj23-sports-bar-grill.json';
import mm_bistro from './mm-bistro.json';
import modern_apizza from './modern-apizza.json';
import modern_pastry_shop from './modern-pastry-shop.json';
import mofongo from './mofongo.json';
import moli from './moli.json';
import molly_moons_homemade_ice_cream from './molly-moons-homemade-ice-cream.json';
import mommys_delicious_food from './mommys-delicious-food.json';
import momosan from './momosan.json';
import mondo from './mondo.json';
import monkey_farm from './monkey-farm.json';
import monroe_diner from './monroe-diner.json';
import montys_river_grille from './montys-river-grille.json';
import moody_tongue_brewing_company from './moody-tongue-brewing-company.json';
import moon_rocks from './moon-rocks.json';
import mor_bakehouse from './mor-bakehouse.json';
import mountainside_cafe from './mountainside-cafe.json';
import mouse_proof_brewery from './mouse-proof-brewery.json';
import mr_falafel from './mr-falafel.json';
import mr_mac_canteen from './mr-mac-canteen.json';
import ms_cafe from './ms-cafe.json';
import muggswigz_coffee_tea from './muggswigz-coffee-tea.json';
import muscle_maker_grill from './muscle-maker-grill.json';
import muyri_indian_bistro from './muyri-indian-bistro.json';
import my_place_pizza_restaurant from './my-place-pizza-restaurant.json';
import mykonos_mediterranean_restaurant from './mykonos-mediterranean-restaurant.json';
import mystic_depot_roasters from './mystic-depot-roasters.json';
import mystic_pizza from './mystic-pizza.json';
import naija_restaurant from './naija-restaurant.json';
import nail_creek_pub_brewery from './nail-creek-pub-brewery.json';
import nanas_mystic from './nanas-mystic.json';
import nanny_obriens_irish_pub from './nanny-obriens-irish-pub.json';
import naples_pizza from './naples-pizza.json';
import nardellis from './nardellis.json';
import neat from './neat.json';
import nectar from './nectar.json';
import new_china_wok from './new-china-wok.json';
import new_england_brewing_company from './new-england-brewing-company.json';
import new_harvest from './new-harvest.json';
import new_haven_taste_of_china from './new-haven-taste-of-china.json';
import new_mill_restaurant from './new-mill-restaurant.json';
import new_star___milford_chinese from './new-star---milford-chinese.json';
import new_york_pizza_company from './new-york-pizza-company.json';
import newbury_gelato from './newbury-gelato.json';
import newfield_restaurant_pizza from './newfield-restaurant-pizza.json';
import newsroom from './newsroom.json';
import newtown_road_italian_deli_pizza from './newtown-road-italian-deli-pizza.json';
import nicks_char_pit from './nicks-char-pit.json';
import nicks_diner from './nicks-diner.json';
import nicks_pizza_restaurant from './nicks-pizza-restaurant.json';
import nightcap from './nightcap.json';
import nikkis_dog_house from './nikkis-dog-house.json';
import ninety_nine_restaurant_pub from './ninety-nine-restaurant-pub.json';
import ninety_nine_restaurant from './ninety-nine-restaurant.json';
import ninkasi_tasting_room from './ninkasi-tasting-room.json';
import ninos_pizzeria from './ninos-pizzeria.json';
import no_coast_taqueria from './no-coast-taqueria.json';
import no_mames_mexican_cantina from './no-mames-mexican-cantina.json';
import no1_restaurant_best_chinese_food_take_out from './no1-restaurant-best-chinese-food-take-out.json';
import noisette_pastry_kitchen from './noisette-pastry-kitchen.json';
import noodles_company from './noodles-company.json';
import norbrook_farm_brewery from './norbrook-farm-brewery.json';
import nori from './nori.json';
import nostrano_italian_eatery from './nostrano-italian-eatery.json';
import nouveau_monde___wine_bar_and_restaurant from './nouveau-monde---wine-bar-and-restaurant.json';
import nutty_squirrel_gelato from './nutty-squirrel-gelato.json';
import nv_bakery_and_market from './nv-bakery-and-market.json';
import nyam_bakery from './nyam-bakery.json';
import oak_and_iron_brewing_co from './oak-and-iron-brewing-co.json';
import oakland_pizza_co from './oakland-pizza-co.json';
import oakland_united_beerworks from './oakland-united-beerworks.json';
import oakleaf_cakes from './oakleaf-cakes.json';
import oakshire_brewing from './oakshire-brewing.json';
import oaxaca_taqueria_and_pizza from './oaxaca-taqueria-and-pizza.json';
import oc_2_go from './oc-2-go.json';
import ocha_thai_japanese_cuisine from './ocha-thai-japanese-cuisine.json';
import ocho_cafe from './ocho-cafe.json';
import odeens_bbq_ridgefield_golf_course from './odeens-bbq-ridgefield-golf-course.json';
import oec_brewing from './oec-brewing.json';
import off_the_hook_bar_grill from './off-the-hook-bar-grill.json';
import oishi from './oishi.json';
import ok_diner from './ok-diner.json';
import oko from './oko.json';
import old_greenwich_deli from './old-greenwich-deli.json';
import old_salt_fish_and_bagels from './old-salt-fish-and-bagels.json';
import old_town_cafe from './old-town-cafe.json';
import old_towne_resturant from './old-towne-resturant.json';
import old_tymes_restaurant from './old-tymes-restaurant.json';
import old_well_tavern from './old-well-tavern.json';
import olde_mother_brewing from './olde-mother-brewing.json';
import olees_pizza_farmington from './olees-pizza-farmington.json';
import olio_restaurant_bar from './olio-restaurant-bar.json';
import olive_garden from './olive-garden.json';
import olives_and_oil from './olives-and-oil.json';
import ollies_pizza from './ollies-pizza.json';
import olympic_family_restaurant from './olympic-family-restaurant.json';
import olympic_pizza_restaurant from './olympic-pizza-restaurant.json';
import one_way_brewing from './one-way-brewing.json';
import one_world_roasters from './one-world-roasters.json';
import ordinary from './ordinary.json';
import oregon_brewlab from './oregon-brewlab.json';
import orems_diner from './orems-diner.json';
import oronoque_farms_gift_bakery from './oronoque-farms-gift-bakery.json';
import osa_restaurant_bar from './osa-restaurant-bar.json';
import otto from './otto.json';
import outback_steakhouse from './outback-steakhouse.json';
import owl_shop from './owl-shop.json';
import oxford_baking_company from './oxford-baking-company.json';
import oyster_club from './oyster-club.json';
import paci_restaurant from './paci-restaurant.json';
import pad_thai from './pad-thai.json';
import palma from './palma.json';
import panaros_pizzeria from './panaros-pizzeria.json';
import papa_ginos from './papa-ginos.json';
import papa_joes_ristorante_italiano from './papa-joes-ristorante-italiano.json';
import papa_zs_restaurant_and_pizza from './papa-zs-restaurant-and-pizza.json';
import papas_pizza_and_roast_beef from './papas-pizza-and-roast-beef.json';
import papii from './papii.json';
import par_four_restaurant from './par-four-restaurant.json';
import paramount_pizza from './paramount-pizza.json';
import parker_eatery_bar from './parker-eatery-bar.json';
import party_pizza from './party-pizza.json';
import pasta_bowls from './pasta-bowls.json';
import pasta_heaven from './pasta-heaven.json';
import paul_gregories_bistro from './paul-gregories-bistro.json';
import pauls_pizza from './pauls-pizza.json';
import pavement_coffeehouse from './pavement-coffeehouse.json';
import peachwave from './peachwave.json';
import peacock_chinese_restaurant from './peacock-chinese-restaurant.json';
import pearl_provisions_tipples from './pearl-provisions-tipples.json';
import peking_tokyo from './peking-tokyo.json';
import penny_black from './penny-black.json';
import pepper_pot from './pepper-pot.json';
import peppermill from './peppermill.json';
import peppers_jamaican_bakery from './peppers-jamaican-bakery.json';
import perfect_fried_chicken from './perfect-fried-chicken.json';
import perfect_poundcakes from './perfect-poundcakes.json';
import perfusion_espresso_and_cocktail_bar from './perfusion-espresso-and-cocktail-bar.json';
import perk_on_main from './perk-on-main.json';
import perks_corks from './perks-corks.json';
import perrot_memorial_library from './perrot-memorial-library.json';
import phantasia_at_sunrise_square from './phantasia-at-sunrise-square.json';
import pho_170 from './pho-170.json';
import pho_501 from './pho-501.json';
import pho_ethan from './pho-ethan.json';
import pho_house_llc from './pho-house-llc.json';
import pho_vietnam from './pho-vietnam.json';
import pick_and_mix from './pick-and-mix.json';
import pig_rig_bbq from './pig-rig-bbq.json';
import pigs_eye_pub from './pigs-eye-pub.json';
import pilot from './pilot.json';
import pine_lodge from './pine-lodge.json';
import pinkberry from './pinkberry.json';
import pinots_palette_stamford from './pinots-palette-stamford.json';
import pitaziki from './pitaziki.json';
import pixel_bakery_flat from './pixel-bakery-flat.json';
import pizza_express from './pizza-express.json';
import pizza_mediterranean from './pizza-mediterranean.json';
import pizza_pilgrims from './pizza-pilgrims.json';
import pizza_pizzazz from './pizza-pizzazz.json';
import pizza_post from './pizza-post.json';
import pizza_station from './pizza-station.json';
import pizza_time from './pizza-time.json';
import pizza_works from './pizza-works.json';
import plan_b_burger_bar from './plan-b-burger-bar.json';
import plan_b from './plan-b.json';
import planet_pizza from './planet-pizza.json';
import plaskos_farm from './plaskos-farm.json';
import plaza_restaurant from './plaza-restaurant.json';
import pokemoto from './pokemoto.json';
import pokeworks from './pokeworks.json';
import pond_house_cafe from './pond-house-cafe.json';
import pontos_taverna from './pontos-taverna.json';
import ponza_italian_kitchen_pizza from './ponza-italian-kitchen-pizza.json';
import popovers from './popovers.json';
import pops_diner from './pops-diner.json';
import porter_house_burger_co from './porter-house-burger-co.json';
import portico_brewing from './portico-brewing.json';
import pos_rice_and_spice from './pos-rice-and-spice.json';
import posa_ristorante_vineria from './posa-ristorante-vineria.json';
import post_corner_pizza from './post-corner-pizza.json';
import potions_and_rations from './potions-and-rations.json';
import powder_room from './powder-room.json';
import prairie_schooner_bar_grill from './prairie-schooner-bar-grill.json';
import pralines_ice_cream from './pralines-ice-cream.json';
import present_company from './present-company.json';
import press_burger from './press-burger.json';
import prime_on_whitfield from './prime-on-whitfield.json';
import prime from './prime.json';
import priory_brewing_company from './priory-brewing-company.json';
import private_cafeteria from './private-cafeteria.json';
import project_canele from './project-canele.json';
import pronto from './pronto.json';
import proof_coffe_roasters from './proof-coffe-roasters.json';
import propeller_cafe from './propeller-cafe.json';
import puente_american_latino_pub from './puente-american-latino-pub.json';
import puerto_vallarta from './puerto-vallarta.json';
import pure_all_natural_juice_bar from './pure-all-natural-juice-bar.json';
import puritan_tavern from './puritan-tavern.json';
import putnam_restaurant from './putnam-restaurant.json';
import quaker_diner from './quaker-diner.json';
import quarry_coffee_co from './quarry-coffee-co.json';
import quartiere from './quartiere.json';
import queen_b_coffee_company from './queen-b-coffee-company.json';
import raineys_best from './raineys-best.json';
import ralph_n_richs from './ralph-n-richs.json';
import raon_jena_coffee_and_dessert from './raon-jena-coffee-and-dessert.json';
import rascals_brewing_company from './rascals-brewing-company.json';
import ray_kellys_pub from './ray-kellys-pub.json';
import rebel_dog_coffee_co from './rebel-dog-coffee-co.json';
import rebel_dog_coffee_company from './rebel-dog-coffee-company.json';
import rebel_dog_coffee from './rebel-dog-coffee.json';
import red_36_seafood from './red-36-seafood.json';
import red_apple from './red-apple.json';
import red_barn from './red-barn.json';
import red_fox_amore_restaurant from './red-fox-amore-restaurant.json';
import red_lobster from './red-lobster.json';
import red_lotus from './red-lotus.json';
import red_robin from './red-robin.json';
import red_rooster_deli_catering from './red-rooster-deli-catering.json';
import red_rooster_pub from './red-rooster-pub.json';
import red_rooster from './red-rooster.json';
import red_stone_pub from './red-stone-pub.json';
import redemption_brewing_co from './redemption-brewing-co.json';
import reds_cafe from './reds-cafe.json';
import remnant_brewing_company from './remnant-brewing-company.json';
import remos from './remos.json';
import reverends_restaurant from './reverends-restaurant.json';
import revolutions_sand_patio_bar from './revolutions-sand-patio-bar.json';
import ricotta_pizza_bakery from './ricotta-pizza-bakery.json';
import rikos_pizza from './rikos-pizza.json';
import rinconcito_salvadoreno from './rinconcito-salvadoreno.json';
import rio_bravo from './rio-bravo.json';
import ritas_italian_ice from './ritas-italian-ice.json';
import river_pub from './river-pub.json';
import river_rock_tavern from './river-rock-tavern.json';
import riverdale_diner from './riverdale-diner.json';
import rivers_edge_pub_and_grille from './rivers-edge-pub-and-grille.json';
import riverside_pizza from './riverside-pizza.json';
import rizzutos_oyster_bar_restaurant from './rizzutos-oyster-bar-restaurant.json';
import roadhouse_billiards_and_brew from './roadhouse-billiards-and-brew.json';
import roasted from './roasted.json';
import robertos_log_cabin from './robertos-log-cabin.json';
import robertos_restaurant from './robertos-restaurant.json';
import roddys_pizza_and_salad from './roddys-pizza-and-salad.json';
import roly_poly_bakery from './roly-poly-bakery.json';
import roma_sports_bar_grill from './roma-sports-bar-grill.json';
import rooftop120 from './rooftop120.json';
import roseland_apizza from './roseland-apizza.json';
import rossittos_ristorante from './rossittos-ristorante.json';
import rosy_tomorrows from './rosy-tomorrows.json';
import roti_boti from './roti-boti.json';
import roti_king from './roti-king.json';
import royal_buffet from './royal-buffet.json';
import royal_house_thai_cuisine from './royal-house-thai-cuisine.json';
import rozzis_eatery from './rozzis-eatery.json';
import ruby_tuesday from './ruby-tuesday.json';
import ruse_brewery_taproom from './ruse-brewery-taproom.json';
import rusty_rail_tavern from './rusty-rail-tavern.json';
import saint_bess_jerk_hub from './saint-bess-jerk-hub.json';
import saints from './saints.json';
import sake_cafe from './sake-cafe.json';
import sakura_garden_japanese_steak_house from './sakura-garden-japanese-steak-house.json';
import salem_valley_farms_ice_cream from './salem-valley-farms-ice-cream.json';
import salernos_apizza from './salernos-apizza.json';
import sallys_apizza from './sallys-apizza.json';
import salt_and_light from './salt-and-light.json';
import salt_straw from './salt-straw.json';
import salute from './salute.json';
import sam_adams_boston_brewery from './sam-adams-boston-brewery.json';
import sammamish_cafe_spirits from './sammamish-cafe-spirits.json';
import sammy_gs_tavern from './sammy-gs-tavern.json';
import san_souci_restaurant_lounge from './san-souci-restaurant-lounge.json';
import sandpiper from './sandpiper.json';
import santa_maria from './santa-maria.json';
import sarahs_breakfast_lunch from './sarahs-breakfast-lunch.json';
import saravanaa_bhavan from './saravanaa-bhavan.json';
import sassos_coal_fired_pizza from './sassos-coal-fired-pizza.json';
import sassy_mama from './sassy-mama.json';
import saugatuck_sweets_fairfield from './saugatuck-sweets-fairfield.json';
import saybrook_fish_house from './saybrook-fish-house.json';
import sbc_brewery_restaurant from './sbc-brewery-restaurant.json';
import scooters_coffee_shop from './scooters-coffee-shop.json';
import scooters_coffee from './scooters-coffee.json';
import scotts_jamaican_bakery from './scotts-jamaican-bakery.json';
import seed_kitchen_and_bagelry from './seed-kitchen-and-bagelry.json';
import sellfish_restaurant from './sellfish-restaurant.json';
import serenata_restaurant from './serenata-restaurant.json';
import seven_moons_thai_cuisine from './seven-moons-thai-cuisine.json';
import seven_star_chinese_food_take_out from './seven-star-chinese-food-take-out.json';
import shandals_vegetarian_cafe from './shandals-vegetarian-cafe.json';
import shangri_la_hot_woks_sushi_bar from './shangri-la-hot-woks-sushi-bar.json';
import shapeshifters_brewery from './shapeshifters-brewery.json';
import shayna_bs_by_the_sea from './shayna-bs-by-the-sea.json';
import sherwood_diner from './sherwood-diner.json';
import shiki_hana_japanese_bistro from './shiki-hana-japanese-bistro.json';
import shinjuku_station_revolving_sushi_bar from './shinjuku-station-revolving-sushi-bar.json';
import shipwreck_tavern from './shipwreck-tavern.json';
import shortts_farm_ice_cream from './shortts-farm-ice-cream.json';
import shukran from './shukran.json';
import shumi_revolving_sushi from './shumi-revolving-sushi.json';
import siam_glastonbury from './siam-glastonbury.json';
import sicily_coal_fired_pizza from './sicily-coal-fired-pizza.json';
import sign_of_the_whale from './sign-of-the-whale.json';
import silk_city_coffee from './silk-city-coffee.json';
import silver_sands_pizza from './silver-sands-pizza.json';
import simply_bowls_co from './simply-bowls-co.json';
import sip_wine_bar from './sip-wine-bar.json';
import sisson_ave_pizza from './sisson-ave-pizza.json';
import skeleton_key_brewery from './skeleton-key-brewery.json';
import skyline_restaurant from './skyline-restaurant.json';
import sliceworks from './sliceworks.json';
import sliders_cafe from './sliders-cafe.json';
import sliders_sports_bar_grill from './sliders-sports-bar-grill.json';
import sloe_lane_bakery from './sloe-lane-bakery.json';
import sneekers_cafe from './sneekers-cafe.json';
import snowy_village from './snowy-village.json';
import social_club from './social-club.json';
import sofias_restaurant_and_pizzeria from './sofias-restaurant-and-pizzeria.json';
import solun_bartapas from './solun-bartapas.json';
import soma_grille from './soma-grille.json';
import somos from './somos.json';
import sophias_breakfast_and_lunch from './sophias-breakfast-and-lunch.json';
import sophies from './sophies.json';
import sorrento_importing from './sorrento-importing.json';
import sound_coffee from './sound-coffee.json';
import south_highland_street_little_library_charter_32424 from './south-highland-street-little-library-charter-32424.json';
import southwest_cafe from './southwest-cafe.json';
import spice_venue from './spice-venue.json';
import spicy_green_bean from './spicy-green-bean.json';
import spring_garden_bakery_cafe from './spring-garden-bakery-cafe.json';
import spruce_coffee from './spruce-coffee.json';
import spruce_street_little_free_library_charter_150603 from './spruce-street-little-free-library-charter-150603.json';
import square_peg from './square-peg.json';
import stamford_pizza from './stamford-pizza.json';
import stamford_repairs from './stamford-repairs.json';
import stamford_salads from './stamford-salads.json';
import stans_jamaican_kitchen from './stans-jamaican-kitchen.json';
import star_pizza from './star-pizza.json';
import state_line_pizza from './state-line-pizza.json';
import steak_loft_of_mystic from './steak-loft-of-mystic.json';
import steam from './steam.json';
import stella_blues from './stella-blues.json';
import steves_place from './steves-place.json';
import stewards_of_the_land_brewery from './stewards-of-the-land-brewery.json';
import stone_barrel_brewing_company from './stone-barrel-brewing-company.json';
import stor_coffee from './stor-coffee.json';
import strange_brew_pub from './strange-brew-pub.json';
import streibels_espresso from './streibels-espresso.json';
import strykers_cafe from './strykers-cafe.json';
import submarine_veterans_club_inc_subvets from './submarine-veterans-club-inc-subvets.json';
import suffield_pizza from './suffield-pizza.json';
import sugar_daddys from './sugar-daddys.json';
import sun_patio_bar from './sun-patio-bar.json';
import sun_splash_bar_and_grill from './sun-splash-bar-and-grill.json';
import sunberry from './sunberry.json';
import sundown_saloon from './sundown-saloon.json';
import sunny_daes from './sunny-daes.json';
import sunset_bar_grill from './sunset-bar-grill.json';
import super_taqueria from './super-taqueria.json';
import supreme_pizza from './supreme-pizza.json';
import surf_cantina from './surf-cantina.json';
import sushi_koya from './sushi-koya.json';
import sushi_on_chapel from './sushi-on-chapel.json';
import sushi_soba from './sushi-soba.json';
import svea_club from './svea-club.json';
import sweet_alchemy_ice_creamery from './sweet-alchemy-ice-creamery.json';
import sweet_aroma_coffehouse_bakery from './sweet-aroma-coffehouse-bakery.json';
import sweet_claudes from './sweet-claudes.json';
import sweet_harmony_cafe_bakery from './sweet-harmony-cafe-bakery.json';
import sweet_mango___sushi_and_bistro from './sweet-mango---sushi-and-bistro.json';
import sweet_me_bakery from './sweet-me-bakery.json';
import sweet_scoops from './sweet-scoops.json';
import t_bone_prime_steakhouse from './t-bone-prime-steakhouse.json';
import t_swirl_crepe from './t-swirl-crepe.json';
import table_570 from './table-570.json';
import tabouli_and_more from './tabouli-and-more.json';
import taco_dia from './taco-dia.json';
import tacos_la_rosa from './tacos-la-rosa.json';
import tai_jiang from './tai-jiang.json';
import tailor_upholstery_co_inc from './tailor-upholstery-co-inc.json';
import taj from './taj.json';
import tan_wong from './tan-wong.json';
import tandoor_indian_restaurant from './tandoor-indian-restaurant.json';
import tandoori_taste_of_india from './tandoori-taste-of-india.json';
import tanoor_restaurant from './tanoor-restaurant.json';
import tapas from './tapas.json';
import taqueria_cinco from './taqueria-cinco.json';
import taqueria_cordoba from './taqueria-cordoba.json';
import taqueria_la_frontera from './taqueria-la-frontera.json';
import taqueria_la_michoacana from './taqueria-la-michoacana.json';
import taqueria_la_ventana from './taqueria-la-ventana.json';
import taquerio from './taquerio.json';
import tarantino_restaurant_bar from './tarantino-restaurant-bar.json';
import taste_of_thai from './taste-of-thai.json';
import tavern_in_the_square from './tavern-in-the-square.json';
import tavern_on_3 from './tavern-on-3.json';
import taverna_grill from './taverna-grill.json';
import tazza_cafe from './tazza-cafe.json';
import tazza_osteria_bar from './tazza-osteria-bar.json';
import tcby from './tcby.json';
import teds_restaurant_bar from './teds-restaurant-bar.json';
import teds_restaurant_steamed_cheeseburgers from './teds-restaurant-steamed-cheeseburgers.json';
import temescal_brewing from './temescal-brewing.json';
import ten_asian_bistro from './ten-asian-bistro.json';
import ten_one_tea_house from './ten-one-tea-house.json';
import tenth_hole_tavern from './tenth-hole-tavern.json';
import tequila_escape from './tequila-escape.json';
import terra_ristorante_italiano from './terra-ristorante-italiano.json';
import texas_de_brazil from './texas-de-brazil.json';
import thai_basil from './thai-basil.json';
import thai_palace from './thai-palace.json';
import thai_stories from './thai-stories.json';
import thai_time from './thai-time.json';
import thai_trio from './thai-trio.json';
import thamel from './thamel.json';
import the_202_tavern from './the-202-tavern.json';
import the_alley_scoop from './the-alley-scoop.json';
import the_angry_egg from './the-angry-egg.json';
import the_barony_bar from './the-barony-bar.json';
import the_basement from './the-basement.json';
import the_beamhouse from './the-beamhouse.json';
import the_beehive from './the-beehive.json';
import the_benjamin from './the-benjamin.json';
import the_big_dipper_ice_cream_parlour from './the-big-dipper-ice-cream-parlour.json';
import the_billiards_cafe from './the-billiards-cafe.json';
import the_black_duck from './the-black-duck.json';
import the_black_horse from './the-black-horse.json';
import the_black_rock_oyster_bar_grill from './the-black-rock-oyster-bar-grill.json';
import the_black_rose from './the-black-rose.json';
import the_blue_duck from './the-blue-duck.json';
import the_blue_elephant_trail from './the-blue-elephant-trail.json';
import the_boca_oyster_bar from './the-boca-oyster-bar.json';
import the_boot from './the-boot.json';
import the_botanist from './the-botanist.json';
import the_breakfast_express_by_mealionaire from './the-breakfast-express-by-mealionaire.json';
import the_brownstone from './the-brownstone.json';
import the_capital_grille from './the-capital-grille.json';
import the_cask_and_barrel from './the-cask-and-barrel.json';
import the_cheesecake_factory from './the-cheesecake-factory.json';
import the_cheesecake_shop from './the-cheesecake-shop.json';
import the_cone_zone from './the-cone-zone.json';
import the_copper_fox_saloon from './the-copper-fox-saloon.json';
import the_cove_deli from './the-cove-deli.json';
import the_cracker_barrel_pub from './the-cracker-barrel-pub.json';
import the_deck_at_river_twist from './the-deck-at-river-twist.json';
import the_dock_restaurant from './the-dock-restaurant.json';
import the_dome_bar_and_grill from './the-dome-bar-and-grill.json';
import the_doric from './the-doric.json';
import the_dragon from './the-dragon.json';
import the_elephant_trail from './the-elephant-trail.json';
import the_farm_italy_at_ballo_italian from './the-farm-italy-at-ballo-italian.json';
import the_farm_kitchen_ct from './the-farm-kitchen-ct.json';
import the_farmers_cow_calfe from './the-farmers-cow-calfe.json';
import the_fire_place from './the-fire-place.json';
import the_flour_girl_bakery from './the-flour-girl-bakery.json';
import the_flour_girl_cafe from './the-flour-girl-cafe.json';
import the_georgetown_owl_saloon_restaurant from './the-georgetown-owl-saloon-restaurant.json';
import the_ginger_man from './the-ginger-man.json';
import the_granola_bar from './the-granola-bar.json';
import the_green_man from './the-green-man.json';
import the_half_door from './the-half-door.json';
import the_harbor_coffeehouse from './the-harbor-coffeehouse.json';
import the_hare_and_hounds from './the-hare-and-hounds.json';
import the_hideaway from './the-hideaway.json';
import the_hop_pickers from './the-hop-pickers.json';
import the_hub_pizza_bar from './the-hub-pizza-bar.json';
import the_hungry_crab from './the-hungry-crab.json';
import the_ivory_restaurant_pub from './the-ivory-restaurant-pub.json';
import the_ivy from './the-ivy.json';
import the_juniper_library_charter_171197 from './the-juniper-library-charter-171197.json';
import the_kakery from './the-kakery.json';
import the_lake_tavern from './the-lake-tavern.json';
import the_landing from './the-landing.json';
import the_lansdowne_irish_pub_music_house from './the-lansdowne-irish-pub-music-house.json';
import the_lantern from './the-lantern.json';
import the_little_pub from './the-little-pub.json';
import the_loaded_goat_coffee_company from './the-loaded-goat-coffee-company.json';
import the_local_sixty_three from './the-local-sixty-three.json';
import the_lodge_bar from './the-lodge-bar.json';
import the_luke_brasserie from './the-luke-brasserie.json';
import the_magic_cup from './the-magic-cup.json';
import the_magnum from './the-magnum.json';
import the_main_pub from './the-main-pub.json';
import the_malt_shovel from './the-malt-shovel.json';
import the_market_at_the_meetinghouse from './the-market-at-the-meetinghouse.json';
import the_mill_on_the_river from './the-mill-on-the-river.json';
import the_moo_bar from './the-moo-bar.json';
import the_ninety_nine from './the-ninety-nine.json';
import the_north_house from './the-north-house.json';
import the_olde_mecklenburg_brewery from './the-olde-mecklenburg-brewery.json';
import the_olive_bar from './the-olive-bar.json';
import the_olive_branch_bistro from './the-olive-branch-bistro.json';
import the_one_korean_bbq_bar from './the-one-korean-bbq-bar.json';
import the_original_antonios from './the-original-antonios.json';
import the_owl from './the-owl.json';
import the_painted_pony from './the-painted-pony.json';
import the_pantry from './the-pantry.json';
import the_parkway_inn from './the-parkway-inn.json';
import the_pho_bar from './the-pho-bar.json';
import the_place_2_be from './the-place-2-be.json';
import the_place from './the-place.json';
import the_plum_tomato from './the-plum-tomato.json';
import the_port_of_call from './the-port-of-call.json';
import the_post from './the-post.json';
import the_quarterback_club from './the-quarterback-club.json';
import the_recovery_room from './the-recovery-room.json';
import the_red_hill_baker from './the-red-hill-baker.json';
import the_red_lion from './the-red-lion.json';
import the_restaurant_at_gillette_ridge from './the-restaurant-at-gillette-ridge.json';
import the_rooftop from './the-rooftop.json';
import the_royal_coachman_tavern from './the-royal-coachman-tavern.json';
import the_russell_grab_go from './the-russell-grab-go.json';
import the_russian_lady from './the-russian-lady.json';
import the_shack from './the-shack.json';
import the_shipwrights_daughter from './the-shipwrights-daughter.json';
import the_side_door_jazz_club from './the-side-door-jazz-club.json';
import the_sloppy_waffle from './the-sloppy-waffle.json';
import the_smithy_cafe from './the-smithy-cafe.json';
import the_social from './the-social.json';
import the_spence from './the-spence.json';
import the_spot from './the-spot.json';
import the_station_restaurant from './the-station-restaurant.json';
import the_towne_parlor from './the-towne-parlor.json';
import the_trinity from './the-trinity.json';
import the_triple_three_333 from './the-triple-three-333.json';
import the_village_restaurant from './the-village-restaurant.json';
import the_village_table from './the-village-table.json';
import the_waters_edge_at_giovannis from './the-waters-edge-at-giovannis.json';
import the_waterview from './the-waterview.json';
import the_wedge_inn from './the-wedge-inn.json';
import the_whale_tea from './the-whale-tea.json';
import the_white_horse_country_pub from './the-white-horse-country-pub.json';
import the_white_rabbit_coffee_house from './the-white-rabbit-coffee-house.json';
import the_whole_enchilada from './the-whole-enchilada.json';
import the_wicked_press from './the-wicked-press.json';
import the_winged_bear from './the-winged-bear.json';
import the_wu from './the-wu.json';
import thecafe from './thecafe.json';
import these_guys_brewing_company from './these-guys-brewing-company.json';
import third_barrel_brewing from './third-barrel-brewing.json';
import thistle_street_bar from './thistle-street-bar.json';
import thread_city_diner from './thread-city-diner.json';
import three_notchd_brewing_co from './three-notchd-brewing-co.json';
import tibetan_himalyan from './tibetan-himalyan.json';
import tiernans_bar_pub from './tiernans-bar-pub.json';
import tiger_belly_noodle_bar from './tiger-belly-noodle-bar.json';
import tigin_irish_pub from './tigin-irish-pub.json';
import tiles_bar from './tiles-bar.json';
import tinderbox from './tinderbox.json';
import ting_ho from './ting-ho.json';
import tisumi_sushi from './tisumi-sushi.json';
import toast_co from './toast-co.json';
import toast_coffee_house from './toast-coffee-house.json';
import toast_four_corners from './toast-four-corners.json';
import toasted from './toasted.json';
import tokyo_ramen from './tokyo-ramen.json';
import tolland_pizza from './tolland-pizza.json';
import tomato_basil from './tomato-basil.json';
import tomlinsons_restaurant from './tomlinsons-restaurant.json';
import tomo_202 from './tomo-202.json';
import tonos_mexican_restaurant from './tonos-mexican-restaurant.json';
import tonys_deli from './tonys-deli.json';
import tonys_flour_shop from './tonys-flour-shop.json';
import tonys_pizza from './tonys-pizza.json';
import torito_mexican from './torito-mexican.json';
import toro from './toro.json';
import toscana_italian_restaurant from './toscana-italian-restaurant.json';
import toshi from './toshi.json';
import town_brewing_co from './town-brewing-co.json';
import tox_brewing_company from './tox-brewing-company.json';
import tpk_brewing_co from './tpk-brewing-co.json';
import trackside_deli from './trackside-deli.json';
import trevi_lounge from './trevi-lounge.json';
import tribus_beer_company from './tribus-beer-company.json';
import trigo_wood_fired_pizza from './trigo-wood-fired-pizza.json';
import trillium_brewing from './trillium-brewing.json';
import trinfo_cafe_technology_center from './trinfo-cafe-technology-center.json';
import triple_aaa_pizza_restaurant from './triple-aaa-pizza-restaurant.json';
import triple_tri_coffee from './triple-tri-coffee.json';
import troupe429 from './troupe429.json';
import trumbull_kitchen from './trumbull-kitchen.json';
import ts_pizza_cafe from './ts-pizza-cafe.json';
import tunxis_grill_pizzeria from './tunxis-grill-pizzeria.json';
import turks_seafood_resturant_of_savin_rock from './turks-seafood-resturant-of-savin-rock.json';
import turtle_cafe from './turtle-cafe.json';
import twins_bbq_co from './twins-bbq-co.json';
import two_roads_food_hall_bar from './two-roads-food-hall-bar.json';
import uncle_bucks_fishbowl_grill from './uncle-bucks-fishbowl-grill.json';
import uncle_krankys_cafe from './uncle-krankys-cafe.json';
import uncle_leos_rosny_cafe from './uncle-leos-rosny-cafe.json';
import undercroft from './undercroft.json';
import union_league_cafe from './union-league-cafe.json';
import unionville_family_restaurant from './unionville-family-restaurant.json';
import uno_mas from './uno-mas.json';
import upper_crust_bakery_cafe from './upper-crust-bakery-cafe.json';
import upper_crust from './upper-crust.json';
import uptown_sandwiches from './uptown-sandwiches.json';
import urban_angel from './urban-angel.json';
import urban_lodge_brewing from './urban-lodge-brewing.json';
import urban_street_pizza from './urban-street-pizza.json';
import utica_bread from './utica-bread.json';
import utsav from './utsav.json';
import valbella_ristorante from './valbella-ristorante.json';
import valentinos_restaurant from './valentinos-restaurant.json';
import valley_diner from './valley-diner.json';
import van_leeuwen_ice_cream from './van-leeuwen-ice-cream.json';
import vanilla_bean_cafe from './vanilla-bean-cafe.json';
import vans from './vans.json';
import vault_coffeedeviant_donuts from './vault-coffeedeviant-donuts.json';
import vavalas_deli_catering_darien from './vavalas-deli-catering-darien.json';
import vazzys from './vazzys.json';
import vernon_diner from './vernon-diner.json';
import vespino from './vespino.json';
import vessel from './vessel.json';
import vi_vi_bubble_tea from './vi-vi-bubble-tea.json';
import via_emilia from './via-emilia.json';
import vietnam_palace from './vietnam-palace.json';
import villa_pizza from './villa-pizza.json';
import village_pizza from './village-pizza.json';
import vinette from './vinette.json';
import vinnys_ale_house from './vinnys-ale-house.json';
import vinnys_backyard_bbq from './vinnys-backyard-bbq.json';
import viron_rondo_osteria from './viron-rondo-osteria.json';
import viva_zapata from './viva-zapata.json';
import vivien from './vivien.json';
import vivo from './vivo.json';
import voodoo_rooms from './voodoo-rooms.json';
import wafu from './wafu.json';
import wagamama from './wagamama.json';
import wahaca from './wahaca.json';
import ward_9 from './ward-9.json';
import washington_pizza_house from './washington-pizza-house.json';
import washington_street_coffee_house from './washington-street-coffee-house.json';
import west_end_pizza from './west-end-pizza.json';
import west_main_pizza from './west-main-pizza.json';
import west_street_grill from './west-street-grill.json';
import wetlands_brew_pub_sports_bar from './wetlands-brew-pub-sports-bar.json';
import whale_tea from './whale-tea.json';
import when_pigs_fly from './when-pigs-fly.json';
import whiplash_beer from './whiplash-beer.json';
import white_horse from './white-horse.json';
import white_lotus_thai_restaurant_pho from './white-lotus-thai-restaurant-pho.json';
import whiting_lane_little_free_library_charter_43300 from './whiting-lane-little-free-library-charter-43300.json';
import whole_deli from './whole-deli.json';
import wicked_slice_pizza from './wicked-slice-pizza.json';
import wicked_slice from './wicked-slice.json';
import wicked_sweets_by_alyssa from './wicked-sweets-by-alyssa.json';
import willamette_brewery from './willamette-brewery.json';
import willimantic_brewing_company from './willimantic-brewing-company.json';
import willington_pizza_too from './willington-pizza-too.json';
import winchester_cafe from './winchester-cafe.json';
import windmill_tavern from './windmill-tavern.json';
import winfield_street_coffee from './winfield-street-coffee.json';
import wings_n_pies from './wings-n-pies.json';
import witchdoctor from './witchdoctor.json';
import wood_n_tap_restaurant from './wood-n-tap-restaurant.json';
import woodbury_brewing_company from './woodbury-brewing-company.json';
import woods_end_delicatessen from './woods-end-delicatessen.json';
import world_of_beer_bar_and_kitchen from './world-of-beer-bar-and-kitchen.json';
import yankee_cider_company from './yankee-cider-company.json';
import yannis_pizza_restaurant from './yannis-pizza-restaurant.json';
import yorkside_pizza_restaurant from './yorkside-pizza-restaurant.json';
import your_pie from './your-pie.json';
import yue_se_chinese_yue_se_chinese from './yue-se-chinese-yue-se-chinese.json';
import yume_hibachi_steak_and_sushi from './yume-hibachi-steak-and-sushi.json';
import zarraffas_coffee from './zarraffas-coffee.json';
import zaytoon_mediterranean_cuisine from './zaytoon-mediterranean-cuisine.json';
import zaza from './zaza.json';
import zinas_cucina from './zinas-cucina.json';
import zinc_new_haven from './zinc-new-haven.json';
import zoiglhaus_brewing from './zoiglhaus-brewing.json';
import zois_pizza from './zois-pizza.json';
import zois from './zois.json';

export const MERCHANTS_REGISTRY: Record<string, MerchantConfig> = {
  '1741-pub-and-grill': m_1741_pub_and_grill as unknown as MerchantConfig,
  '1754-house-restaurant-and-inn': m_1754_house_restaurant_and_inn as unknown as MerchantConfig,
  '19th-hole': m_19th_hole as unknown as MerchantConfig,
  '2-wives-brick-oven-eatery': m_2_wives_brick_oven_eatery as unknown as MerchantConfig,
  '374-kitchen-cocktails': m_374_kitchen_cocktails as unknown as MerchantConfig,
  '50-west': m_50_west as unknown as MerchantConfig,
  '7-brew-coffee': m_7_brew_coffee as unknown as MerchantConfig,
  '7-brew': m_7_brew as unknown as MerchantConfig,
  '80-proof-american-kitchen-bar': m_80_proof_american_kitchen_bar as unknown as MerchantConfig,
  '903-brewers': m_903_brewers as unknown as MerchantConfig,
  'abbotsford': abbotsford as unknown as MerchantConfig,
  'abc-pizza-restaurant': abc_pizza_restaurant as unknown as MerchantConfig,
  'abigails-grille-and-wine-bar': abigails_grille_and_wine_bar as unknown as MerchantConfig,
  'abis': abis as unknown as MerchantConfig,
  'ac-petersons-farms': ac_petersons_farms as unknown as MerchantConfig,
  'ackert-electric-and-barbershop': ackert_electric_and_barbershop as unknown as MerchantConfig,
  'adriatiks-restaurant-and-pizzeria': adriatiks_restaurant_and_pizzeria as unknown as MerchantConfig,
  'aero-diner': aero_diner as unknown as MerchantConfig,
  'aeronaut-brewing-company': aeronaut_brewing_company as unknown as MerchantConfig,
  'airway-restaurant': airway_restaurant as unknown as MerchantConfig,
  'akita-ramen': akita_ramen as unknown as MerchantConfig,
  'akita': akita as unknown as MerchantConfig,
  'alameda-brewing-co': alameda_brewing_co as unknown as MerchantConfig,
  'albany-avenue-food-mart-and-deli': albany_avenue_food_mart_and_deli as unknown as MerchantConfig,
  'alebrijes-mexican-fusion': alebrijes_mexican_fusion as unknown as MerchantConfig,
  'alinas-patisserie': alinas_patisserie as unknown as MerchantConfig,
  'almanac': almanac as unknown as MerchantConfig,
  'along-came-carol': along_came_carol as unknown as MerchantConfig,
  'alpen-pantry': alpen_pantry as unknown as MerchantConfig,
  'alpine-eatery': alpine_eatery as unknown as MerchantConfig,
  'altones-italian-restaurant': altones_italian_restaurant as unknown as MerchantConfig,
  'alvarium-beer-co': alvarium_beer_co as unknown as MerchantConfig,
  'ambrosia-bakery': ambrosia_bakery as unknown as MerchantConfig,
  'american-mexican-diner': american_mexican_diner as unknown as MerchantConfig,
  'anas-kitchen': anas_kitchen as unknown as MerchantConfig,
  'anaya-sushi': anaya_sushi as unknown as MerchantConfig,
  'anchor-cafe-sweets': anchor_cafe_sweets as unknown as MerchantConfig,
  'anchor-spa': anchor_spa as unknown as MerchantConfig,
  'andros-diner-restaurant': andros_diner_restaurant as unknown as MerchantConfig,
  'angelos-restaurant': angelos_restaurant as unknown as MerchantConfig,
  'angies-pizza': angies_pizza as unknown as MerchantConfig,
  'anna-liffeys-irish-pub-restaurant': anna_liffeys_irish_pub_restaurant as unknown as MerchantConfig,
  'antep-evi': antep_evi as unknown as MerchantConfig,
  'anya': anya as unknown as MerchantConfig,
  'applausi': applausi as unknown as MerchantConfig,
  'applebees': applebees as unknown as MerchantConfig,
  'aqui-me-quedo-restaurant': aqui_me_quedo_restaurant as unknown as MerchantConfig,
  'arch-street-tavern': arch_street_tavern as unknown as MerchantConfig,
  'archie-moores': archie_moores as unknown as MerchantConfig,
  'area-143': area_143 as unknown as MerchantConfig,
  'arezzo': arezzo as unknown as MerchantConfig,
  'armstrong-dining-hall': armstrong_dining_hall as unknown as MerchantConfig,
  'aroma-joes': aroma_joes as unknown as MerchantConfig,
  'aroma': aroma as unknown as MerchantConfig,
  'aroom': aroom as unknown as MerchantConfig,
  'artisan-roast': artisan_roast as unknown as MerchantConfig,
  'artisan': artisan as unknown as MerchantConfig,
  'ashleys-ice-cream': ashleys_ice_cream as unknown as MerchantConfig,
  'aspen': aspen as unknown as MerchantConfig,
  'aspetuk-brew-lab': aspetuk_brew_lab as unknown as MerchantConfig,
  'astin-js': astin_js as unknown as MerchantConfig,
  'asylum-street-pizza': asylum_street_pizza as unknown as MerchantConfig,
  'athena-diner': athena_diner as unknown as MerchantConfig,
  'atwater-brewery': atwater_brewery as unknown as MerchantConfig,
  'augie-and-rays': augie_and_rays as unknown as MerchantConfig,
  'august-moon': august_moon as unknown as MerchantConfig,
  'aux-delices': aux_delices as unknown as MerchantConfig,
  'away-days-brewing-co': away_days_brewing_co as unknown as MerchantConfig,
  'awesom-sauce-caribbean-restaurant': awesom_sauce_caribbean_restaurant as unknown as MerchantConfig,
  'azteca-market': azteca_market as unknown as MerchantConfig,
  'back-9-social': back_9_social as unknown as MerchantConfig,
  'back-east-brewing-company': back_east_brewing_company as unknown as MerchantConfig,
  'back-porch': back_porch as unknown as MerchantConfig,
  'baerlic-brewery': baerlic_brewery as unknown as MerchantConfig,
  'bagel-grove': bagel_grove as unknown as MerchantConfig,
  'bagel-king': bagel_king as unknown as MerchantConfig,
  'bagelicious-ii': bagelicious_ii as unknown as MerchantConfig,
  'bagelman': bagelman as unknown as MerchantConfig,
  'baingan': baingan as unknown as MerchantConfig,
  'baiten-bakery': baiten_bakery as unknown as MerchantConfig,
  'baja-cocina': baja_cocina as unknown as MerchantConfig,
  'bajas-mexican-restaurant': bajas_mexican_restaurant as unknown as MerchantConfig,
  'bakers-dozen': bakers_dozen as unknown as MerchantConfig,
  'bamboo-leaf': bamboo_leaf as unknown as MerchantConfig,
  'bar-louie': bar_louie as unknown as MerchantConfig,
  'bar-max': bar_max as unknown as MerchantConfig,
  'bar-new-haven': bar_new_haven as unknown as MerchantConfig,
  'bar-noodle': bar_noodle as unknown as MerchantConfig,
  'bar-rosso': bar_rosso as unknown as MerchantConfig,
  'barbary-coast': barbary_coast as unknown as MerchantConfig,
  'barcelona-wine-bar-stamford': barcelona_wine_bar_stamford as unknown as MerchantConfig,
  'barcelona': barcelona as unknown as MerchantConfig,
  'bare-burger': bare_burger as unknown as MerchantConfig,
  'barnaan': barnaan as unknown as MerchantConfig,
  'barosa-indian-kitchen-bar': barosa_indian_kitchen_bar as unknown as MerchantConfig,
  'barracuda': barracuda as unknown as MerchantConfig,
  'barrel-brothers-brewing-company': barrel_brothers_brewing_company as unknown as MerchantConfig,
  'bartaco': bartaco as unknown as MerchantConfig,
  'barvida': barvida as unknown as MerchantConfig,
  'baskin-robbins': baskin_robbins as unknown as MerchantConfig,
  'basta-tratoria': basta_tratoria as unknown as MerchantConfig,
  'bastard-barista': bastard_barista as unknown as MerchantConfig,
  'bears-smokehouse-barbecue': bears_smokehouse_barbecue as unknown as MerchantConfig,
  'bears-smokehouse-bbq': bears_smokehouse_bbq as unknown as MerchantConfig,
  'bedford-thai': bedford_thai as unknown as MerchantConfig,
  'beijing': beijing as unknown as MerchantConfig,
  'bella-fiore-restaurante-modelo-bar-grill': bella_fiore_restaurante_modelo_bar_grill as unknown as MerchantConfig,
  'bella-napoli-pizza-restaurant': bella_napoli_pizza_restaurant as unknown as MerchantConfig,
  'bella-napoli': bella_napoli as unknown as MerchantConfig,
  'bella-sicily': bella_sicily as unknown as MerchantConfig,
  'belle-vie-cafe': belle_vie_cafe as unknown as MerchantConfig,
  'bennys': bennys as unknown as MerchantConfig,
  'bereket-turkish-restaurant': bereket_turkish_restaurant as unknown as MerchantConfig,
  'berkins-blend-cafe': berkins_blend_cafe as unknown as MerchantConfig,
  'bertuccis': bertuccis as unknown as MerchantConfig,
  'bess-eaton': bess_eaton as unknown as MerchantConfig,
  'best-video-coffee-bar': best_video_coffee_bar as unknown as MerchantConfig,
  'between-rounds-bagels': between_rounds_bagels as unknown as MerchantConfig,
  'between-rounds': between_rounds as unknown as MerchantConfig,
  'bf-clydes-cider-mill': bf_clydes_cider_mill as unknown as MerchantConfig,
  'bianca': bianca as unknown as MerchantConfig,
  'big-fish-sushi': big_fish_sushi as unknown as MerchantConfig,
  'bills-crossroads-cafe': bills_crossroads_cafe as unknown as MerchantConfig,
  'billys-bakery': billys_bakery as unknown as MerchantConfig,
  'bin-228': bin_228 as unknown as MerchantConfig,
  'bin228': bin228 as unknown as MerchantConfig,
  'birdhouse-coffee': birdhouse_coffee as unknown as MerchantConfig,
  'bistro-coco': bistro_coco as unknown as MerchantConfig,
  'bistro-mediterranean-tapas-bar': bistro_mediterranean_tapas_bar as unknown as MerchantConfig,
  'bistro-v': bistro_v as unknown as MerchantConfig,
  'black-angus': black_angus as unknown as MerchantConfig,
  'black-bear-saloon': black_bear_saloon as unknown as MerchantConfig,
  'black-eyed-sallys': black_eyed_sallys as unknown as MerchantConfig,
  'black-pond-brews': black_pond_brews as unknown as MerchantConfig,
  'black-rock-tavern-and-restaurant': black_rock_tavern_and_restaurant as unknown as MerchantConfig,
  'black-sheep-coffee': black_sheep_coffee as unknown as MerchantConfig,
  'blackies': blackies as unknown as MerchantConfig,
  'blackwood-cafe-bar': blackwood_cafe_bar as unknown as MerchantConfig,
  'blairally-vintage-arcade': blairally_vintage_arcade as unknown as MerchantConfig,
  'bliss-vybz-west-indian-restaurant': bliss_vybz_west_indian_restaurant as unknown as MerchantConfig,
  'bloom-bake-shop': bloom_bake_shop as unknown as MerchantConfig,
  'blue-camel-cafe': blue_camel_cafe as unknown as MerchantConfig,
  'blue-colony-diner': blue_colony_diner as unknown as MerchantConfig,
  'blue-gate-boulanger-patissier': blue_gate_boulanger_patissier as unknown as MerchantConfig,
  'blue-orchid': blue_orchid as unknown as MerchantConfig,
  'blue-state-coffee': blue_state_coffee as unknown as MerchantConfig,
  'bobby-bs-roti-shop': bobby_bs_roti_shop as unknown as MerchantConfig,
  'bobby-vs-restaurant-sports-bar': bobby_vs_restaurant_sports_bar as unknown as MerchantConfig,
  'bobbys-apizza': bobbys_apizza as unknown as MerchantConfig,
  'bobbys-burger-palace': bobbys_burger_palace as unknown as MerchantConfig,
  'bohemian-pizza-and-taco': bohemian_pizza_and_taco as unknown as MerchantConfig,
  'bollywood-dreams': bollywood_dreams as unknown as MerchantConfig,
  'bombay-olive': bombay_olive as unknown as MerchantConfig,
  'bonani-kitchen': bonani_kitchen as unknown as MerchantConfig,
  'bonchon-chicken': bonchon_chicken as unknown as MerchantConfig,
  'bonessesss-apizza': bonessesss_apizza as unknown as MerchantConfig,
  'boost-bowls': boost_bowls as unknown as MerchantConfig,
  'border-grill': border_grill as unknown as MerchantConfig,
  'boston-bar': boston_bar as unknown as MerchantConfig,
  'boston-fish-n-chips': boston_fish_n_chips as unknown as MerchantConfig,
  'boulders-bar-grill': boulders_bar_grill as unknown as MerchantConfig,
  'bozrah-pizza-restaurant': bozrah_pizza_restaurant as unknown as MerchantConfig,
  'bradfords-grill-tavern': bradfords_grill_tavern as unknown as MerchantConfig,
  'bradley-wall': bradley_wall as unknown as MerchantConfig,
  'bramble': bramble as unknown as MerchantConfig,
  'branford-townhouse-restaurant': branford_townhouse_restaurant as unknown as MerchantConfig,
  'bravo': bravo as unknown as MerchantConfig,
  'bread-chocolate': bread_chocolate as unknown as MerchantConfig,
  'bread-street-kitchen-bar': bread_street_kitchen_bar as unknown as MerchantConfig,
  'brendas-cupcakes': brendas_cupcakes as unknown as MerchantConfig,
  'brennans-by-the-beach': brennans_by_the_beach as unknown as MerchantConfig,
  'brennans-shebeen': brennans_shebeen as unknown as MerchantConfig,
  'brewdog': brewdog as unknown as MerchantConfig,
  'brewery-legitimus': brewery_legitimus as unknown as MerchantConfig,
  'brewett-city-pub': brewett_city_pub as unknown as MerchantConfig,
  'brewport': brewport as unknown as MerchantConfig,
  'brick-basil-wood-fired-pizza-company': brick_basil_wood_fired_pizza_company as unknown as MerchantConfig,
  'brick-oven-on-main': brick_oven_on_main as unknown as MerchantConfig,
  'brickhouse-bar-grill': brickhouse_bar_grill as unknown as MerchantConfig,
  'brickoven-pizza': brickoven_pizza as unknown as MerchantConfig,
  'bridgewater-village-store-bistro': bridgewater_village_store_bistro as unknown as MerchantConfig,
  'brisas-latin-fusion': brisas_latin_fusion as unknown as MerchantConfig,
  'broasterant': broasterant as unknown as MerchantConfig,
  'brookside-bagels': brookside_bagels as unknown as MerchantConfig,
  'brown-beard-brewing': brown_beard_brewing as unknown as MerchantConfig,
  'brunos-cafe': brunos_cafe as unknown as MerchantConfig,
  'brunos-pizza': brunos_pizza as unknown as MerchantConfig,
  'bryac-restaurant-raw-bar': bryac_restaurant_raw_bar as unknown as MerchantConfig,
  'buckland-billiards': buckland_billiards as unknown as MerchantConfig,
  'buckland-grill-pizza': buckland_grill_pizza as unknown as MerchantConfig,
  'bucks-bar': bucks_bar as unknown as MerchantConfig,
  'buen-gusto': buen_gusto as unknown as MerchantConfig,
  'buffalo-bills': buffalo_bills as unknown as MerchantConfig,
  'buffalo-wild-wings': buffalo_wild_wings as unknown as MerchantConfig,
  'burrito-shack': burrito_shack as unknown as MerchantConfig,
  'butchers-bakers': butchers_bakers as unknown as MerchantConfig,
  'butterfly-chinese-restaurant': butterfly_chinese_restaurant as unknown as MerchantConfig,
  'buttonwood-farm-ice-cream': buttonwood_farm_ice_cream as unknown as MerchantConfig,
  'c-j-eaterieslottery-bar': c_j_eaterieslottery_bar as unknown as MerchantConfig,
  'cadys-tea-room': cadys_tea_room as unknown as MerchantConfig,
  'cafe-amici': cafe_amici as unknown as MerchantConfig,
  'cafe-ca-nole': cafe_ca_nole as unknown as MerchantConfig,
  'cafe-feva': cafe_feva as unknown as MerchantConfig,
  'cafe-hanover': cafe_hanover as unknown as MerchantConfig,
  'cafe-luca-belle': cafe_luca_belle as unknown as MerchantConfig,
  'cafe-portrait': cafe_portrait as unknown as MerchantConfig,
  'cafe-sofia': cafe_sofia as unknown as MerchantConfig,
  'cafe-susu-by-suitsupply': cafe_susu_by_suitsupply as unknown as MerchantConfig,
  'cafe-tinto': cafe_tinto as unknown as MerchantConfig,
  'cafeteria-morla-restaurant': cafeteria_morla_restaurant as unknown as MerchantConfig,
  'caffe-bene': caffe_bene as unknown as MerchantConfig,
  'caffe-nero': caffe_nero as unknown as MerchantConfig,
  'caffe-strega': caffe_strega as unknown as MerchantConfig,
  'caio-cafe': caio_cafe as unknown as MerchantConfig,
  'cairngorm-coffee-shop': cairngorm_coffee_shop as unknown as MerchantConfig,
  'caius-farm-brewery': caius_farm_brewery as unknown as MerchantConfig,
  'cajun-cafe': cajun_cafe as unknown as MerchantConfig,
  'calabria-coffee': calabria_coffee as unknown as MerchantConfig,
  'california-pizza-kitchen': california_pizza_kitchen as unknown as MerchantConfig,
  'cambridge-brew-pub': cambridge_brew_pub as unknown as MerchantConfig,
  'camerons-deli': camerons_deli as unknown as MerchantConfig,
  'camilles-wood-fired-pizza': camilles_wood_fired_pizza as unknown as MerchantConfig,
  'candlewood-inn': candlewood_inn as unknown as MerchantConfig,
  'canggio-restaurant-and-bar': canggio_restaurant_and_bar as unknown as MerchantConfig,
  'canoe': canoe as unknown as MerchantConfig,
  'cantina-mexicana': cantina_mexicana as unknown as MerchantConfig,
  'cantina-ristorante': cantina_ristorante as unknown as MerchantConfig,
  'capitol-lunch': capitol_lunch as unknown as MerchantConfig,
  'capitol-pizza': capitol_pizza as unknown as MerchantConfig,
  'captain-scotts-lobster-dock': captain_scotts_lobster_dock as unknown as MerchantConfig,
  'cardillos-deli-plus': cardillos_deli_plus as unknown as MerchantConfig,
  'carmelas-pasta-shop': carmelas_pasta_shop as unknown as MerchantConfig,
  'carmines-bar-grill-stage': carmines_bar_grill_stage as unknown as MerchantConfig,
  'carmines-pizza': carmines_pizza as unknown as MerchantConfig,
  'carribean-delices': carribean_delices as unknown as MerchantConfig,
  'carvel': carvel as unknown as MerchantConfig,
  'casa-mia-at-the-hawthorne': casa_mia_at_the_hawthorne as unknown as MerchantConfig,
  'caseys-cavern': caseys_cavern as unknown as MerchantConfig,
  'catch-a-healthy-habit-cafe': catch_a_healthy_habit_cafe as unknown as MerchantConfig,
  'cedarhurst-cafe': cedarhurst_cafe as unknown as MerchantConfig,
  'cedars-steaks-oysters': cedars_steaks_oysters as unknown as MerchantConfig,
  'cellarmaker-brewing-co': cellarmaker_brewing_co as unknown as MerchantConfig,
  'center-street-brewing-company': center_street_brewing_company as unknown as MerchantConfig,
  'center-street': center_street as unknown as MerchantConfig,
  'cfcf-coffee': cfcf_coffee as unknown as MerchantConfig,
  'chaiwalla-tea-room': chaiwalla_tea_room as unknown as MerchantConfig,
  'chameleon-cafe': chameleon_cafe as unknown as MerchantConfig,
  'chapter-one': chapter_one as unknown as MerchantConfig,
  'charhause-gluten-free-microbakery-cafe': charhause_gluten_free_microbakery_cafe as unknown as MerchantConfig,
  'charlies-sandwich-shoppe': charlies_sandwich_shoppe as unknown as MerchantConfig,
  'charter-oak-brewing': charter_oak_brewing as unknown as MerchantConfig,
  'chaves-bakery-deli': chaves_bakery_deli as unknown as MerchantConfig,
  'chaves-bakery-ii': chaves_bakery_ii as unknown as MerchantConfig,
  'chaves-restaurant': chaves_restaurant as unknown as MerchantConfig,
  'chengs-kitchen-chinese-restaurant': chengs_kitchen_chinese_restaurant as unknown as MerchantConfig,
  'cherry-tree-farm': cherry_tree_farm as unknown as MerchantConfig,
  'cheshire-pizza-and-ale': cheshire_pizza_and_ale as unknown as MerchantConfig,
  'chesters-barbecue': chesters_barbecue as unknown as MerchantConfig,
  'chiangmai-thai': chiangmai_thai as unknown as MerchantConfig,
  'chicago-sams-sports-bar-grille': chicago_sams_sports_bar_grille as unknown as MerchantConfig,
  'chicken-joes': chicken_joes as unknown as MerchantConfig,
  'chicken-pizza-point': chicken_pizza_point as unknown as MerchantConfig,
  'chickys-grill-and-pizza': chickys_grill_and_pizza as unknown as MerchantConfig,
  'chilis': chilis as unknown as MerchantConfig,
  'china-garden': china_garden as unknown as MerchantConfig,
  'china-one': china_one as unknown as MerchantConfig,
  'china-sky': china_sky as unknown as MerchantConfig,
  'chinese-gourmet-kitchen': chinese_gourmet_kitchen as unknown as MerchantConfig,
  'choupette-creperie': choupette_creperie as unknown as MerchantConfig,
  'christys-restaurant': christys_restaurant as unknown as MerchantConfig,
  'chubbys': chubbys as unknown as MerchantConfig,
  'chucks-steak-house-and-margarita-grill-mexican-cantina': chucks_steak_house_and_margarita_grill_mexican_cantina as unknown as MerchantConfig,
  'chucks-steak-house': chucks_steak_house as unknown as MerchantConfig,
  'chulos-stuffed-cookies': chulos_stuffed_cookies as unknown as MerchantConfig,
  'chutni-biryani-noodle-bar': chutni_biryani_noodle_bar as unknown as MerchantConfig,
  'cinco-de-mayo': cinco_de_mayo as unknown as MerchantConfig,
  'citrus': citrus as unknown as MerchantConfig,
  'cjs-coffee-cafe': cjs_coffee_cafe as unknown as MerchantConfig,
  'claim-52-brewing': claim_52_brewing as unknown as MerchantConfig,
  'claires-corner-copia': claires_corner_copia as unknown as MerchantConfig,
  'clocktown-brewing': clocktown_brewing as unknown as MerchantConfig,
  'clover-pharmacy': clover_pharmacy as unknown as MerchantConfig,
  'club-glenvale': club_glenvale as unknown as MerchantConfig,
  'cobs-bread': cobs_bread as unknown as MerchantConfig,
  'coco-thai-kitchen': coco_thai_kitchen as unknown as MerchantConfig,
  'coconut-cafe': coconut_cafe as unknown as MerchantConfig,
  'coffee-an-donut-shop': coffee_an_donut_shop as unknown as MerchantConfig,
  'coffee-cabinet': coffee_cabinet as unknown as MerchantConfig,
  'cold-plate-dessert': cold_plate_dessert as unknown as MerchantConfig,
  'cold-stone-creamery': cold_stone_creamery as unknown as MerchantConfig,
  'cold-town-brewery': cold_town_brewery as unknown as MerchantConfig,
  'coldfire-brewing': coldfire_brewing as unknown as MerchantConfig,
  'coles-road-brewing': coles_road_brewing as unknown as MerchantConfig,
  'colony-grill': colony_grill as unknown as MerchantConfig,
  'comix-roadhouse-comedy-club': comix_roadhouse_comedy_club as unknown as MerchantConfig,
  'common-grounds': common_grounds as unknown as MerchantConfig,
  'concentric-brewing-company': concentric_brewing_company as unknown as MerchantConfig,
  'connecticut-valley-brewing-company': connecticut_valley_brewing_company as unknown as MerchantConfig,
  'corbos-corner-deli': corbos_corner_deli as unknown as MerchantConfig,
  'coromandel-cuisine-of-india': coromandel_cuisine_of_india as unknown as MerchantConfig,
  'corwall-country-bakery': corwall_country_bakery as unknown as MerchantConfig,
  'cos-cobber': cos_cobber as unknown as MerchantConfig,
  'cosmic-cafe-smoothie-bar': cosmic_cafe_smoothie_bar as unknown as MerchantConfig,
  'costellos-clam-shack': costellos_clam_shack as unknown as MerchantConfig,
  'counterweight-brewing-company': counterweight_brewing_company as unknown as MerchantConfig,
  'country-diner': country_diner as unknown as MerchantConfig,
  'country-view-restaurant': country_view_restaurant as unknown as MerchantConfig,
  'courtyard-restaurant': courtyard_restaurant as unknown as MerchantConfig,
  'cove-pizza': cove_pizza as unknown as MerchantConfig,
  'crackd-kitchen-coffee': crackd_kitchen_coffee as unknown as MerchantConfig,
  'craft-260': craft_260 as unknown as MerchantConfig,
  'craft-pizza-company': craft_pizza_company as unknown as MerchantConfig,
  'cravings-an-american-eatery': cravings_an_american_eatery as unknown as MerchantConfig,
  'cream-coffee-shop': cream_coffee_shop as unknown as MerchantConfig,
  'creative-cakes-by-donna': creative_cakes_by_donna as unknown as MerchantConfig,
  'cromwell-diner': cromwell_diner as unknown as MerchantConfig,
  'cromwell-pizza-pasta': cromwell_pizza_pasta as unknown as MerchantConfig,
  'crown-and-hammer': crown_and_hammer as unknown as MerchantConfig,
  'crown-bar': crown_bar as unknown as MerchantConfig,
  'crown-super-market': crown_super_market as unknown as MerchantConfig,
  'crumbles-bakery': crumbles_bakery as unknown as MerchantConfig,
  'cuginos-pizzeria': cuginos_pizzeria as unknown as MerchantConfig,
  'curleys-diner': curleys_diner as unknown as MerchantConfig,
  'curry-hurry': curry_hurry as unknown as MerchantConfig,
  'cw-chops-catch': cw_chops_catch as unknown as MerchantConfig,
  'da-capo': da_capo as unknown as MerchantConfig,
  'dads-restaurant': dads_restaurant as unknown as MerchantConfig,
  'dal-mare-pizzeria': dal_mare_pizzeria as unknown as MerchantConfig,
  'danos-pub': danos_pub as unknown as MerchantConfig,
  'daybreak': daybreak as unknown as MerchantConfig,
  'deborah-anns-sweet-shoppe': deborah_anns_sweet_shoppe as unknown as MerchantConfig,
  'debs-place-restaurant': debs_place_restaurant as unknown as MerchantConfig,
  'dees-one-smart-cookie': dees_one_smart_cookie as unknown as MerchantConfig,
  'deja-brew': deja_brew as unknown as MerchantConfig,
  'demo': demo as unknown as MerchantConfig,
  'democracy-brewing': democracy_brewing as unknown as MerchantConfig,
  'den-sake-brewery': den_sake_brewery as unknown as MerchantConfig,
  'denovellis': denovellis as unknown as MerchantConfig,
  'devils-advocate': devils_advocate as unknown as MerchantConfig,
  'dew-drop-inn': dew_drop_inn as unknown as MerchantConfig,
  'dhaba-wala-indian-kitchen': dhaba_wala_indian_kitchen as unknown as MerchantConfig,
  'dianes-restaurant': dianes_restaurant as unknown as MerchantConfig,
  'diesel-lounge': diesel_lounge as unknown as MerchantConfig,
  'difrancos': difrancos as unknown as MerchantConfig,
  'dimare-pastry-shop': dimare_pastry_shop as unknown as MerchantConfig,
  'dimitris-diner': dimitris_diner as unknown as MerchantConfig,
  'dippin-dots': dippin_dots as unknown as MerchantConfig,
  'dirt-20': dirt_20 as unknown as MerchantConfig,
  'dish-n-dat': dish_n_dat as unknown as MerchantConfig,
  'dishoom': dishoom as unknown as MerchantConfig,
  'dive-bar-restaurant': dive_bar_restaurant as unknown as MerchantConfig,
  'dockside-brewery': dockside_brewery as unknown as MerchantConfig,
  'dog-lane-cafe': dog_lane_cafe as unknown as MerchantConfig,
  'dolce-italian-cafe': dolce_italian_cafe as unknown as MerchantConfig,
  'doms-broad-st-eatery': doms_broad_st_eatery as unknown as MerchantConfig,
  'doms-coffee': doms_coffee as unknown as MerchantConfig,
  'donde-julio-restaurant': donde_julio_restaurant as unknown as MerchantConfig,
  'dono-cafe': dono_cafe as unknown as MerchantConfig,
  'donovans-reef': donovans_reef as unknown as MerchantConfig,
  'donut-delight': donut_delight as unknown as MerchantConfig,
  'donut-inc': donut_inc as unknown as MerchantConfig,
  'dora-marketplace': dora_marketplace as unknown as MerchantConfig,
  'dorchester-brewing-company': dorchester_brewing_company as unknown as MerchantConfig,
  'doro-marketplace': doro_marketplace as unknown as MerchantConfig,
  'downeast-cider': downeast_cider as unknown as MerchantConfig,
  'dp-dough': dp_dough as unknown as MerchantConfig,
  'dragon-168': dragon_168 as unknown as MerchantConfig,
  'dragos-kitchen': dragos_kitchen as unknown as MerchantConfig,
  'drawdown-brewing': drawdown_brewing as unknown as MerchantConfig,
  'duchess': duchess as unknown as MerchantConfig,
  'e-e-enterprises': e_e_enterprises as unknown as MerchantConfig,
  'early-bird-cafe': early_bird_cafe as unknown as MerchantConfig,
  'east-london-brewery': east_london_brewery as unknown as MerchantConfig,
  'east-rock-bakery': east_rock_bakery as unknown as MerchantConfig,
  'east-side-restaurant': east_side_restaurant as unknown as MerchantConfig,
  'eastend': eastend as unknown as MerchantConfig,
  'echo-craft-kitchen': echo_craft_kitchen as unknown as MerchantConfig,
  'eco-chic-salon-spa-blow-dry-bar': eco_chic_salon_spa_blow_dry_bar as unknown as MerchantConfig,
  'eco-coffee-house': eco_coffee_house as unknown as MerchantConfig,
  'eddies-pizza-pasta': eddies_pizza_pasta as unknown as MerchantConfig,
  'edis-pizzaria-restaurant': edis_pizzaria_restaurant as unknown as MerchantConfig,
  'edo-ichi-sushi-and-hibachi-steakhouse': edo_ichi_sushi_and_hibachi_steakhouse as unknown as MerchantConfig,
  'ekkalak-thai-cuisine': ekkalak_thai_cuisine as unknown as MerchantConfig,
  'el-agave-bar': el_agave_bar as unknown as MerchantConfig,
  'el-coyote': el_coyote as unknown as MerchantConfig,
  'el-pollo-guapo': el_pollo_guapo as unknown as MerchantConfig,
  'el-rio-lempa': el_rio_lempa as unknown as MerchantConfig,
  'el-tenampa': el_tenampa as unknown as MerchantConfig,
  'el-traviezo-mexican-food': el_traviezo_mexican_food as unknown as MerchantConfig,
  'el-traviezo-mexican-restaurant': el_traviezo_mexican_restaurant as unknown as MerchantConfig,
  'elbow-room': elbow_room as unknown as MerchantConfig,
  'elephant-trail': elephant_trail as unknown as MerchantConfig,
  'elicit-brewing-co': elicit_brewing_co as unknown as MerchantConfig,
  'elis-on-whitney': elis_on_whitney as unknown as MerchantConfig,
  'elis-tavern': elis_tavern as unknown as MerchantConfig,
  'elm-city-social': elm_city_social as unknown as MerchantConfig,
  'elm-st-diner': elm_st_diner as unknown as MerchantConfig,
  'elmos-dockside': elmos_dockside as unknown as MerchantConfig,
  'elsol': elsol as unknown as MerchantConfig,
  'eltana': eltana as unknown as MerchantConfig,
  'enl-nail-bar': enl_nail_bar as unknown as MerchantConfig,
  'entenmanns-oroweat-bimbo-bakeries': entenmanns_oroweat_bimbo_bakeries as unknown as MerchantConfig,
  'enzos-pizza-deli': enzos_pizza_deli as unknown as MerchantConfig,
  'epicure-brewing': epicure_brewing as unknown as MerchantConfig,
  'erniess-delicatessen': erniess_delicatessen as unknown as MerchantConfig,
  'esca-restaurant-wine-bar': esca_restaurant_wine_bar as unknown as MerchantConfig,
  'euro-lounge': euro_lounge as unknown as MerchantConfig,
  'euro-plate': euro_plate as unknown as MerchantConfig,
  'evelyns-coffee-banh-mi': evelyns_coffee_banh_mi as unknown as MerchantConfig,
  'everlasting-nutrition': everlasting_nutrition as unknown as MerchantConfig,
  'express-ranch-pizzeria': express_ranch_pizzeria as unknown as MerchantConfig,
  'faction-brewing': faction_brewing as unknown as MerchantConfig,
  'fairfield-pizza': fairfield_pizza as unknown as MerchantConfig,
  'fairways-bar-and-grill': fairways_bar_and_grill as unknown as MerchantConfig,
  'falling-sky-brewing-house': falling_sky_brewing_house as unknown as MerchantConfig,
  'family-pizza': family_pizza as unknown as MerchantConfig,
  'fancy-pants-cafe': fancy_pants_cafe as unknown as MerchantConfig,
  'far-eastern-buffet': far_eastern_buffet as unknown as MerchantConfig,
  'farmer-horse-coffee': farmer_horse_coffee as unknown as MerchantConfig,
  'farmington-gardens': farmington_gardens as unknown as MerchantConfig,
  'farmington-miniature-golf-ice-cream-parlor': farmington_miniature_golf_ice_cream_parlor as unknown as MerchantConfig,
  'farrells': farrells as unknown as MerchantConfig,
  'fat-cat-grill-and-bar': fat_cat_grill_and_bar as unknown as MerchantConfig,
  'federal-cafe': federal_cafe as unknown as MerchantConfig,
  'feng': feng as unknown as MerchantConfig,
  'fenton-river-grill': fenton_river_grill as unknown as MerchantConfig,
  'ferris-acres-creamery': ferris_acres_creamery as unknown as MerchantConfig,
  'fifen-drum-restaurant-and-inn': fifen_drum_restaurant_and_inn as unknown as MerchantConfig,
  'figaro': figaro as unknown as MerchantConfig,
  'figs': figs as unknown as MerchantConfig,
  'fika': fika as unknown as MerchantConfig,
  'filberts-old-time-root-beer': filberts_old_time_root_beer as unknown as MerchantConfig,
  'filhos-cucina': filhos_cucina as unknown as MerchantConfig,
  'fino-wood-fired-pizza-bar': fino_wood_fired_pizza_bar as unknown as MerchantConfig,
  'fire-at-the-ridge': fire_at_the_ridge as unknown as MerchantConfig,
  'fire-engine-pizza-co': fire_engine_pizza_co as unknown as MerchantConfig,
  'fire-n-spice-vegan-restaurant-sports-cafe': fire_n_spice_vegan_restaurant_sports_cafe as unknown as MerchantConfig,
  'firebrands-bar-grill': firebrands_bar_grill as unknown as MerchantConfig,
  'fish': fish as unknown as MerchantConfig,
  'five-horses-tavern---south-end': five_horses_tavern___south_end as unknown as MerchantConfig,
  'flanders-fish-market-and-restaurant': flanders_fish_market_and_restaurant as unknown as MerchantConfig,
  'fletchs-bagels': fletchs_bagels as unknown as MerchantConfig,
  'flinders-lane': flinders_lane as unknown as MerchantConfig,
  'flipside': flipside as unknown as MerchantConfig,
  'flobees-roadside-stand': flobees_roadside_stand as unknown as MerchantConfig,
  'flour-bakery-cafe': flour_bakery_cafe as unknown as MerchantConfig,
  'flour-water-salt-bread': flour_water_salt_bread as unknown as MerchantConfig,
  'flying-monkey-grill-bar': flying_monkey_grill_bar as unknown as MerchantConfig,
  'flying-saucer-cypress-waters': flying_saucer_cypress_waters as unknown as MerchantConfig,
  'focaccias-cafe-catery': focaccias_cafe_catery as unknown as MerchantConfig,
  'fonda': fonda as unknown as MerchantConfig,
  'forbidden-llama': forbidden_llama as unknown as MerchantConfig,
  'ford-news-diner': ford_news_diner as unknown as MerchantConfig,
  'fords-lobster': fords_lobster as unknown as MerchantConfig,
  'forge-vine': forge_vine as unknown as MerchantConfig,
  'fornarellis-ristorante-bar': fornarellis_ristorante_bar as unknown as MerchantConfig,
  'fortina': fortina as unknown as MerchantConfig,
  'fortune-fuel': fortune_fuel as unknown as MerchantConfig,
  'fortune-garden-chinese-restaurant': fortune_garden_chinese_restaurant as unknown as MerchantConfig,
  'four-mile-inn': four_mile_inn as unknown as MerchantConfig,
  'four-provinces-brewing-company': four_provinces_brewing_company as unknown as MerchantConfig,
  'francescas-wine-bar-bistro': francescas_wine_bar_bistro as unknown as MerchantConfig,
  'franco-giannis': franco_giannis as unknown as MerchantConfig,
  'franco-manca': franco_manca as unknown as MerchantConfig,
  'frank-pepe-pizzeria-napoletana': frank_pepe_pizzeria_napoletana as unknown as MerchantConfig,
  'frank-pepes-west-hartford': frank_pepes_west_hartford as unknown as MerchantConfig,
  'frankie-jos': frankie_jos as unknown as MerchantConfig,
  'frankies-diner': frankies_diner as unknown as MerchantConfig,
  'frankies-pizza': frankies_pizza as unknown as MerchantConfig,
  'frankies-sport-lounge': frankies_sport_lounge as unknown as MerchantConfig,
  'franklin-town-line-diner': franklin_town_line_diner as unknown as MerchantConfig,
  'franks-willow-inn-bar-and-grill': franks_willow_inn_bar_and_grill as unknown as MerchantConfig,
  'fremont-brewing-company': fremont_brewing_company as unknown as MerchantConfig,
  'frenchies-coffee-bar-at-silver-sands-state-park': frenchies_coffee_bar_at_silver_sands_state_park as unknown as MerchantConfig,
  'fresca-tequila-bar-grill': fresca_tequila_bar_grill as unknown as MerchantConfig,
  'friendlys': friendlys as unknown as MerchantConfig,
  'front-street-bistro': front_street_bistro as unknown as MerchantConfig,
  'frost-bite': frost_bite as unknown as MerchantConfig,
  'frutiere-de-bevaix': frutiere_de_bevaix as unknown as MerchantConfig,
  'fuego-mexican-grill': fuego_mexican_grill as unknown as MerchantConfig,
  'fuji': fuji as unknown as MerchantConfig,
  'g-cafe': g_cafe as unknown as MerchantConfig,
  'galaxy-diner': galaxy_diner as unknown as MerchantConfig,
  'gallo': gallo as unknown as MerchantConfig,
  'gallos-restaurant': gallos_restaurant as unknown as MerchantConfig,
  'gansett-wraps': gansett_wraps as unknown as MerchantConfig,
  'garibaldi': garibaldi as unknown as MerchantConfig,
  'garos-place': garos_place as unknown as MerchantConfig,
  'gaucho': gaucho as unknown as MerchantConfig,
  'gayles-depot-square-farm-shoppe': gayles_depot_square_farm_shoppe as unknown as MerchantConfig,
  'gennaros-apizza-cafe': gennaros_apizza_cafe as unknown as MerchantConfig,
  'georges-pizza-restaurant': georges_pizza_restaurant as unknown as MerchantConfig,
  'georgetown-cupcake': georgetown_cupcake as unknown as MerchantConfig,
  'georgias-restaurant-pizza': georgias_restaurant_pizza as unknown as MerchantConfig,
  'georginas-restaurant': georginas_restaurant as unknown as MerchantConfig,
  'geronimo': geronimo as unknown as MerchantConfig,
  'ggs-wood-fired-pizza': ggs_wood_fired_pizza as unknown as MerchantConfig,
  'gibbet-hill-grill': gibbet_hill_grill as unknown as MerchantConfig,
  'giffords': giffords as unknown as MerchantConfig,
  'gigantic-brewing-company': gigantic_brewing_company as unknown as MerchantConfig,
  'gilson-cafe-cinema': gilson_cafe_cinema as unknown as MerchantConfig,
  'ginger-root': ginger_root as unknown as MerchantConfig,
  'ginos-pizza': ginos_pizza as unknown as MerchantConfig,
  'ginza-express': ginza_express as unknown as MerchantConfig,
  'giovannis-brick-oven-pizzaria': giovannis_brick_oven_pizzaria as unknown as MerchantConfig,
  'giv-coffee': giv_coffee as unknown as MerchantConfig,
  'glastonbury-coffee-shop': glastonbury_coffee_shop as unknown as MerchantConfig,
  'glastonbury-pizza-house': glastonbury_pizza_house as unknown as MerchantConfig,
  'gluten-free-oasis': gluten_free_oasis as unknown as MerchantConfig,
  'godfathers-pizza': godfathers_pizza as unknown as MerchantConfig,
  'golden-greek': golden_greek as unknown as MerchantConfig,
  'golden-lamb-buttery': golden_lamb_buttery as unknown as MerchantConfig,
  'golden-mango': golden_mango as unknown as MerchantConfig,
  'golden-palace': golden_palace as unknown as MerchantConfig,
  'goldenrod-pastries': goldenrod_pastries as unknown as MerchantConfig,
  'goldies-eatery': goldies_eatery as unknown as MerchantConfig,
  'golf-lounge-18': golf_lounge_18 as unknown as MerchantConfig,
  'gong-cha': gong_cha as unknown as MerchantConfig,
  'goofy-foot-tap-room': goofy_foot_tap_room as unknown as MerchantConfig,
  'gran-cafe-coffee-lounge': gran_cafe_coffee_lounge as unknown as MerchantConfig,
  'grand-cru': grand_cru as unknown as MerchantConfig,
  'grand-pizza': grand_pizza as unknown as MerchantConfig,
  'grandpas-diner': grandpas_diner as unknown as MerchantConfig,
  'grano-arso': grano_arso as unknown as MerchantConfig,
  'granola-bar': granola_bar as unknown as MerchantConfig,
  'grapevine': grapevine as unknown as MerchantConfig,
  'gratitude-cafe-bakery': gratitude_cafe_bakery as unknown as MerchantConfig,
  'gravity-well-brewing': gravity_well_brewing as unknown as MerchantConfig,
  'grays-inn': grays_inn as unknown as MerchantConfig,
  'great-harvest-bread-co': great_harvest_bread_co as unknown as MerchantConfig,
  'great-northern-hotel': great_northern_hotel as unknown as MerchantConfig,
  'great-tso': great_tso as unknown as MerchantConfig,
  'great-wall-chinese-food': great_wall_chinese_food as unknown as MerchantConfig,
  'green-door-restaurant': green_door_restaurant as unknown as MerchantConfig,
  'green-kettle-coffee': green_kettle_coffee as unknown as MerchantConfig,
  'green-olive-diner': green_olive_diner as unknown as MerchantConfig,
  'green-onion-pizza-restaurant': green_onion_pizza_restaurant as unknown as MerchantConfig,
  'green-tonic': green_tonic as unknown as MerchantConfig,
  'greggs': greggs as unknown as MerchantConfig,
  'grey-sail-taproom': grey_sail_taproom as unknown as MerchantConfig,
  'grigg-street': grigg_street as unknown as MerchantConfig,
  'grill-four-forty-five': grill_four_forty_five as unknown as MerchantConfig,
  'grindstone-tavern': grindstone_tavern as unknown as MerchantConfig,
  'griswold-coastal-forest-trail': griswold_coastal_forest_trail as unknown as MerchantConfig,
  'grk-grill': grk_grill as unknown as MerchantConfig,
  'groton-publick-house-pub-eatery': groton_publick_house_pub_eatery as unknown as MerchantConfig,
  'grounded-coffee-co': grounded_coffee_co as unknown as MerchantConfig,
  'gs-burgers': gs_burgers as unknown as MerchantConfig,
  'guildford-arms': guildford_arms as unknown as MerchantConfig,
  'gulianos-bakery': gulianos_bakery as unknown as MerchantConfig,
  'haagen-dazs': haagen_dazs as unknown as MerchantConfig,
  'hachiroku': hachiroku as unknown as MerchantConfig,
  'hacienda-villa': hacienda_villa as unknown as MerchantConfig,
  'hadlyme-country-market': hadlyme_country_market as unknown as MerchantConfig,
  'hammonesset-connector': hammonesset_connector as unknown as MerchantConfig,
  'hana-tokyo': hana_tokyo as unknown as MerchantConfig,
  'hanami-japanese-sushi-hibachi-steak-house': hanami_japanese_sushi_hibachi_steak_house as unknown as MerchantConfig,
  'hangry-goose': hangry_goose as unknown as MerchantConfig,
  'hanks-dairy-bar': hanks_dairy_bar as unknown as MerchantConfig,
  'hanks-restaurant': hanks_restaurant as unknown as MerchantConfig,
  'hanover-tap': hanover_tap as unknown as MerchantConfig,
  'happy-monkey': happy_monkey as unknown as MerchantConfig,
  'harborview-market': harborview_market as unknown as MerchantConfig,
  'hard-rock-cafe': hard_rock_cafe as unknown as MerchantConfig,
  'harlan-social': harlan_social as unknown as MerchantConfig,
  'harp-dragon-pub': harp_dragon_pub as unknown as MerchantConfig,
  'harpoon-brewery': harpoon_brewery as unknown as MerchantConfig,
  'harpoon-tap-room': harpoon_tap_room as unknown as MerchantConfig,
  'harpos-bakery-and-cafe': harpos_bakery_and_cafe as unknown as MerchantConfig,
  'harriet-beecher-stowe-center-little-free-library-charter-175887': harriet_beecher_stowe_center_little_free_library_charter_175887 as unknown as MerchantConfig,
  'harrys-pizzeria-tavern': harrys_pizzeria_tavern as unknown as MerchantConfig,
  'hartford-baking-co': hartford_baking_co as unknown as MerchantConfig,
  'hartford-road-cafe': hartford_road_cafe as unknown as MerchantConfig,
  'harvest-cafe-and-bakery': harvest_cafe_and_bakery as unknown as MerchantConfig,
  'harvest': harvest as unknown as MerchantConfig,
  'harwinton-pizza-restaurant': harwinton_pizza_restaurant as unknown as MerchantConfig,
  'hash-house-a-go-go': hash_house_a_go_go as unknown as MerchantConfig,
  'hata': hata as unknown as MerchantConfig,
  'havenly': havenly as unknown as MerchantConfig,
  'hawksmoor': hawksmoor as unknown as MerchantConfig,
  'heibecks-stand': heibecks_stand as unknown as MerchantConfig,
  'heights-pizza': heights_pizza as unknown as MerchantConfig,
  'heirloom-food-company': heirloom_food_company as unknown as MerchantConfig,
  'heirloom-market-at-comstock-ferre': heirloom_market_at_comstock_ferre as unknown as MerchantConfig,
  'hf-crave': hf_crave as unknown as MerchantConfig,
  'high-nine-brewing': high_nine_brewing as unknown as MerchantConfig,
  'hilltop-cafe': hilltop_cafe as unknown as MerchantConfig,
  'hinoki': hinoki as unknown as MerchantConfig,
  'hm-pizza-giant-grinders': hm_pizza_giant_grinders as unknown as MerchantConfig,
  'hog-river-brewing-company': hog_river_brewing_company as unknown as MerchantConfig,
  'holy-cow': holy_cow as unknown as MerchantConfig,
  'home-on-the-range': home_on_the_range as unknown as MerchantConfig,
  'honey-joes-family-coffeehouse': honey_joes_family_coffeehouse as unknown as MerchantConfig,
  'honeyspot-pizza': honeyspot_pizza as unknown as MerchantConfig,
  'honeysuckle-gelato': honeysuckle_gelato as unknown as MerchantConfig,
  'hong-kong-tokyo': hong_kong_tokyo as unknown as MerchantConfig,
  'hooters': hooters as unknown as MerchantConfig,
  'hop-culture-farms-brew-company': hop_culture_farms_brew_company as unknown as MerchantConfig,
  'hop-haus-berlin': hop_haus_berlin as unknown as MerchantConfig,
  'hop-valley-brewing': hop_valley_brewing as unknown as MerchantConfig,
  'hopfly-brewing-co': hopfly_brewing_co as unknown as MerchantConfig,
  'hops-on-the-hill': hops_on_the_hill as unknown as MerchantConfig,
  'hopsters-brewing-co': hopsters_brewing_co as unknown as MerchantConfig,
  'hornets-nest-deli': hornets_nest_deli as unknown as MerchantConfig,
  'hot-table': hot_table as unknown as MerchantConfig,
  'hotshots-sports-bar-and-grill': hotshots_sports_bar_and_grill as unknown as MerchantConfig,
  'howells': howells as unknown as MerchantConfig,
  'hteao': hteao as unknown as MerchantConfig,
  'hubbard-park-pizza': hubbard_park_pizza as unknown as MerchantConfig,
  'hudson-social': hudson_social as unknown as MerchantConfig,
  'hunan-noodle-bar': hunan_noodle_bar as unknown as MerchantConfig,
  'hungry-pot-korean-bbq-hot-pot': hungry_pot_korean_bbq_hot_pot as unknown as MerchantConfig,
  'hungry-pot': hungry_pot as unknown as MerchantConfig,
  'hushys-sandwiches': hushys_sandwiches as unknown as MerchantConfig,
  'huxleys-bookmark-cafe-family-restaurant': huxleys_bookmark_cafe_family_restaurant as unknown as MerchantConfig,
  'ichiban-express-hibachi-grill-and-poke-bowl': ichiban_express_hibachi_grill_and_poke_bowl as unknown as MerchantConfig,
  'ihop': ihop as unknown as MerchantConfig,
  'illianos-pizzeria-and-restaurant': illianos_pizzeria_and_restaurant as unknown as MerchantConfig,
  'illianos-ristorante-pizzeria': illianos_ristorante_pizzeria as unknown as MerchantConfig,
  'inferno-apizza': inferno_apizza as unknown as MerchantConfig,
  'irenes-family-restaurant': irenes_family_restaurant as unknown as MerchantConfig,
  'irmak-restaurant': irmak_restaurant as unknown as MerchantConfig,
  'iron-horse-pizzeria-and-sports-pub': iron_horse_pizzeria_and_sports_pub as unknown as MerchantConfig,
  'iron-horse-sports-pub': iron_horse_sports_pub as unknown as MerchantConfig,
  'iso-iso-ramen-boba': iso_iso_ramen_boba as unknown as MerchantConfig,
  'istanbul-gyro-kebab': istanbul_gyro_kebab as unknown as MerchantConfig,
  'italian-gourmet-deli-and-bakery': italian_gourmet_deli_and_bakery as unknown as MerchantConfig,
  'ivy-cafe': ivy_cafe as unknown as MerchantConfig,
  'izumi': izumi as unknown as MerchantConfig,
  'j-roos-restaurant': j_roos_restaurant as unknown as MerchantConfig,
  'j-timothys-taverne': j_timothys_taverne as unknown as MerchantConfig,
  'jack-smyth-brewing-company': jack_smyth_brewing_company as unknown as MerchantConfig,
  'jacks-brick-oven-pizza': jacks_brick_oven_pizza as unknown as MerchantConfig,
  'jade-lee-kitchen': jade_lee_kitchen as unknown as MerchantConfig,
  'jaho-coffee-roaster-wine-bar': jaho_coffee_roaster_wine_bar as unknown as MerchantConfig,
  'jazzys-cabaret': jazzys_cabaret as unknown as MerchantConfig,
  'jds-pizzeria-bar-grill': jds_pizzeria_bar_grill as unknown as MerchantConfig,
  'jesses-ice-cream': jesses_ice_cream as unknown as MerchantConfig,
  'jessicas-lobster-shack': jessicas_lobster_shack as unknown as MerchantConfig,
  'jessies-cafe': jessies_cafe as unknown as MerchantConfig,
  'jesters-coffee': jesters_coffee as unknown as MerchantConfig,
  'jewett-city-pizza-palace': jewett_city_pizza_palace as unknown as MerchantConfig,
  'jg-ross': jg_ross as unknown as MerchantConfig,
  'jia-mei-asian-kitchen': jia_mei_asian_kitchen as unknown as MerchantConfig,
  'jitters': jitters as unknown as MerchantConfig,
  'jjstacks': jjstacks as unknown as MerchantConfig,
  'joe-pizza': joe_pizza as unknown as MerchantConfig,
  'joeys-pizza-pie': joeys_pizza_pie as unknown as MerchantConfig,
  'john-marias-pizzeria-restaurant': john_marias_pizzeria_restaurant as unknown as MerchantConfig,
  'john-the-baker': john_the_baker as unknown as MerchantConfig,
  'johnny-rockets': johnny_rockets as unknown as MerchantConfig,
  'johnnys-clam-shack': johnnys_clam_shack as unknown as MerchantConfig,
  'johnnys-diner': johnnys_diner as unknown as MerchantConfig,
  'johns-cafe': johns_cafe as unknown as MerchantConfig,
  'jojos-breakfast-mexican-food': jojos_breakfast_mexican_food as unknown as MerchantConfig,
  'josephs-bagel-grill': josephs_bagel_grill as unknown as MerchantConfig,
  'josies-corner': josies_corner as unknown as MerchantConfig,
  'joy-bar-coffee-co': joy_bar_coffee_co as unknown as MerchantConfig,
  'jregos-gathering-place': jregos_gathering_place as unknown as MerchantConfig,
  'jubi-tea-sweets': jubi_tea_sweets as unknown as MerchantConfig,
  'jukebox-studios': jukebox_studios as unknown as MerchantConfig,
  'julianos-pizza-restaurant': julianos_pizza_restaurant as unknown as MerchantConfig,
  'juniors-coffee-shop': juniors_coffee_shop as unknown as MerchantConfig,
  'juno-bakery': juno_bakery as unknown as MerchantConfig,
  'kaliabon-ramen': kaliabon_ramen as unknown as MerchantConfig,
  'kaliubon-ramen': kaliubon_ramen as unknown as MerchantConfig,
  'karma': karma as unknown as MerchantConfig,
  'kat-su-restaurant': kat_su_restaurant as unknown as MerchantConfig,
  'katsute100': katsute100 as unknown as MerchantConfig,
  'keller': keller as unknown as MerchantConfig,
  'kennedy-fried-chicken': kennedy_fried_chicken as unknown as MerchantConfig,
  'kens-corner-breakfast-and-lunch': kens_corner_breakfast_and_lunch as unknown as MerchantConfig,
  'kent-falls-brewing-company': kent_falls_brewing_company as unknown as MerchantConfig,
  'kent-pizza-garden': kent_pizza_garden as unknown as MerchantConfig,
  'kenyon-st-little-free-library-charter-27991': kenyon_st_little_free_library_charter_27991 as unknown as MerchantConfig,
  'kg-corner-deli': kg_corner_deli as unknown as MerchantConfig,
  'kickback-lunch-lounge': kickback_lunch_lounge as unknown as MerchantConfig,
  'kimberlys-diner': kimberlys_diner as unknown as MerchantConfig,
  'king-donut': king_donut as unknown as MerchantConfig,
  'kings-court': kings_court as unknown as MerchantConfig,
  'kingston-kafe': kingston_kafe as unknown as MerchantConfig,
  'kira-sushi': kira_sushi as unknown as MerchantConfig,
  'kiwi-spoon': kiwi_spoon as unknown as MerchantConfig,
  'klekolo-world-coffee': klekolo_world_coffee as unknown as MerchantConfig,
  'knead-doughnuts': knead_doughnuts as unknown as MerchantConfig,
  'kokomos-restaurant-beach-bar': kokomos_restaurant_beach_bar as unknown as MerchantConfig,
  'koma-japanese-steakhouse': koma_japanese_steakhouse as unknown as MerchantConfig,
  'koon-thai': koon_thai as unknown as MerchantConfig,
  'kosciuszko-club': kosciuszko_club as unknown as MerchantConfig,
  'koto-japanese-steak-house-and-sushi-bar': koto_japanese_steak_house_and_sushi_bar as unknown as MerchantConfig,
  'kotobuki': kotobuki as unknown as MerchantConfig,
  'kumo': kumo as unknown as MerchantConfig,
  'kung-fu-tea': kung_fu_tea as unknown as MerchantConfig,
  'kuyi-sushi': kuyi_sushi as unknown as MerchantConfig,
  'la-cabana': la_cabana as unknown as MerchantConfig,
  'la-carnita-of-bantam': la_carnita_of_bantam as unknown as MerchantConfig,
  'la-casa-mania-mexican-restaurant-cantina': la_casa_mania_mexican_restaurant_cantina as unknown as MerchantConfig,
  'la-figata': la_figata as unknown as MerchantConfig,
  'la-flor-de-mexico': la_flor_de_mexico as unknown as MerchantConfig,
  'la-foresta': la_foresta as unknown as MerchantConfig,
  'la-fortuna-bar-restaurant': la_fortuna_bar_restaurant as unknown as MerchantConfig,
  'la-kabanita': la_kabanita as unknown as MerchantConfig,
  'la-mexicana': la_mexicana as unknown as MerchantConfig,
  'la-molienda': la_molienda as unknown as MerchantConfig,
  'la-piastra': la_piastra as unknown as MerchantConfig,
  'la-piazza': la_piazza as unknown as MerchantConfig,
  'la-poblanita': la_poblanita as unknown as MerchantConfig,
  'la-quartier-bakery-cafe': la_quartier_bakery_cafe as unknown as MerchantConfig,
  'la-taqueria': la_taqueria as unknown as MerchantConfig,
  'la-vita-gustosa': la_vita_gustosa as unknown as MerchantConfig,
  'lady-libertine': lady_libertine as unknown as MerchantConfig,
  'lagan': lagan as unknown as MerchantConfig,
  'lalibela': lalibela as unknown as MerchantConfig,
  'lamplighter-brewing-company': lamplighter_brewing_company as unknown as MerchantConfig,
  'las-vetas-lounge': las_vetas_lounge as unknown as MerchantConfig,
  'lasalle-market-deli': lasalle_market_deli as unknown as MerchantConfig,
  'lasowiak-polish-deli-baker-grocery': lasowiak_polish_deli_baker_grocery as unknown as MerchantConfig,
  'lasting-brass-brewing': lasting_brass_brewing as unknown as MerchantConfig,
  'laylas-falafel-greenleaf-cafe': laylas_falafel_greenleaf_cafe as unknown as MerchantConfig,
  'laylas-falafel': laylas_falafel as unknown as MerchantConfig,
  'lc-chens': lc_chens as unknown as MerchantConfig,
  'le-fat-poodle': le_fat_poodle as unknown as MerchantConfig,
  'le-gamin': le_gamin as unknown as MerchantConfig,
  'le-pain-quotidien': le_pain_quotidien as unknown as MerchantConfig,
  'le-petit-beefbar-edinburgh': le_petit_beefbar_edinburgh as unknown as MerchantConfig,
  'legends-pizzeria': legends_pizzeria as unknown as MerchantConfig,
  'lenas-pizzaria': lenas_pizzaria as unknown as MerchantConfig,
  'lennys-steakhouse': lennys_steakhouse as unknown as MerchantConfig,
  'les-dairy-bar': les_dairy_bar as unknown as MerchantConfig,
  'lescargot-bleu-wine-bar': lescargot_bleu_wine_bar as unknown as MerchantConfig,
  'lescargot-bleu': lescargot_bleu as unknown as MerchantConfig,
  'lianas-trottoria': lianas_trottoria as unknown as MerchantConfig,
  'lilac-house': lilac_house as unknown as MerchantConfig,
  'lime-rock-cafe': lime_rock_cafe as unknown as MerchantConfig,
  'linden-lounge': linden_lounge as unknown as MerchantConfig,
  'little-aladdin-mediterranean-grill': little_aladdin_mediterranean_grill as unknown as MerchantConfig,
  'little-barn': little_barn as unknown as MerchantConfig,
  'little-free-library': little_free_library as unknown as MerchantConfig,
  'little-house-brewing-co': little_house_brewing_co as unknown as MerchantConfig,
  'little-india': little_india as unknown as MerchantConfig,
  'little-mazen-pizza': little_mazen_pizza as unknown as MerchantConfig,
  'little-pub-greenwich': little_pub_greenwich as unknown as MerchantConfig,
  'livs-oyster-bar': livs_oyster_bar as unknown as MerchantConfig,
  'lobster-landing': lobster_landing as unknown as MerchantConfig,
  'local-63': local_63 as unknown as MerchantConfig,
  'locali': locali as unknown as MerchantConfig,
  'lorca': lorca as unknown as MerchantConfig,
  'los-sarapes-mexican-restaurant': los_sarapes_mexican_restaurant as unknown as MerchantConfig,
  'louies': louies as unknown as MerchantConfig,
  'loyal-order-of-moose': loyal_order_of_moose as unknown as MerchantConfig,
  'luanns-bakery-cafe': luanns_bakery_cafe as unknown as MerchantConfig,
  'lubranos-pizza': lubranos_pizza as unknown as MerchantConfig,
  'lucas-pizzeria': lucas_pizzeria as unknown as MerchantConfig,
  'luce': luce as unknown as MerchantConfig,
  'lucky-dragon-chinese-restaurant': lucky_dragon_chinese_restaurant as unknown as MerchantConfig,
  'lucky-lous': lucky_lous as unknown as MerchantConfig,
  'lucky-star': lucky_star as unknown as MerchantConfig,
  'lucky-yu': lucky_yu as unknown as MerchantConfig,
  'lugano-salumeria-wine-bar': lugano_salumeria_wine_bar as unknown as MerchantConfig,
  'luigis-bakery': luigis_bakery as unknown as MerchantConfig,
  'luigis-restaurant': luigis_restaurant as unknown as MerchantConfig,
  'lumleys-bakery': lumleys_bakery as unknown as MerchantConfig,
  'lupi-legna': lupi_legna as unknown as MerchantConfig,
  'lyme-grill': lyme_grill as unknown as MerchantConfig,
  'm-white-lounge': m_white_lounge as unknown as MerchantConfig,
  'mac-n-out': mac_n_out as unknown as MerchantConfig,
  'macros-healthy-restaurant': macros_healthy_restaurant as unknown as MerchantConfig,
  'made-by-pam': made_by_pam as unknown as MerchantConfig,
  'mademoiselle-macaron': mademoiselle_macaron as unknown as MerchantConfig,
  'madhouse-cafe': madhouse_cafe as unknown as MerchantConfig,
  'maggie-mcflys-brookfield': maggie_mcflys_brookfield as unknown as MerchantConfig,
  'maggie-mcflys': maggie_mcflys as unknown as MerchantConfig,
  'main-street-diner': main_street_diner as unknown as MerchantConfig,
  'main-street-restaurant-and-bar': main_street_restaurant_and_bar as unknown as MerchantConfig,
  'main-wah': main_wah as unknown as MerchantConfig,
  'maine-fish-market-restaurant': maine_fish_market_restaurant as unknown as MerchantConfig,
  'mainport-fish-chips': mainport_fish_chips as unknown as MerchantConfig,
  'maiones': maiones as unknown as MerchantConfig,
  'maison-mathis': maison_mathis as unknown as MerchantConfig,
  'maks-deli': maks_deli as unknown as MerchantConfig,
  'mama-carmelas-deli': mama_carmelas_deli as unknown as MerchantConfig,
  'mama-chow': mama_chow as unknown as MerchantConfig,
  'mamas-house-family-diner': mamas_house_family_diner as unknown as MerchantConfig,
  'mamas-kitchen': mamas_kitchen as unknown as MerchantConfig,
  'mamouns': mamouns as unknown as MerchantConfig,
  'manana-cafe': manana_cafe as unknown as MerchantConfig,
  'mangetsu-farmington': mangetsu_farmington as unknown as MerchantConfig,
  'mangos-wood-fired-pizza-co': mangos_wood_fired_pizza_co as unknown as MerchantConfig,
  'many-bennies': many_bennies as unknown as MerchantConfig,
  'marco-polo': marco_polo as unknown as MerchantConfig,
  'marcos-pizzeria-and-restaurant': marcos_pizzeria_and_restaurant as unknown as MerchantConfig,
  'marcos': marcos as unknown as MerchantConfig,
  'marina-indian-restaurant': marina_indian_restaurant as unknown as MerchantConfig,
  'marker-37': marker_37 as unknown as MerchantConfig,
  'market-place-tavern': market_place_tavern as unknown as MerchantConfig,
  'marlborough-pizza-restaurant': marlborough_pizza_restaurant as unknown as MerchantConfig,
  'marlborough-tavern': marlborough_tavern as unknown as MerchantConfig,
  'marmalade': marmalade as unknown as MerchantConfig,
  'martel-bistro-bar': martel_bistro_bar as unknown as MerchantConfig,
  'martys-cafe': martys_cafe as unknown as MerchantConfig,
  'marys-diner': marys_diner as unknown as MerchantConfig,
  'massimo-pizzeria-ristorante': massimo_pizzeria_ristorante as unknown as MerchantConfig,
  'max-a-mia': max_a_mia as unknown as MerchantConfig,
  'max-bibos': max_bibos as unknown as MerchantConfig,
  'max-burger': max_burger as unknown as MerchantConfig,
  'max-downtown': max_downtown as unknown as MerchantConfig,
  'max-fish': max_fish as unknown as MerchantConfig,
  'max-oyster-bar': max_oyster_bar as unknown as MerchantConfig,
  'mcalisters-deli': mcalisters_deli as unknown as MerchantConfig,
  'mcconneys-farm': mcconneys_farm as unknown as MerchantConfig,
  'meadow': meadow as unknown as MerchantConfig,
  'mecha-noodle-bar': mecha_noodle_bar as unknown as MerchantConfig,
  'mediterranea': mediterranea as unknown as MerchantConfig,
  'mediterraneo': mediterraneo as unknown as MerchantConfig,
  'melilli-pizzeria': melilli_pizzeria as unknown as MerchantConfig,
  'menchies': menchies as unknown as MerchantConfig,
  'menya-gumi': menya_gumi as unknown as MerchantConfig,
  'meraki': meraki as unknown as MerchantConfig,
  'mercurys-coffee-co': mercurys_coffee_co as unknown as MerchantConfig,
  'metro-bis': metro_bis as unknown as MerchantConfig,
  'mew-haven-cat-cafe': mew_haven_cat_cafe as unknown as MerchantConfig,
  'mex-on-main': mex_on_main as unknown as MerchantConfig,
  'mexicali-cantina-grill': mexicali_cantina_grill as unknown as MerchantConfig,
  'mexicue': mexicue as unknown as MerchantConfig,
  'mexipho': mexipho as unknown as MerchantConfig,
  'mezcal': mezcal as unknown as MerchantConfig,
  'mezzo-grille': mezzo_grille as unknown as MerchantConfig,
  'mi-pueblo': mi_pueblo as unknown as MerchantConfig,
  'mi-rancho-deli': mi_rancho_deli as unknown as MerchantConfig,
  'michael-jordans-steakhouse': michael_jordans_steakhouse as unknown as MerchantConfig,
  'michelinas-apizza-ristorante': michelinas_apizza_ristorante as unknown as MerchantConfig,
  'micro-creamery': micro_creamery as unknown as MerchantConfig,
  'middletown-halal-grill-and-pizza': middletown_halal_grill_and_pizza as unknown as MerchantConfig,
  'midway-pizza': midway_pizza as unknown as MerchantConfig,
  'midwest-coast-brewing': midwest_coast_brewing as unknown as MerchantConfig,
  'mighty-squirrel-taproom-kitchen': mighty_squirrel_taproom_kitchen as unknown as MerchantConfig,
  'mikes-pizza-diner': mikes_pizza_diner as unknown as MerchantConfig,
  'mikes-pizza-restaurant': mikes_pizza_restaurant as unknown as MerchantConfig,
  'miku': miku as unknown as MerchantConfig,
  'milestone': milestone as unknown as MerchantConfig,
  'milford-pizza-and-mexican-food': milford_pizza_and_mexican_food as unknown as MerchantConfig,
  'mill-plain-diner': mill_plain_diner as unknown as MerchantConfig,
  'millwrights': millwrights as unknown as MerchantConfig,
  'mimmo-play-cafe': mimmo_play_cafe as unknown as MerchantConfig,
  'ming-moon-restaurant': ming_moon_restaurant as unknown as MerchantConfig,
  'ming-moon': ming_moon as unknown as MerchantConfig,
  'mini-melts-usa': mini_melts_usa as unknown as MerchantConfig,
  'misfit-club': misfit_club as unknown as MerchantConfig,
  'misimi': misimi as unknown as MerchantConfig,
  'mitchells-on-main': mitchells_on_main as unknown as MerchantConfig,
  'mizzas-pizza': mizzas_pizza as unknown as MerchantConfig,
  'mj23-sports-bar-grill': mj23_sports_bar_grill as unknown as MerchantConfig,
  'mm-bistro': mm_bistro as unknown as MerchantConfig,
  'modern-apizza': modern_apizza as unknown as MerchantConfig,
  'modern-pastry-shop': modern_pastry_shop as unknown as MerchantConfig,
  'mofongo': mofongo as unknown as MerchantConfig,
  'moli': moli as unknown as MerchantConfig,
  'molly-moons-homemade-ice-cream': molly_moons_homemade_ice_cream as unknown as MerchantConfig,
  'mommys-delicious-food': mommys_delicious_food as unknown as MerchantConfig,
  'momosan': momosan as unknown as MerchantConfig,
  'mondo': mondo as unknown as MerchantConfig,
  'monkey-farm': monkey_farm as unknown as MerchantConfig,
  'monroe-diner': monroe_diner as unknown as MerchantConfig,
  'montys-river-grille': montys_river_grille as unknown as MerchantConfig,
  'moody-tongue-brewing-company': moody_tongue_brewing_company as unknown as MerchantConfig,
  'moon-rocks': moon_rocks as unknown as MerchantConfig,
  'mor-bakehouse': mor_bakehouse as unknown as MerchantConfig,
  'mountainside-cafe': mountainside_cafe as unknown as MerchantConfig,
  'mouse-proof-brewery': mouse_proof_brewery as unknown as MerchantConfig,
  'mr-falafel': mr_falafel as unknown as MerchantConfig,
  'mr-mac-canteen': mr_mac_canteen as unknown as MerchantConfig,
  'ms-cafe': ms_cafe as unknown as MerchantConfig,
  'muggswigz-coffee-tea': muggswigz_coffee_tea as unknown as MerchantConfig,
  'muscle-maker-grill': muscle_maker_grill as unknown as MerchantConfig,
  'muyri-indian-bistro': muyri_indian_bistro as unknown as MerchantConfig,
  'my-place-pizza-restaurant': my_place_pizza_restaurant as unknown as MerchantConfig,
  'mykonos-mediterranean-restaurant': mykonos_mediterranean_restaurant as unknown as MerchantConfig,
  'mystic-depot-roasters': mystic_depot_roasters as unknown as MerchantConfig,
  'mystic-pizza': mystic_pizza as unknown as MerchantConfig,
  'naija-restaurant': naija_restaurant as unknown as MerchantConfig,
  'nail-creek-pub-brewery': nail_creek_pub_brewery as unknown as MerchantConfig,
  'nanas-mystic': nanas_mystic as unknown as MerchantConfig,
  'nanny-obriens-irish-pub': nanny_obriens_irish_pub as unknown as MerchantConfig,
  'naples-pizza': naples_pizza as unknown as MerchantConfig,
  'nardellis': nardellis as unknown as MerchantConfig,
  'neat': neat as unknown as MerchantConfig,
  'nectar': nectar as unknown as MerchantConfig,
  'new-china-wok': new_china_wok as unknown as MerchantConfig,
  'new-england-brewing-company': new_england_brewing_company as unknown as MerchantConfig,
  'new-harvest': new_harvest as unknown as MerchantConfig,
  'new-haven-taste-of-china': new_haven_taste_of_china as unknown as MerchantConfig,
  'new-mill-restaurant': new_mill_restaurant as unknown as MerchantConfig,
  'new-star---milford-chinese': new_star___milford_chinese as unknown as MerchantConfig,
  'new-york-pizza-company': new_york_pizza_company as unknown as MerchantConfig,
  'newbury-gelato': newbury_gelato as unknown as MerchantConfig,
  'newfield-restaurant-pizza': newfield_restaurant_pizza as unknown as MerchantConfig,
  'newsroom': newsroom as unknown as MerchantConfig,
  'newtown-road-italian-deli-pizza': newtown_road_italian_deli_pizza as unknown as MerchantConfig,
  'nicks-char-pit': nicks_char_pit as unknown as MerchantConfig,
  'nicks-diner': nicks_diner as unknown as MerchantConfig,
  'nicks-pizza-restaurant': nicks_pizza_restaurant as unknown as MerchantConfig,
  'nightcap': nightcap as unknown as MerchantConfig,
  'nikkis-dog-house': nikkis_dog_house as unknown as MerchantConfig,
  'ninety-nine-restaurant-pub': ninety_nine_restaurant_pub as unknown as MerchantConfig,
  'ninety-nine-restaurant': ninety_nine_restaurant as unknown as MerchantConfig,
  'ninkasi-tasting-room': ninkasi_tasting_room as unknown as MerchantConfig,
  'ninos-pizzeria': ninos_pizzeria as unknown as MerchantConfig,
  'no-coast-taqueria': no_coast_taqueria as unknown as MerchantConfig,
  'no-mames-mexican-cantina': no_mames_mexican_cantina as unknown as MerchantConfig,
  'no1-restaurant-best-chinese-food-take-out': no1_restaurant_best_chinese_food_take_out as unknown as MerchantConfig,
  'noisette-pastry-kitchen': noisette_pastry_kitchen as unknown as MerchantConfig,
  'noodles-company': noodles_company as unknown as MerchantConfig,
  'norbrook-farm-brewery': norbrook_farm_brewery as unknown as MerchantConfig,
  'nori': nori as unknown as MerchantConfig,
  'nostrano-italian-eatery': nostrano_italian_eatery as unknown as MerchantConfig,
  'nouveau-monde---wine-bar-and-restaurant': nouveau_monde___wine_bar_and_restaurant as unknown as MerchantConfig,
  'nutty-squirrel-gelato': nutty_squirrel_gelato as unknown as MerchantConfig,
  'nv-bakery-and-market': nv_bakery_and_market as unknown as MerchantConfig,
  'nyam-bakery': nyam_bakery as unknown as MerchantConfig,
  'oak-and-iron-brewing-co': oak_and_iron_brewing_co as unknown as MerchantConfig,
  'oakland-pizza-co': oakland_pizza_co as unknown as MerchantConfig,
  'oakland-united-beerworks': oakland_united_beerworks as unknown as MerchantConfig,
  'oakleaf-cakes': oakleaf_cakes as unknown as MerchantConfig,
  'oakshire-brewing': oakshire_brewing as unknown as MerchantConfig,
  'oaxaca-taqueria-and-pizza': oaxaca_taqueria_and_pizza as unknown as MerchantConfig,
  'oc-2-go': oc_2_go as unknown as MerchantConfig,
  'ocha-thai-japanese-cuisine': ocha_thai_japanese_cuisine as unknown as MerchantConfig,
  'ocho-cafe': ocho_cafe as unknown as MerchantConfig,
  'odeens-bbq-ridgefield-golf-course': odeens_bbq_ridgefield_golf_course as unknown as MerchantConfig,
  'oec-brewing': oec_brewing as unknown as MerchantConfig,
  'off-the-hook-bar-grill': off_the_hook_bar_grill as unknown as MerchantConfig,
  'oishi': oishi as unknown as MerchantConfig,
  'ok-diner': ok_diner as unknown as MerchantConfig,
  'oko': oko as unknown as MerchantConfig,
  'old-greenwich-deli': old_greenwich_deli as unknown as MerchantConfig,
  'old-salt-fish-and-bagels': old_salt_fish_and_bagels as unknown as MerchantConfig,
  'old-town-cafe': old_town_cafe as unknown as MerchantConfig,
  'old-towne-resturant': old_towne_resturant as unknown as MerchantConfig,
  'old-tymes-restaurant': old_tymes_restaurant as unknown as MerchantConfig,
  'old-well-tavern': old_well_tavern as unknown as MerchantConfig,
  'olde-mother-brewing': olde_mother_brewing as unknown as MerchantConfig,
  'olees-pizza-farmington': olees_pizza_farmington as unknown as MerchantConfig,
  'olio-restaurant-bar': olio_restaurant_bar as unknown as MerchantConfig,
  'olive-garden': olive_garden as unknown as MerchantConfig,
  'olives-and-oil': olives_and_oil as unknown as MerchantConfig,
  'ollies-pizza': ollies_pizza as unknown as MerchantConfig,
  'olympic-family-restaurant': olympic_family_restaurant as unknown as MerchantConfig,
  'olympic-pizza-restaurant': olympic_pizza_restaurant as unknown as MerchantConfig,
  'one-way-brewing': one_way_brewing as unknown as MerchantConfig,
  'one-world-roasters': one_world_roasters as unknown as MerchantConfig,
  'ordinary': ordinary as unknown as MerchantConfig,
  'oregon-brewlab': oregon_brewlab as unknown as MerchantConfig,
  'orems-diner': orems_diner as unknown as MerchantConfig,
  'oronoque-farms-gift-bakery': oronoque_farms_gift_bakery as unknown as MerchantConfig,
  'osa-restaurant-bar': osa_restaurant_bar as unknown as MerchantConfig,
  'otto': otto as unknown as MerchantConfig,
  'outback-steakhouse': outback_steakhouse as unknown as MerchantConfig,
  'owl-shop': owl_shop as unknown as MerchantConfig,
  'oxford-baking-company': oxford_baking_company as unknown as MerchantConfig,
  'oyster-club': oyster_club as unknown as MerchantConfig,
  'paci-restaurant': paci_restaurant as unknown as MerchantConfig,
  'pad-thai': pad_thai as unknown as MerchantConfig,
  'palma': palma as unknown as MerchantConfig,
  'panaros-pizzeria': panaros_pizzeria as unknown as MerchantConfig,
  'papa-ginos': papa_ginos as unknown as MerchantConfig,
  'papa-joes-ristorante-italiano': papa_joes_ristorante_italiano as unknown as MerchantConfig,
  'papa-zs-restaurant-and-pizza': papa_zs_restaurant_and_pizza as unknown as MerchantConfig,
  'papas-pizza-and-roast-beef': papas_pizza_and_roast_beef as unknown as MerchantConfig,
  'papii': papii as unknown as MerchantConfig,
  'par-four-restaurant': par_four_restaurant as unknown as MerchantConfig,
  'paramount-pizza': paramount_pizza as unknown as MerchantConfig,
  'parker-eatery-bar': parker_eatery_bar as unknown as MerchantConfig,
  'party-pizza': party_pizza as unknown as MerchantConfig,
  'pasta-bowls': pasta_bowls as unknown as MerchantConfig,
  'pasta-heaven': pasta_heaven as unknown as MerchantConfig,
  'paul-gregories-bistro': paul_gregories_bistro as unknown as MerchantConfig,
  'pauls-pizza': pauls_pizza as unknown as MerchantConfig,
  'pavement-coffeehouse': pavement_coffeehouse as unknown as MerchantConfig,
  'peachwave': peachwave as unknown as MerchantConfig,
  'peacock-chinese-restaurant': peacock_chinese_restaurant as unknown as MerchantConfig,
  'pearl-provisions-tipples': pearl_provisions_tipples as unknown as MerchantConfig,
  'peking-tokyo': peking_tokyo as unknown as MerchantConfig,
  'penny-black': penny_black as unknown as MerchantConfig,
  'pepper-pot': pepper_pot as unknown as MerchantConfig,
  'peppermill': peppermill as unknown as MerchantConfig,
  'peppers-jamaican-bakery': peppers_jamaican_bakery as unknown as MerchantConfig,
  'perfect-fried-chicken': perfect_fried_chicken as unknown as MerchantConfig,
  'perfect-poundcakes': perfect_poundcakes as unknown as MerchantConfig,
  'perfusion-espresso-and-cocktail-bar': perfusion_espresso_and_cocktail_bar as unknown as MerchantConfig,
  'perk-on-main': perk_on_main as unknown as MerchantConfig,
  'perks-corks': perks_corks as unknown as MerchantConfig,
  'perrot-memorial-library': perrot_memorial_library as unknown as MerchantConfig,
  'phantasia-at-sunrise-square': phantasia_at_sunrise_square as unknown as MerchantConfig,
  'pho-170': pho_170 as unknown as MerchantConfig,
  'pho-501': pho_501 as unknown as MerchantConfig,
  'pho-ethan': pho_ethan as unknown as MerchantConfig,
  'pho-house-llc': pho_house_llc as unknown as MerchantConfig,
  'pho-vietnam': pho_vietnam as unknown as MerchantConfig,
  'pick-and-mix': pick_and_mix as unknown as MerchantConfig,
  'pig-rig-bbq': pig_rig_bbq as unknown as MerchantConfig,
  'pigs-eye-pub': pigs_eye_pub as unknown as MerchantConfig,
  'pilot': pilot as unknown as MerchantConfig,
  'pine-lodge': pine_lodge as unknown as MerchantConfig,
  'pinkberry': pinkberry as unknown as MerchantConfig,
  'pinots-palette-stamford': pinots_palette_stamford as unknown as MerchantConfig,
  'pitaziki': pitaziki as unknown as MerchantConfig,
  'pixel-bakery-flat': pixel_bakery_flat as unknown as MerchantConfig,
  'pizza-express': pizza_express as unknown as MerchantConfig,
  'pizza-mediterranean': pizza_mediterranean as unknown as MerchantConfig,
  'pizza-pilgrims': pizza_pilgrims as unknown as MerchantConfig,
  'pizza-pizzazz': pizza_pizzazz as unknown as MerchantConfig,
  'pizza-post': pizza_post as unknown as MerchantConfig,
  'pizza-station': pizza_station as unknown as MerchantConfig,
  'pizza-time': pizza_time as unknown as MerchantConfig,
  'pizza-works': pizza_works as unknown as MerchantConfig,
  'plan-b-burger-bar': plan_b_burger_bar as unknown as MerchantConfig,
  'plan-b': plan_b as unknown as MerchantConfig,
  'planet-pizza': planet_pizza as unknown as MerchantConfig,
  'plaskos-farm': plaskos_farm as unknown as MerchantConfig,
  'plaza-restaurant': plaza_restaurant as unknown as MerchantConfig,
  'pokemoto': pokemoto as unknown as MerchantConfig,
  'pokeworks': pokeworks as unknown as MerchantConfig,
  'pond-house-cafe': pond_house_cafe as unknown as MerchantConfig,
  'pontos-taverna': pontos_taverna as unknown as MerchantConfig,
  'ponza-italian-kitchen-pizza': ponza_italian_kitchen_pizza as unknown as MerchantConfig,
  'popovers': popovers as unknown as MerchantConfig,
  'pops-diner': pops_diner as unknown as MerchantConfig,
  'porter-house-burger-co': porter_house_burger_co as unknown as MerchantConfig,
  'portico-brewing': portico_brewing as unknown as MerchantConfig,
  'pos-rice-and-spice': pos_rice_and_spice as unknown as MerchantConfig,
  'posa-ristorante-vineria': posa_ristorante_vineria as unknown as MerchantConfig,
  'post-corner-pizza': post_corner_pizza as unknown as MerchantConfig,
  'potions-and-rations': potions_and_rations as unknown as MerchantConfig,
  'powder-room': powder_room as unknown as MerchantConfig,
  'prairie-schooner-bar-grill': prairie_schooner_bar_grill as unknown as MerchantConfig,
  'pralines-ice-cream': pralines_ice_cream as unknown as MerchantConfig,
  'present-company': present_company as unknown as MerchantConfig,
  'press-burger': press_burger as unknown as MerchantConfig,
  'prime-on-whitfield': prime_on_whitfield as unknown as MerchantConfig,
  'prime': prime as unknown as MerchantConfig,
  'priory-brewing-company': priory_brewing_company as unknown as MerchantConfig,
  'private-cafeteria': private_cafeteria as unknown as MerchantConfig,
  'project-canele': project_canele as unknown as MerchantConfig,
  'pronto': pronto as unknown as MerchantConfig,
  'proof-coffe-roasters': proof_coffe_roasters as unknown as MerchantConfig,
  'propeller-cafe': propeller_cafe as unknown as MerchantConfig,
  'puente-american-latino-pub': puente_american_latino_pub as unknown as MerchantConfig,
  'puerto-vallarta': puerto_vallarta as unknown as MerchantConfig,
  'pure-all-natural-juice-bar': pure_all_natural_juice_bar as unknown as MerchantConfig,
  'puritan-tavern': puritan_tavern as unknown as MerchantConfig,
  'putnam-restaurant': putnam_restaurant as unknown as MerchantConfig,
  'quaker-diner': quaker_diner as unknown as MerchantConfig,
  'quarry-coffee-co': quarry_coffee_co as unknown as MerchantConfig,
  'quartiere': quartiere as unknown as MerchantConfig,
  'queen-b-coffee-company': queen_b_coffee_company as unknown as MerchantConfig,
  'raineys-best': raineys_best as unknown as MerchantConfig,
  'ralph-n-richs': ralph_n_richs as unknown as MerchantConfig,
  'raon-jena-coffee-and-dessert': raon_jena_coffee_and_dessert as unknown as MerchantConfig,
  'rascals-brewing-company': rascals_brewing_company as unknown as MerchantConfig,
  'ray-kellys-pub': ray_kellys_pub as unknown as MerchantConfig,
  'rebel-dog-coffee-co': rebel_dog_coffee_co as unknown as MerchantConfig,
  'rebel-dog-coffee-company': rebel_dog_coffee_company as unknown as MerchantConfig,
  'rebel-dog-coffee': rebel_dog_coffee as unknown as MerchantConfig,
  'red-36-seafood': red_36_seafood as unknown as MerchantConfig,
  'red-apple': red_apple as unknown as MerchantConfig,
  'red-barn': red_barn as unknown as MerchantConfig,
  'red-fox-amore-restaurant': red_fox_amore_restaurant as unknown as MerchantConfig,
  'red-lobster': red_lobster as unknown as MerchantConfig,
  'red-lotus': red_lotus as unknown as MerchantConfig,
  'red-robin': red_robin as unknown as MerchantConfig,
  'red-rooster-deli-catering': red_rooster_deli_catering as unknown as MerchantConfig,
  'red-rooster-pub': red_rooster_pub as unknown as MerchantConfig,
  'red-rooster': red_rooster as unknown as MerchantConfig,
  'red-stone-pub': red_stone_pub as unknown as MerchantConfig,
  'redemption-brewing-co': redemption_brewing_co as unknown as MerchantConfig,
  'reds-cafe': reds_cafe as unknown as MerchantConfig,
  'remnant-brewing-company': remnant_brewing_company as unknown as MerchantConfig,
  'remos': remos as unknown as MerchantConfig,
  'reverends-restaurant': reverends_restaurant as unknown as MerchantConfig,
  'revolutions-sand-patio-bar': revolutions_sand_patio_bar as unknown as MerchantConfig,
  'ricotta-pizza-bakery': ricotta_pizza_bakery as unknown as MerchantConfig,
  'rikos-pizza': rikos_pizza as unknown as MerchantConfig,
  'rinconcito-salvadoreno': rinconcito_salvadoreno as unknown as MerchantConfig,
  'rio-bravo': rio_bravo as unknown as MerchantConfig,
  'ritas-italian-ice': ritas_italian_ice as unknown as MerchantConfig,
  'river-pub': river_pub as unknown as MerchantConfig,
  'river-rock-tavern': river_rock_tavern as unknown as MerchantConfig,
  'riverdale-diner': riverdale_diner as unknown as MerchantConfig,
  'rivers-edge-pub-and-grille': rivers_edge_pub_and_grille as unknown as MerchantConfig,
  'riverside-pizza': riverside_pizza as unknown as MerchantConfig,
  'rizzutos-oyster-bar-restaurant': rizzutos_oyster_bar_restaurant as unknown as MerchantConfig,
  'roadhouse-billiards-and-brew': roadhouse_billiards_and_brew as unknown as MerchantConfig,
  'roasted': roasted as unknown as MerchantConfig,
  'robertos-log-cabin': robertos_log_cabin as unknown as MerchantConfig,
  'robertos-restaurant': robertos_restaurant as unknown as MerchantConfig,
  'roddys-pizza-and-salad': roddys_pizza_and_salad as unknown as MerchantConfig,
  'roly-poly-bakery': roly_poly_bakery as unknown as MerchantConfig,
  'roma-sports-bar-grill': roma_sports_bar_grill as unknown as MerchantConfig,
  'rooftop120': rooftop120 as unknown as MerchantConfig,
  'roseland-apizza': roseland_apizza as unknown as MerchantConfig,
  'rossittos-ristorante': rossittos_ristorante as unknown as MerchantConfig,
  'rosy-tomorrows': rosy_tomorrows as unknown as MerchantConfig,
  'roti-boti': roti_boti as unknown as MerchantConfig,
  'roti-king': roti_king as unknown as MerchantConfig,
  'royal-buffet': royal_buffet as unknown as MerchantConfig,
  'royal-house-thai-cuisine': royal_house_thai_cuisine as unknown as MerchantConfig,
  'rozzis-eatery': rozzis_eatery as unknown as MerchantConfig,
  'ruby-tuesday': ruby_tuesday as unknown as MerchantConfig,
  'ruse-brewery-taproom': ruse_brewery_taproom as unknown as MerchantConfig,
  'rusty-rail-tavern': rusty_rail_tavern as unknown as MerchantConfig,
  'saint-bess-jerk-hub': saint_bess_jerk_hub as unknown as MerchantConfig,
  'saints': saints as unknown as MerchantConfig,
  'sake-cafe': sake_cafe as unknown as MerchantConfig,
  'sakura-garden-japanese-steak-house': sakura_garden_japanese_steak_house as unknown as MerchantConfig,
  'salem-valley-farms-ice-cream': salem_valley_farms_ice_cream as unknown as MerchantConfig,
  'salernos-apizza': salernos_apizza as unknown as MerchantConfig,
  'sallys-apizza': sallys_apizza as unknown as MerchantConfig,
  'salt-and-light': salt_and_light as unknown as MerchantConfig,
  'salt-straw': salt_straw as unknown as MerchantConfig,
  'salute': salute as unknown as MerchantConfig,
  'sam-adams-boston-brewery': sam_adams_boston_brewery as unknown as MerchantConfig,
  'sammamish-cafe-spirits': sammamish_cafe_spirits as unknown as MerchantConfig,
  'sammy-gs-tavern': sammy_gs_tavern as unknown as MerchantConfig,
  'san-souci-restaurant-lounge': san_souci_restaurant_lounge as unknown as MerchantConfig,
  'sandpiper': sandpiper as unknown as MerchantConfig,
  'santa-maria': santa_maria as unknown as MerchantConfig,
  'sarahs-breakfast-lunch': sarahs_breakfast_lunch as unknown as MerchantConfig,
  'saravanaa-bhavan': saravanaa_bhavan as unknown as MerchantConfig,
  'sassos-coal-fired-pizza': sassos_coal_fired_pizza as unknown as MerchantConfig,
  'sassy-mama': sassy_mama as unknown as MerchantConfig,
  'saugatuck-sweets-fairfield': saugatuck_sweets_fairfield as unknown as MerchantConfig,
  'saybrook-fish-house': saybrook_fish_house as unknown as MerchantConfig,
  'sbc-brewery-restaurant': sbc_brewery_restaurant as unknown as MerchantConfig,
  'scooters-coffee-shop': scooters_coffee_shop as unknown as MerchantConfig,
  'scooters-coffee': scooters_coffee as unknown as MerchantConfig,
  'scotts-jamaican-bakery': scotts_jamaican_bakery as unknown as MerchantConfig,
  'seed-kitchen-and-bagelry': seed_kitchen_and_bagelry as unknown as MerchantConfig,
  'sellfish-restaurant': sellfish_restaurant as unknown as MerchantConfig,
  'serenata-restaurant': serenata_restaurant as unknown as MerchantConfig,
  'seven-moons-thai-cuisine': seven_moons_thai_cuisine as unknown as MerchantConfig,
  'seven-star-chinese-food-take-out': seven_star_chinese_food_take_out as unknown as MerchantConfig,
  'shandals-vegetarian-cafe': shandals_vegetarian_cafe as unknown as MerchantConfig,
  'shangri-la-hot-woks-sushi-bar': shangri_la_hot_woks_sushi_bar as unknown as MerchantConfig,
  'shapeshifters-brewery': shapeshifters_brewery as unknown as MerchantConfig,
  'shayna-bs-by-the-sea': shayna_bs_by_the_sea as unknown as MerchantConfig,
  'sherwood-diner': sherwood_diner as unknown as MerchantConfig,
  'shiki-hana-japanese-bistro': shiki_hana_japanese_bistro as unknown as MerchantConfig,
  'shinjuku-station-revolving-sushi-bar': shinjuku_station_revolving_sushi_bar as unknown as MerchantConfig,
  'shipwreck-tavern': shipwreck_tavern as unknown as MerchantConfig,
  'shortts-farm-ice-cream': shortts_farm_ice_cream as unknown as MerchantConfig,
  'shukran': shukran as unknown as MerchantConfig,
  'shumi-revolving-sushi': shumi_revolving_sushi as unknown as MerchantConfig,
  'siam-glastonbury': siam_glastonbury as unknown as MerchantConfig,
  'sicily-coal-fired-pizza': sicily_coal_fired_pizza as unknown as MerchantConfig,
  'sign-of-the-whale': sign_of_the_whale as unknown as MerchantConfig,
  'silk-city-coffee': silk_city_coffee as unknown as MerchantConfig,
  'silver-sands-pizza': silver_sands_pizza as unknown as MerchantConfig,
  'simply-bowls-co': simply_bowls_co as unknown as MerchantConfig,
  'sip-wine-bar': sip_wine_bar as unknown as MerchantConfig,
  'sisson-ave-pizza': sisson_ave_pizza as unknown as MerchantConfig,
  'skeleton-key-brewery': skeleton_key_brewery as unknown as MerchantConfig,
  'skyline-restaurant': skyline_restaurant as unknown as MerchantConfig,
  'sliceworks': sliceworks as unknown as MerchantConfig,
  'sliders-cafe': sliders_cafe as unknown as MerchantConfig,
  'sliders-sports-bar-grill': sliders_sports_bar_grill as unknown as MerchantConfig,
  'sloe-lane-bakery': sloe_lane_bakery as unknown as MerchantConfig,
  'sneekers-cafe': sneekers_cafe as unknown as MerchantConfig,
  'snowy-village': snowy_village as unknown as MerchantConfig,
  'social-club': social_club as unknown as MerchantConfig,
  'sofias-restaurant-and-pizzeria': sofias_restaurant_and_pizzeria as unknown as MerchantConfig,
  'solun-bartapas': solun_bartapas as unknown as MerchantConfig,
  'soma-grille': soma_grille as unknown as MerchantConfig,
  'somos': somos as unknown as MerchantConfig,
  'sophias-breakfast-and-lunch': sophias_breakfast_and_lunch as unknown as MerchantConfig,
  'sophies': sophies as unknown as MerchantConfig,
  'sorrento-importing': sorrento_importing as unknown as MerchantConfig,
  'sound-coffee': sound_coffee as unknown as MerchantConfig,
  'south-highland-street-little-library-charter-32424': south_highland_street_little_library_charter_32424 as unknown as MerchantConfig,
  'southwest-cafe': southwest_cafe as unknown as MerchantConfig,
  'spice-venue': spice_venue as unknown as MerchantConfig,
  'spicy-green-bean': spicy_green_bean as unknown as MerchantConfig,
  'spring-garden-bakery-cafe': spring_garden_bakery_cafe as unknown as MerchantConfig,
  'spruce-coffee': spruce_coffee as unknown as MerchantConfig,
  'spruce-street-little-free-library-charter-150603': spruce_street_little_free_library_charter_150603 as unknown as MerchantConfig,
  'square-peg': square_peg as unknown as MerchantConfig,
  'stamford-pizza': stamford_pizza as unknown as MerchantConfig,
  'stamford-repairs': stamford_repairs as unknown as MerchantConfig,
  'stamford-salads': stamford_salads as unknown as MerchantConfig,
  'stans-jamaican-kitchen': stans_jamaican_kitchen as unknown as MerchantConfig,
  'star-pizza': star_pizza as unknown as MerchantConfig,
  'state-line-pizza': state_line_pizza as unknown as MerchantConfig,
  'steak-loft-of-mystic': steak_loft_of_mystic as unknown as MerchantConfig,
  'steam': steam as unknown as MerchantConfig,
  'stella-blues': stella_blues as unknown as MerchantConfig,
  'steves-place': steves_place as unknown as MerchantConfig,
  'stewards-of-the-land-brewery': stewards_of_the_land_brewery as unknown as MerchantConfig,
  'stone-barrel-brewing-company': stone_barrel_brewing_company as unknown as MerchantConfig,
  'stor-coffee': stor_coffee as unknown as MerchantConfig,
  'strange-brew-pub': strange_brew_pub as unknown as MerchantConfig,
  'streibels-espresso': streibels_espresso as unknown as MerchantConfig,
  'strykers-cafe': strykers_cafe as unknown as MerchantConfig,
  'submarine-veterans-club-inc-subvets': submarine_veterans_club_inc_subvets as unknown as MerchantConfig,
  'suffield-pizza': suffield_pizza as unknown as MerchantConfig,
  'sugar-daddys': sugar_daddys as unknown as MerchantConfig,
  'sun-patio-bar': sun_patio_bar as unknown as MerchantConfig,
  'sun-splash-bar-and-grill': sun_splash_bar_and_grill as unknown as MerchantConfig,
  'sunberry': sunberry as unknown as MerchantConfig,
  'sundown-saloon': sundown_saloon as unknown as MerchantConfig,
  'sunny-daes': sunny_daes as unknown as MerchantConfig,
  'sunset-bar-grill': sunset_bar_grill as unknown as MerchantConfig,
  'super-taqueria': super_taqueria as unknown as MerchantConfig,
  'supreme-pizza': supreme_pizza as unknown as MerchantConfig,
  'surf-cantina': surf_cantina as unknown as MerchantConfig,
  'sushi-koya': sushi_koya as unknown as MerchantConfig,
  'sushi-on-chapel': sushi_on_chapel as unknown as MerchantConfig,
  'sushi-soba': sushi_soba as unknown as MerchantConfig,
  'svea-club': svea_club as unknown as MerchantConfig,
  'sweet-alchemy-ice-creamery': sweet_alchemy_ice_creamery as unknown as MerchantConfig,
  'sweet-aroma-coffehouse-bakery': sweet_aroma_coffehouse_bakery as unknown as MerchantConfig,
  'sweet-claudes': sweet_claudes as unknown as MerchantConfig,
  'sweet-harmony-cafe-bakery': sweet_harmony_cafe_bakery as unknown as MerchantConfig,
  'sweet-mango---sushi-and-bistro': sweet_mango___sushi_and_bistro as unknown as MerchantConfig,
  'sweet-me-bakery': sweet_me_bakery as unknown as MerchantConfig,
  'sweet-scoops': sweet_scoops as unknown as MerchantConfig,
  't-bone-prime-steakhouse': t_bone_prime_steakhouse as unknown as MerchantConfig,
  't-swirl-crepe': t_swirl_crepe as unknown as MerchantConfig,
  'table-570': table_570 as unknown as MerchantConfig,
  'tabouli-and-more': tabouli_and_more as unknown as MerchantConfig,
  'taco-dia': taco_dia as unknown as MerchantConfig,
  'tacos-la-rosa': tacos_la_rosa as unknown as MerchantConfig,
  'tai-jiang': tai_jiang as unknown as MerchantConfig,
  'tailor-upholstery-co-inc': tailor_upholstery_co_inc as unknown as MerchantConfig,
  'taj': taj as unknown as MerchantConfig,
  'tan-wong': tan_wong as unknown as MerchantConfig,
  'tandoor-indian-restaurant': tandoor_indian_restaurant as unknown as MerchantConfig,
  'tandoori-taste-of-india': tandoori_taste_of_india as unknown as MerchantConfig,
  'tanoor-restaurant': tanoor_restaurant as unknown as MerchantConfig,
  'tapas': tapas as unknown as MerchantConfig,
  'taqueria-cinco': taqueria_cinco as unknown as MerchantConfig,
  'taqueria-cordoba': taqueria_cordoba as unknown as MerchantConfig,
  'taqueria-la-frontera': taqueria_la_frontera as unknown as MerchantConfig,
  'taqueria-la-michoacana': taqueria_la_michoacana as unknown as MerchantConfig,
  'taqueria-la-ventana': taqueria_la_ventana as unknown as MerchantConfig,
  'taquerio': taquerio as unknown as MerchantConfig,
  'tarantino-restaurant-bar': tarantino_restaurant_bar as unknown as MerchantConfig,
  'taste-of-thai': taste_of_thai as unknown as MerchantConfig,
  'tavern-in-the-square': tavern_in_the_square as unknown as MerchantConfig,
  'tavern-on-3': tavern_on_3 as unknown as MerchantConfig,
  'taverna-grill': taverna_grill as unknown as MerchantConfig,
  'tazza-cafe': tazza_cafe as unknown as MerchantConfig,
  'tazza-osteria-bar': tazza_osteria_bar as unknown as MerchantConfig,
  'tcby': tcby as unknown as MerchantConfig,
  'teds-restaurant-bar': teds_restaurant_bar as unknown as MerchantConfig,
  'teds-restaurant-steamed-cheeseburgers': teds_restaurant_steamed_cheeseburgers as unknown as MerchantConfig,
  'temescal-brewing': temescal_brewing as unknown as MerchantConfig,
  'ten-asian-bistro': ten_asian_bistro as unknown as MerchantConfig,
  'ten-one-tea-house': ten_one_tea_house as unknown as MerchantConfig,
  'tenth-hole-tavern': tenth_hole_tavern as unknown as MerchantConfig,
  'tequila-escape': tequila_escape as unknown as MerchantConfig,
  'terra-ristorante-italiano': terra_ristorante_italiano as unknown as MerchantConfig,
  'texas-de-brazil': texas_de_brazil as unknown as MerchantConfig,
  'thai-basil': thai_basil as unknown as MerchantConfig,
  'thai-palace': thai_palace as unknown as MerchantConfig,
  'thai-stories': thai_stories as unknown as MerchantConfig,
  'thai-time': thai_time as unknown as MerchantConfig,
  'thai-trio': thai_trio as unknown as MerchantConfig,
  'thamel': thamel as unknown as MerchantConfig,
  'the-202-tavern': the_202_tavern as unknown as MerchantConfig,
  'the-alley-scoop': the_alley_scoop as unknown as MerchantConfig,
  'the-angry-egg': the_angry_egg as unknown as MerchantConfig,
  'the-barony-bar': the_barony_bar as unknown as MerchantConfig,
  'the-basement': the_basement as unknown as MerchantConfig,
  'the-beamhouse': the_beamhouse as unknown as MerchantConfig,
  'the-beehive': the_beehive as unknown as MerchantConfig,
  'the-benjamin': the_benjamin as unknown as MerchantConfig,
  'the-big-dipper-ice-cream-parlour': the_big_dipper_ice_cream_parlour as unknown as MerchantConfig,
  'the-billiards-cafe': the_billiards_cafe as unknown as MerchantConfig,
  'the-black-duck': the_black_duck as unknown as MerchantConfig,
  'the-black-horse': the_black_horse as unknown as MerchantConfig,
  'the-black-rock-oyster-bar-grill': the_black_rock_oyster_bar_grill as unknown as MerchantConfig,
  'the-black-rose': the_black_rose as unknown as MerchantConfig,
  'the-blue-duck': the_blue_duck as unknown as MerchantConfig,
  'the-blue-elephant-trail': the_blue_elephant_trail as unknown as MerchantConfig,
  'the-boca-oyster-bar': the_boca_oyster_bar as unknown as MerchantConfig,
  'the-boot': the_boot as unknown as MerchantConfig,
  'the-botanist': the_botanist as unknown as MerchantConfig,
  'the-breakfast-express-by-mealionaire': the_breakfast_express_by_mealionaire as unknown as MerchantConfig,
  'the-brownstone': the_brownstone as unknown as MerchantConfig,
  'the-capital-grille': the_capital_grille as unknown as MerchantConfig,
  'the-cask-and-barrel': the_cask_and_barrel as unknown as MerchantConfig,
  'the-cheesecake-factory': the_cheesecake_factory as unknown as MerchantConfig,
  'the-cheesecake-shop': the_cheesecake_shop as unknown as MerchantConfig,
  'the-cone-zone': the_cone_zone as unknown as MerchantConfig,
  'the-copper-fox-saloon': the_copper_fox_saloon as unknown as MerchantConfig,
  'the-cove-deli': the_cove_deli as unknown as MerchantConfig,
  'the-cracker-barrel-pub': the_cracker_barrel_pub as unknown as MerchantConfig,
  'the-deck-at-river-twist': the_deck_at_river_twist as unknown as MerchantConfig,
  'the-dock-restaurant': the_dock_restaurant as unknown as MerchantConfig,
  'the-dome-bar-and-grill': the_dome_bar_and_grill as unknown as MerchantConfig,
  'the-doric': the_doric as unknown as MerchantConfig,
  'the-dragon': the_dragon as unknown as MerchantConfig,
  'the-elephant-trail': the_elephant_trail as unknown as MerchantConfig,
  'the-farm-italy-at-ballo-italian': the_farm_italy_at_ballo_italian as unknown as MerchantConfig,
  'the-farm-kitchen-ct': the_farm_kitchen_ct as unknown as MerchantConfig,
  'the-farmers-cow-calfe': the_farmers_cow_calfe as unknown as MerchantConfig,
  'the-fire-place': the_fire_place as unknown as MerchantConfig,
  'the-flour-girl-bakery': the_flour_girl_bakery as unknown as MerchantConfig,
  'the-flour-girl-cafe': the_flour_girl_cafe as unknown as MerchantConfig,
  'the-georgetown-owl-saloon-restaurant': the_georgetown_owl_saloon_restaurant as unknown as MerchantConfig,
  'the-ginger-man': the_ginger_man as unknown as MerchantConfig,
  'the-granola-bar': the_granola_bar as unknown as MerchantConfig,
  'the-green-man': the_green_man as unknown as MerchantConfig,
  'the-half-door': the_half_door as unknown as MerchantConfig,
  'the-harbor-coffeehouse': the_harbor_coffeehouse as unknown as MerchantConfig,
  'the-hare-and-hounds': the_hare_and_hounds as unknown as MerchantConfig,
  'the-hideaway': the_hideaway as unknown as MerchantConfig,
  'the-hop-pickers': the_hop_pickers as unknown as MerchantConfig,
  'the-hub-pizza-bar': the_hub_pizza_bar as unknown as MerchantConfig,
  'the-hungry-crab': the_hungry_crab as unknown as MerchantConfig,
  'the-ivory-restaurant-pub': the_ivory_restaurant_pub as unknown as MerchantConfig,
  'the-ivy': the_ivy as unknown as MerchantConfig,
  'the-juniper-library-charter-171197': the_juniper_library_charter_171197 as unknown as MerchantConfig,
  'the-kakery': the_kakery as unknown as MerchantConfig,
  'the-lake-tavern': the_lake_tavern as unknown as MerchantConfig,
  'the-landing': the_landing as unknown as MerchantConfig,
  'the-lansdowne-irish-pub-music-house': the_lansdowne_irish_pub_music_house as unknown as MerchantConfig,
  'the-lantern': the_lantern as unknown as MerchantConfig,
  'the-little-pub': the_little_pub as unknown as MerchantConfig,
  'the-loaded-goat-coffee-company': the_loaded_goat_coffee_company as unknown as MerchantConfig,
  'the-local-sixty-three': the_local_sixty_three as unknown as MerchantConfig,
  'the-lodge-bar': the_lodge_bar as unknown as MerchantConfig,
  'the-luke-brasserie': the_luke_brasserie as unknown as MerchantConfig,
  'the-magic-cup': the_magic_cup as unknown as MerchantConfig,
  'the-magnum': the_magnum as unknown as MerchantConfig,
  'the-main-pub': the_main_pub as unknown as MerchantConfig,
  'the-malt-shovel': the_malt_shovel as unknown as MerchantConfig,
  'the-market-at-the-meetinghouse': the_market_at_the_meetinghouse as unknown as MerchantConfig,
  'the-mill-on-the-river': the_mill_on_the_river as unknown as MerchantConfig,
  'the-moo-bar': the_moo_bar as unknown as MerchantConfig,
  'the-ninety-nine': the_ninety_nine as unknown as MerchantConfig,
  'the-north-house': the_north_house as unknown as MerchantConfig,
  'the-olde-mecklenburg-brewery': the_olde_mecklenburg_brewery as unknown as MerchantConfig,
  'the-olive-bar': the_olive_bar as unknown as MerchantConfig,
  'the-olive-branch-bistro': the_olive_branch_bistro as unknown as MerchantConfig,
  'the-one-korean-bbq-bar': the_one_korean_bbq_bar as unknown as MerchantConfig,
  'the-original-antonios': the_original_antonios as unknown as MerchantConfig,
  'the-owl': the_owl as unknown as MerchantConfig,
  'the-painted-pony': the_painted_pony as unknown as MerchantConfig,
  'the-pantry': the_pantry as unknown as MerchantConfig,
  'the-parkway-inn': the_parkway_inn as unknown as MerchantConfig,
  'the-pho-bar': the_pho_bar as unknown as MerchantConfig,
  'the-place-2-be': the_place_2_be as unknown as MerchantConfig,
  'the-place': the_place as unknown as MerchantConfig,
  'the-plum-tomato': the_plum_tomato as unknown as MerchantConfig,
  'the-port-of-call': the_port_of_call as unknown as MerchantConfig,
  'the-post': the_post as unknown as MerchantConfig,
  'the-quarterback-club': the_quarterback_club as unknown as MerchantConfig,
  'the-recovery-room': the_recovery_room as unknown as MerchantConfig,
  'the-red-hill-baker': the_red_hill_baker as unknown as MerchantConfig,
  'the-red-lion': the_red_lion as unknown as MerchantConfig,
  'the-restaurant-at-gillette-ridge': the_restaurant_at_gillette_ridge as unknown as MerchantConfig,
  'the-rooftop': the_rooftop as unknown as MerchantConfig,
  'the-royal-coachman-tavern': the_royal_coachman_tavern as unknown as MerchantConfig,
  'the-russell-grab-go': the_russell_grab_go as unknown as MerchantConfig,
  'the-russian-lady': the_russian_lady as unknown as MerchantConfig,
  'the-shack': the_shack as unknown as MerchantConfig,
  'the-shipwrights-daughter': the_shipwrights_daughter as unknown as MerchantConfig,
  'the-side-door-jazz-club': the_side_door_jazz_club as unknown as MerchantConfig,
  'the-sloppy-waffle': the_sloppy_waffle as unknown as MerchantConfig,
  'the-smithy-cafe': the_smithy_cafe as unknown as MerchantConfig,
  'the-social': the_social as unknown as MerchantConfig,
  'the-spence': the_spence as unknown as MerchantConfig,
  'the-spot': the_spot as unknown as MerchantConfig,
  'the-station-restaurant': the_station_restaurant as unknown as MerchantConfig,
  'the-towne-parlor': the_towne_parlor as unknown as MerchantConfig,
  'the-trinity': the_trinity as unknown as MerchantConfig,
  'the-triple-three-333': the_triple_three_333 as unknown as MerchantConfig,
  'the-village-restaurant': the_village_restaurant as unknown as MerchantConfig,
  'the-village-table': the_village_table as unknown as MerchantConfig,
  'the-waters-edge-at-giovannis': the_waters_edge_at_giovannis as unknown as MerchantConfig,
  'the-waterview': the_waterview as unknown as MerchantConfig,
  'the-wedge-inn': the_wedge_inn as unknown as MerchantConfig,
  'the-whale-tea': the_whale_tea as unknown as MerchantConfig,
  'the-white-horse-country-pub': the_white_horse_country_pub as unknown as MerchantConfig,
  'the-white-rabbit-coffee-house': the_white_rabbit_coffee_house as unknown as MerchantConfig,
  'the-whole-enchilada': the_whole_enchilada as unknown as MerchantConfig,
  'the-wicked-press': the_wicked_press as unknown as MerchantConfig,
  'the-winged-bear': the_winged_bear as unknown as MerchantConfig,
  'the-wu': the_wu as unknown as MerchantConfig,
  'thecafe': thecafe as unknown as MerchantConfig,
  'these-guys-brewing-company': these_guys_brewing_company as unknown as MerchantConfig,
  'third-barrel-brewing': third_barrel_brewing as unknown as MerchantConfig,
  'thistle-street-bar': thistle_street_bar as unknown as MerchantConfig,
  'thread-city-diner': thread_city_diner as unknown as MerchantConfig,
  'three-notchd-brewing-co': three_notchd_brewing_co as unknown as MerchantConfig,
  'tibetan-himalyan': tibetan_himalyan as unknown as MerchantConfig,
  'tiernans-bar-pub': tiernans_bar_pub as unknown as MerchantConfig,
  'tiger-belly-noodle-bar': tiger_belly_noodle_bar as unknown as MerchantConfig,
  'tigin-irish-pub': tigin_irish_pub as unknown as MerchantConfig,
  'tiles-bar': tiles_bar as unknown as MerchantConfig,
  'tinderbox': tinderbox as unknown as MerchantConfig,
  'ting-ho': ting_ho as unknown as MerchantConfig,
  'tisumi-sushi': tisumi_sushi as unknown as MerchantConfig,
  'toast-co': toast_co as unknown as MerchantConfig,
  'toast-coffee-house': toast_coffee_house as unknown as MerchantConfig,
  'toast-four-corners': toast_four_corners as unknown as MerchantConfig,
  'toasted': toasted as unknown as MerchantConfig,
  'tokyo-ramen': tokyo_ramen as unknown as MerchantConfig,
  'tolland-pizza': tolland_pizza as unknown as MerchantConfig,
  'tomato-basil': tomato_basil as unknown as MerchantConfig,
  'tomlinsons-restaurant': tomlinsons_restaurant as unknown as MerchantConfig,
  'tomo-202': tomo_202 as unknown as MerchantConfig,
  'tonos-mexican-restaurant': tonos_mexican_restaurant as unknown as MerchantConfig,
  'tonys-deli': tonys_deli as unknown as MerchantConfig,
  'tonys-flour-shop': tonys_flour_shop as unknown as MerchantConfig,
  'tonys-pizza': tonys_pizza as unknown as MerchantConfig,
  'torito-mexican': torito_mexican as unknown as MerchantConfig,
  'toro': toro as unknown as MerchantConfig,
  'toscana-italian-restaurant': toscana_italian_restaurant as unknown as MerchantConfig,
  'toshi': toshi as unknown as MerchantConfig,
  'town-brewing-co': town_brewing_co as unknown as MerchantConfig,
  'tox-brewing-company': tox_brewing_company as unknown as MerchantConfig,
  'tpk-brewing-co': tpk_brewing_co as unknown as MerchantConfig,
  'trackside-deli': trackside_deli as unknown as MerchantConfig,
  'trevi-lounge': trevi_lounge as unknown as MerchantConfig,
  'tribus-beer-company': tribus_beer_company as unknown as MerchantConfig,
  'trigo-wood-fired-pizza': trigo_wood_fired_pizza as unknown as MerchantConfig,
  'trillium-brewing': trillium_brewing as unknown as MerchantConfig,
  'trinfo-cafe-technology-center': trinfo_cafe_technology_center as unknown as MerchantConfig,
  'triple-aaa-pizza-restaurant': triple_aaa_pizza_restaurant as unknown as MerchantConfig,
  'triple-tri-coffee': triple_tri_coffee as unknown as MerchantConfig,
  'troupe429': troupe429 as unknown as MerchantConfig,
  'trumbull-kitchen': trumbull_kitchen as unknown as MerchantConfig,
  'ts-pizza-cafe': ts_pizza_cafe as unknown as MerchantConfig,
  'tunxis-grill-pizzeria': tunxis_grill_pizzeria as unknown as MerchantConfig,
  'turks-seafood-resturant-of-savin-rock': turks_seafood_resturant_of_savin_rock as unknown as MerchantConfig,
  'turtle-cafe': turtle_cafe as unknown as MerchantConfig,
  'twins-bbq-co': twins_bbq_co as unknown as MerchantConfig,
  'two-roads-food-hall-bar': two_roads_food_hall_bar as unknown as MerchantConfig,
  'uncle-bucks-fishbowl-grill': uncle_bucks_fishbowl_grill as unknown as MerchantConfig,
  'uncle-krankys-cafe': uncle_krankys_cafe as unknown as MerchantConfig,
  'uncle-leos-rosny-cafe': uncle_leos_rosny_cafe as unknown as MerchantConfig,
  'undercroft': undercroft as unknown as MerchantConfig,
  'union-league-cafe': union_league_cafe as unknown as MerchantConfig,
  'unionville-family-restaurant': unionville_family_restaurant as unknown as MerchantConfig,
  'uno-mas': uno_mas as unknown as MerchantConfig,
  'upper-crust-bakery-cafe': upper_crust_bakery_cafe as unknown as MerchantConfig,
  'upper-crust': upper_crust as unknown as MerchantConfig,
  'uptown-sandwiches': uptown_sandwiches as unknown as MerchantConfig,
  'urban-angel': urban_angel as unknown as MerchantConfig,
  'urban-lodge-brewing': urban_lodge_brewing as unknown as MerchantConfig,
  'urban-street-pizza': urban_street_pizza as unknown as MerchantConfig,
  'utica-bread': utica_bread as unknown as MerchantConfig,
  'utsav': utsav as unknown as MerchantConfig,
  'valbella-ristorante': valbella_ristorante as unknown as MerchantConfig,
  'valentinos-restaurant': valentinos_restaurant as unknown as MerchantConfig,
  'valley-diner': valley_diner as unknown as MerchantConfig,
  'van-leeuwen-ice-cream': van_leeuwen_ice_cream as unknown as MerchantConfig,
  'vanilla-bean-cafe': vanilla_bean_cafe as unknown as MerchantConfig,
  'vans': vans as unknown as MerchantConfig,
  'vault-coffeedeviant-donuts': vault_coffeedeviant_donuts as unknown as MerchantConfig,
  'vavalas-deli-catering-darien': vavalas_deli_catering_darien as unknown as MerchantConfig,
  'vazzys': vazzys as unknown as MerchantConfig,
  'vernon-diner': vernon_diner as unknown as MerchantConfig,
  'vespino': vespino as unknown as MerchantConfig,
  'vessel': vessel as unknown as MerchantConfig,
  'vi-vi-bubble-tea': vi_vi_bubble_tea as unknown as MerchantConfig,
  'via-emilia': via_emilia as unknown as MerchantConfig,
  'vietnam-palace': vietnam_palace as unknown as MerchantConfig,
  'villa-pizza': villa_pizza as unknown as MerchantConfig,
  'village-pizza': village_pizza as unknown as MerchantConfig,
  'vinette': vinette as unknown as MerchantConfig,
  'vinnys-ale-house': vinnys_ale_house as unknown as MerchantConfig,
  'vinnys-backyard-bbq': vinnys_backyard_bbq as unknown as MerchantConfig,
  'viron-rondo-osteria': viron_rondo_osteria as unknown as MerchantConfig,
  'viva-zapata': viva_zapata as unknown as MerchantConfig,
  'vivien': vivien as unknown as MerchantConfig,
  'vivo': vivo as unknown as MerchantConfig,
  'voodoo-rooms': voodoo_rooms as unknown as MerchantConfig,
  'wafu': wafu as unknown as MerchantConfig,
  'wagamama': wagamama as unknown as MerchantConfig,
  'wahaca': wahaca as unknown as MerchantConfig,
  'ward-9': ward_9 as unknown as MerchantConfig,
  'washington-pizza-house': washington_pizza_house as unknown as MerchantConfig,
  'washington-street-coffee-house': washington_street_coffee_house as unknown as MerchantConfig,
  'west-end-pizza': west_end_pizza as unknown as MerchantConfig,
  'west-main-pizza': west_main_pizza as unknown as MerchantConfig,
  'west-street-grill': west_street_grill as unknown as MerchantConfig,
  'wetlands-brew-pub-sports-bar': wetlands_brew_pub_sports_bar as unknown as MerchantConfig,
  'whale-tea': whale_tea as unknown as MerchantConfig,
  'when-pigs-fly': when_pigs_fly as unknown as MerchantConfig,
  'whiplash-beer': whiplash_beer as unknown as MerchantConfig,
  'white-horse': white_horse as unknown as MerchantConfig,
  'white-lotus-thai-restaurant-pho': white_lotus_thai_restaurant_pho as unknown as MerchantConfig,
  'whiting-lane-little-free-library-charter-43300': whiting_lane_little_free_library_charter_43300 as unknown as MerchantConfig,
  'whole-deli': whole_deli as unknown as MerchantConfig,
  'wicked-slice-pizza': wicked_slice_pizza as unknown as MerchantConfig,
  'wicked-slice': wicked_slice as unknown as MerchantConfig,
  'wicked-sweets-by-alyssa': wicked_sweets_by_alyssa as unknown as MerchantConfig,
  'willamette-brewery': willamette_brewery as unknown as MerchantConfig,
  'willimantic-brewing-company': willimantic_brewing_company as unknown as MerchantConfig,
  'willington-pizza-too': willington_pizza_too as unknown as MerchantConfig,
  'winchester-cafe': winchester_cafe as unknown as MerchantConfig,
  'windmill-tavern': windmill_tavern as unknown as MerchantConfig,
  'winfield-street-coffee': winfield_street_coffee as unknown as MerchantConfig,
  'wings-n-pies': wings_n_pies as unknown as MerchantConfig,
  'witchdoctor': witchdoctor as unknown as MerchantConfig,
  'wood-n-tap-restaurant': wood_n_tap_restaurant as unknown as MerchantConfig,
  'woodbury-brewing-company': woodbury_brewing_company as unknown as MerchantConfig,
  'woods-end-delicatessen': woods_end_delicatessen as unknown as MerchantConfig,
  'world-of-beer-bar-and-kitchen': world_of_beer_bar_and_kitchen as unknown as MerchantConfig,
  'yankee-cider-company': yankee_cider_company as unknown as MerchantConfig,
  'yannis-pizza-restaurant': yannis_pizza_restaurant as unknown as MerchantConfig,
  'yorkside-pizza-restaurant': yorkside_pizza_restaurant as unknown as MerchantConfig,
  'your-pie': your_pie as unknown as MerchantConfig,
  'yue-se-chinese-yue-se-chinese': yue_se_chinese_yue_se_chinese as unknown as MerchantConfig,
  'yume-hibachi-steak-and-sushi': yume_hibachi_steak_and_sushi as unknown as MerchantConfig,
  'zarraffas-coffee': zarraffas_coffee as unknown as MerchantConfig,
  'zaytoon-mediterranean-cuisine': zaytoon_mediterranean_cuisine as unknown as MerchantConfig,
  'zaza': zaza as unknown as MerchantConfig,
  'zinas-cucina': zinas_cucina as unknown as MerchantConfig,
  'zinc-new-haven': zinc_new_haven as unknown as MerchantConfig,
  'zoiglhaus-brewing': zoiglhaus_brewing as unknown as MerchantConfig,
  'zois-pizza': zois_pizza as unknown as MerchantConfig,
  'zois': zois as unknown as MerchantConfig,
};
